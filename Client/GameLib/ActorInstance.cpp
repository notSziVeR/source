...
void CActorInstance::SetActorType(UINT eType)
{
	...
	if (m_eActorType == TYPE_RENDER_TARGET || eType == TYPE_RENDER_TARGET)
	{
		TraceError("CActorInstance[%p] type %d -> %d", this, m_eActorType, eType);
	}
#endif
...
{
	if (m_eRace >= 34001 && m_eRace <= 34999)
	{
		return true;
	}
	return false;
...
{
	if (m_eRace >= 20110 && m_eRace <= 20299)
	{
		return true;
	}
...
#ifdef ENABLE_RT_EXTENSION
bool CActorInstance::IsRenderTarget()
{
	if (TYPE_RENDER_TARGET == m_eActorType)
	{
		return true;
	}
...
{
	return (m_eActorType != TYPE_PC && GetScale().z > 0.0f) ? CGraphicThingInstance::GetHeight() * GetScale().z : CGraphicThingInstance::GetHeight();
}
