...
CGraphicSubImage * CItemData::GetIconImage()
{
	...
	if (GetIndex() == 90500)
	{
		char szLetterIcon[64 + 1];
		if (bLetterAscii)
		{
			_snprintf(szLetterIcon, sizeof(szLetterIcon), "icon/letters/%c.tga", bLetterAscii);
		}
		else
		{
			_snprintf(szLetterIcon, sizeof(szLetterIcon), "icon/letters/no_letter.tga");
		}

		CGraphicSubImage* letterIcon = NULL;
		if (CResourceManager::Instance().IsFileExist(szLetterIcon))
		{
			letterIcon = (CGraphicSubImage*)CResourceManager::Instance().GetResourcePointer(szLetterIcon);
		}
...
#ifdef ENABLE_FIND_LETTERS_EVENT
void CItemData::SetLetterAscii(BYTE letterAscii)
{
	if (letterAscii < 48 || (letterAscii > 57 && letterAscii < 65) || letterAscii > 90)
	{
		bLetterAscii = 0;
	}
	else
	{
		bLetterAscii = letterAscii;
	}
}
#endif
...
const std::string& CItemData::GetNameString() const
{
...
const char* CItemData::GetUseTypeString() const
{
	...
	case USE_ADD_COSTUME_ATTR_SPECIAL:
		return DEF_STR(USE_ADD_COSTUME_ATTR_SPECIAL);
	case USE_REMOVE_COSTUME_ATTR:
		return DEF_STR(USE_REMOVE_COSTUME_ATTR);
#endif

#ifdef ENABLE_BELT_ATTR
	case USE_ADD_BELT_ATTR:
		return DEF_STR(USE_ADD_BELT_ATTR);
	case USE_CHANGE_BELT_ATTR:
		return DEF_STR(USE_CHANGE_BELT_ATTR);
#endif

#ifdef ENABLE_ORE_REFACTOR
	case USE_REMOVE_ACCESSORY_SOCKET:
		return DEF_STR(USE_REMOVE_ACCESSORY_SOCKET);
#endif
...
DWORD CItemData::GetAntiFlag() const
{
