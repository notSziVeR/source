...
void CActorInstance::AttachSash(DWORD dwParentPartIndex, DWORD dwPartIndex, CItemData* pItemData)
{
	if (!pItemData)
	{
		return;
	...
	if (CRaceData::PART_SASH == dwPartIndex)
	{
		RegisterModelThing(CRaceData::PART_SASH, pItemData->GetSubModelThing());
	}
	else
	{
		RegisterModelThing(dwPartIndex, pItemData->GetModelThing());
...
void CActorInstance::AttachSash(DWORD dwItemIndex, DWORD dwParentPartIndex, DWORD dwPartIndex)
{
	...
	AttachSash(dwParentPartIndex, CRaceData::PART_SASH, pItemData);
}
#endif
