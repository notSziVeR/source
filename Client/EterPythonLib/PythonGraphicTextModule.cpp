...
static inline void __pow(ULL base, ULL p, ULL& out)
{
	out = base;
...
PyObject* ConvertMoneyText(PyObject* poSelf, PyObject* poArgs)
{
	char* szString;
	...
	std::string szMoney(szString);
	std::replace(szMoney.begin(), szMoney.end(), ',', '.');

	if (std::count(szMoney.begin(), szMoney.end(), '.') > 1)
	{
		return Py_BuildNone();
	}

	size_t k = std::count(szMoney.begin(), szMoney.end(), 'k');
	ULL value = 0;
	if (k > 0)
	{
		__pow(1000, min(3, k), value);
		szMoney.erase(std::remove(szMoney.begin(), szMoney.end(), 'k'), szMoney.end());
	}

	long double val = ::atof(szMoney.c_str());
	return Py_BuildValue("L", static_cast<ULL>(val * value));
}
#undef MOD
...
void initgrpText()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "ConvertMoneyText",					ConvertMoneyText,							METH_VARARGS },
