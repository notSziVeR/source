...
void CGridSlotWindow::OnRenderPickingSlot()
{
	...
	if (GetPickedSlotList(byWidth, byHeight, &SlotList))
	{
		...
		else if (m_isSwitchMode && m_isSwitchableItem)
		{
...
void CGridSlotWindow::OnMouseOver()
{
	...
	if (this == pPointWindow)
	{
		...
		{
			TSlot* pRealSlot;
			if (GetPickedSlotPointer(&pRealSlot, false) && pRealSlot->dwSlotNumber == m_dwOverSlotRealNumber && m_dwToolTipSlotNumber == 0)
			{
				return;
			}
			...
			if (pSlot->isItem)
			{
				PyCallClassMemberFunc(m_poHandler, "OnOverInItem", Py_BuildValue("(i)", pSlot->dwSlotNumber));
				m_dwToolTipSlotNumber = 0; // otherwise OnOverOutItem will be ignored
				m_dwOverSlotRealNumber = pRealSlot->dwSlotNumber;
				return;
			}
		}
	}

	m_dwOverSlotRealNumber = MAXDWORD;
	OnOverOutItem();
}

BOOL CGridSlotWindow::GetPickedSlotPointer(TSlot** ppSlot)
{
	return GetPickedSlotPointer(ppSlot, true);
}

BOOL CGridSlotWindow::GetPickedSlotPointer(TSlot** ppSlot, bool focusItem)
{
	if (!UI::CWindowManager::Instance().IsAttaching())
	{
	...
	for (std::list<TSlot*>::iterator itor = SlotList.begin(); itor != SlotList.end(); ++itor)
	{
		...
		else
		{
			...
			{
				if (focusItem && !pMinSlot->isItem && pSlot->isItem)
				{
					pMinSlot = pSlot;
				}
			}
