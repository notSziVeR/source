...
CWindowManager::CWindowManager()
	...
	m_lockedMousePositionWindow(NULL)
#endif
{
...
#ifdef ENABLE_RT_EXTENSION
CWindow* CWindowManager::RegisterRenderTarget(PyObject* object, const char* layer)
{
	assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(layer));

	auto window = static_cast<CWindow*>(new CRenderTarget(object));
	m_LayerWindowMap[layer]->AddChild(window);
	return window;
}
#endif
...
#ifdef ENABLE_RT_EXTENSION
	if (pWindow == m_lockedMousePositionWindow)
	{
		m_lockedMousePositionWindow = NULL;
	}
#endif
...
#ifdef ENABLE_RT_EXTENSION
void CWindowManager::LockCursorOnWindow(CWindow* pWindow)
{
	if (m_lockedMousePositionWindow != NULL)
	{
		UnlockCursor(m_lockedMousePositionWindow);
	}

	m_lockedMousePositionWindow = pWindow;
	m_lockedMousePosition.x = m_lMouseRealX;
	m_lockedMousePosition.y = m_lMouseRealY;
}

void CWindowManager::UnlockCursor(CWindow* pWindow)
{
	if (m_lockedMousePositionWindow != pWindow)
	{
		return;
	}
...
bool CWindowManager::UpdateLockedCursor(int mouseX, int mouseY, POINT* lpReturnPoint)
{
	if (!m_lockedMousePositionWindow)
	{
		return false;
	}

	int x_dif = mouseX - m_lockedMousePosition.x;
	int y_dif = mouseY - m_lockedMousePosition.y;
	lpReturnPoint->x = m_lockedMousePosition.x;
	lpReturnPoint->y = m_lockedMousePosition.y;

	if (x_dif != 0 || y_dif != 0)
	{
		m_lockedMousePositionWindow->OnUpdateLockedCursor(x_dif, y_dif);
	}
	return true;
}
#endif
...
bool CWindowManager::IsTop(CWindow * pWin)
{
	if (!pWin->HasParent())
	{
		return false;
	...
	CWindow * pParentWindow = pWin->GetParent();
	return pParentWindow->IsTop(pWin);
}
...
bool CWindowManager::RunMouseWheel(const int& nLen)
{
	CWindow* pWin = GetPointWindow();
	while (pWin)
	{
		if (pWin->OnScrollEvent(nLen))
		{
			return TRUE;
		}
...
{
	if (m_iIgnoreIMEReturnEndTime && timeGetTime() < m_iIgnoreIMEReturnEndTime)
	{
		return;
	}
	if (m_pLockWindow)
...
void CWindowManager::RunIMEUpdateSelectionEvent()
{
	if (m_pLockWindow)
	{
		m_pLockWindow->OnIMEUpdateSelectionEvent();
		return;
	}
	if (m_pActiveWindow)
		if (m_pActiveWindow->IsRendering())
		{
			if (m_pActiveWindow->OnIMEUpdateSelectionEvent())
			{
				return;
			}
		}
