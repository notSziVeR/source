...
int CIME::ms_curpos;
int CIME::ms_selpos;
bool CIME::ms_selactive;
int CIME::ms_lastpos;

UINT CIME::ms_clipboardFormat = 0;
...
CIME::CIME()
{
	...
	if (!ms_clipboardFormat)
	{
		ms_clipboardFormat = RegisterClipboardFormat("M2CLIPBOARD");
	}
}
...
void CIME::PasteTextFromClipBoard()
{
	...
	HANDLE handle = ms_clipboardFormat ? GetClipboardData(ms_clipboardFormat) : 0;
	if (!handle)
	{
...
void CIME::CopyTextToClipBoard()
{
	...
	TSelectionPos selpos = GetCurSelection();
	int startPos = std::get<0>(selpos);
	int endPos = std::get<1>(selpos);

	std::wstring realData;
	realData.assign(m_wText, startPos, endPos);
	...
	while (i < endPos - startPos)
	{
		...
		int tagType = GetTextTag(m_wText + startPos + i, endPos - startPos - i, curLen, extraInfo);

		if (isInLink)
		{
			if (tagType == TEXT_TAG_HYPERLINK_END)
			{
				isInLink = false;
			}
		}
		...
		{
			if (tagType == TEXT_TAG_PLAIN)
			{
				data += m_wText[startPos + i];
			}
	...
	// CF_TEXT
	int textLen = WideCharToMultiByte(ms_uOutputCodePage, 0, data.c_str(), data.length(), 0, 0, NULL, NULL);
	HGLOBAL handle = GlobalAlloc(GMEM_MOVEABLE, textLen + 1);
	if (handle)
	{
		char* clipboardText = (char*)GlobalLock(handle);
		WideCharToMultiByte(ms_uOutputCodePage, 0, data.c_str(), data.length(), clipboardText, textLen + 1, NULL, NULL);
		clipboardText[textLen] = '\0';
		GlobalUnlock(handle);
	...
	if (ms_clipboardFormat)
	{
		textLen = WideCharToMultiByte(ms_uOutputCodePage, 0, realData.c_str(), realData.length(), 0, 0, NULL, NULL);
		handle = GlobalAlloc(GMEM_MOVEABLE, textLen + 1);
		if (handle)
		{
			char* clipboardText = (char*)GlobalLock(handle);
			WideCharToMultiByte(ms_uOutputCodePage, 0, realData.c_str(), realData.length(), clipboardText, textLen + 1, NULL, NULL);
			clipboardText[textLen] = '\0';
			GlobalUnlock(handle);

			SetClipboardData(ms_clipboardFormat, handle);
		}
	}
...
/*---------------------------------------------------------------------------*/ /* Protected */
void CIME::IncCurPos(bool resetSelection)
{
	if (IsCurSelected() && resetSelection)
	{
		ms_curpos = MAX(ms_curpos, ms_selpos);
		ResetCurSelection();
		OnUpdateSelection();
		return;
	}
...
void CIME::DecCurPos(bool resetSelection)
{
	if (IsCurSelected() && resetSelection)
	{
		ms_curpos = MIN(ms_curpos, ms_selpos);
		ResetCurSelection();
		OnUpdateSelection();
		return;
	}
...
void CIME::ResetCurSelection()
{
	ms_selpos = -1;
	OnUpdateSelection(true);
}

void CIME::SetCurSelection()
{
	ms_selpos = ms_curpos;
	OnUpdateSelection(true);
}

std::tuple<int, int> CIME::GetCurSelection()
{
	if (!IsCurSelected())
	{
		return TSelectionPos(-1, -1);
	}
	...
	return TSelectionPos(start, end);
}

bool CIME::IsCurSelected()
{
	return ms_selpos >= 0 && ms_curpos >= 0 && ms_curpos != ms_selpos;
}

bool CIME::DelCurSelection()
{
	if (!IsCurSelected())
	{
		return false;
	...
	memmove(m_wText + startPos, m_wText + endPos, sizeof(wchar_t) * (ms_lastpos - endPos));
	ms_lastpos -= endPos - startPos;
...
bool CIME::OnUpdateSelection(bool checkIfSelected)
{
	if (checkIfSelected)
	{
		if (!ms_selactive && !IsCurSelected())
		{
			return false;
		}
...
void CIME::PasteString(const char* str)
{
	...
	int wstrLen = MultiByteToWideChar(ms_uInputCodePage, 0, begin, end - begin, wText, IMESTR_MAXLEN);
	InsertString(wText, wstrLen);
	if (ms_pEvent)
	{
		ms_pEvent->OnUpdate();
	}
...
void CIME::CheckInputLocale()
{
	...
	iRc;
	//snprintf(szCodePage, sizeof(szCodePage), "%u", CP_UTF8);
