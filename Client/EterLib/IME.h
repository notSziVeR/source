...
public:
	...
	typedef std::tuple<int, int> TSelectionPos;
...
protected:
	void IncCurPos(bool resetSelection);
	void DecCurPos(bool resetSelection);
	void SetCurPos(int offset);
	void DelCurPos();

	void ResetCurSelection();
	void SetCurSelection();
	TSelectionPos GetCurSelection();
	void CheckCurSelection();
	bool IsCurSelected();
	bool DelCurSelection();
...
public:
	...
	static int				ms_selpos;
	static bool				ms_selactive;
	...
	static UINT				ms_clipboardFormat;
};
