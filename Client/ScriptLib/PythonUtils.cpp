...
bool PyTuple_GetInteger(PyObject* poArgs, int pos, CountType* ret)
{
	int val;
	bool result = PyTuple_GetInteger(poArgs, pos, &val);
	*ret = CountType(val);
...
bool PyTuple_GetLongLong(PyObject* poArgs, int pos, int64_t* ret)
{
	if (pos >= PyTuple_Size(poArgs))
	{
		return false;
...
bool PyTuple_GetUnsignedInteger(PyObject* poArgs, int pos, unsigned int* ret)
{
	...
	*ret = (unsigned int)PyLong_AsLong(poItem);
	return true;
}
