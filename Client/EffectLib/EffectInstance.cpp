...
bool CEffectInstance::IsSpecialRender()
{
	return m_isSpecialRender;
}

void CEffectInstance::SetSpecialRender()
{
	m_isSpecialRender = true;

	std::for_each(m_ParticleInstanceVector.begin(), m_ParticleInstanceVector.end(), [](CParticleSystemInstance*& p) {
...
void CEffectInstance::__SetParticleData(CParticleSystemData * pData)
{
	...
	pInstance->SetParticleScale(m_fScaleParticle);
...
void CEffectInstance::__Initialize()
{
	...
	m_fScaleParticle = 1.0f;
	m_v3ScaleMesh.x = m_v3ScaleMesh.y = m_v3ScaleMesh.z = 1.0f;
