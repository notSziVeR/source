...
const int64_t POINT_MAGIC_NUMBER = 0xe73ac1da;
#else
...
#ifdef EXTANDED_GOLD_AMOUNT
	int64_t CPythonPlayer::SPlayerStatus::GetPoint(UINT ePoint)
#else
...
#ifdef EXTANDED_GOLD_AMOUNT
	void CPythonPlayer::SetStatus(DWORD dwType, int64_t lValue)
#else
...
#ifdef EXTANDED_GOLD_AMOUNT
	int64_t CPythonPlayer::GetStatus(DWORD dwType)
#else
...
void CPythonPlayer::SetCubeItem(const TCubeData & c_rkItemCubeInst, DWORD dwNpcVnum)
{
	TCubeMap::iterator it = m_vecCubeItems.find(dwNpcVnum);
	int i = 0;

	if (it != m_vecCubeItems.end())
	{
		bool bFound = false;
		for (i = 0; i < it->second.size(); ++i)
		{
			if (it->second[i].iIndex == c_rkItemCubeInst.iIndex)
			{
				it->second[i] = c_rkItemCubeInst;
				bFound = true;
			}
		}
		...
		if (!bFound)
		{
			it->second.push_back(c_rkItemCubeInst);
		}
	}
	else
	{
		TCubeVector emptyVec;
		emptyVec.push_back(c_rkItemCubeInst);
		m_vecCubeItems.insert(std::make_pair(dwNpcVnum, emptyVec));
	}
}

int CPythonPlayer::GetCubeListSize(DWORD dwNpcVnum)
{
	TCubeMap::iterator it = m_vecCubeItems.find(dwNpcVnum);
	if (it != m_vecCubeItems.end())
	{
...
const TCubeData * CPythonPlayer::GetCubeItem(DWORD dwNpcVnum, int iIndex) const
{
	TCubeMap::const_iterator it = m_vecCubeItems.find(dwNpcVnum);
	if (it != m_vecCubeItems.end())
	{
		if (it->second.empty())
		{
		...
		if (iIndex >= it->second.size())
		{
			return NULL;
		}

		return &it->second.at(iIndex);
	}

	return NULL;
}

int CPythonPlayer::CountSpecifyItem(DWORD dwVnum)
{
	int iCount = 0;
	...
	for (int i = 0; i < c_ItemSlot_Count; ++i)
	{
		...
		{
			iCount += c_rItemData.count;
		}
	}

	for (int i = c_DragonSoul_Inventory_Start; i < c_DragonSoul_Inventory_End; ++i)
	{
		const TItemData& c_rItemData = m_playerStatus.aDSItem[i];
		if (c_rItemData.vnum == dwVnum)
		{
			iCount += c_rItemData.count;
		}
...
void CPythonPlayer::SetLuckyBoxItem(const TLuckyBoxItem& rItemInstance)
{
	m_LuckyBoxItems.push_back(rItemInstance);
}

bool CPythonPlayer::GetLuckyBoxItem(BYTE bItemPos, const TLuckyBoxItem** ppItemInfo)
{
	if (bItemPos > m_LuckyBoxItems.size())
	{
		return false;
	}

	*ppItemInfo = &m_LuckyBoxItems[bItemPos];
...
CountType CPythonPlayer::GetItemCount(TItemPos Cell)
{
	if (!Cell.IsValidCell())
	{
		return 0;
...
CountType CPythonPlayer::GetItemCountByVnum(DWORD dwVnum)
{
	DWORD dwCount = 0;
...
DWORD CPythonPlayer::GetItemMetinSocket(TItemPos Cell, DWORD dwMetinSocketIndex)
{
	...
	return itemData->alSockets[dwMetinSocketIndex];
}
...
void CPythonPlayer::GetItemAttribute(TItemPos Cell, DWORD dwAttrSlotIndex, BYTE * pbyType, short * psValue)
{
	...
	*pbyType = itemData->aAttr[dwAttrSlotIndex].bType;
	*psValue = itemData->aAttr[dwAttrSlotIndex].sValue;
}
...
bool CPythonPlayer::IsSpecialStorageSlot(TItemPos Cell)
{
	return Cell.IsSpecialStorageCell();
}
#endif
...
bool isIgnored(CItemData* pItemData)
{
	if (pItemData->GetType() == CItemData::ITEM_TYPE_WEAPON)
	{
		if (CPythonSystem::Instance().IsIgnoring(0))
		{
			return true;
		}
	}
	else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_BODY)
	{
		if (CPythonSystem::Instance().IsIgnoring(1))
		{
			return true;
		}
	}
	else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_HEAD)
	{
		if (CPythonSystem::Instance().IsIgnoring(2))
		{
			return true;
		}
	}
	else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_SHIELD)
	{
		if (CPythonSystem::Instance().IsIgnoring(3))
		{
			return true;
		}
	}
	else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_WRIST)
	{
		if (CPythonSystem::Instance().IsIgnoring(4))
		{
			return true;
		}
	}
	else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_FOOTS)
	{
		if (CPythonSystem::Instance().IsIgnoring(5))
		{
			return true;
		}
	}
	else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_NECK)
	{
		if (CPythonSystem::Instance().IsIgnoring(6))
		{
			return true;
		}
	}
	else if (pItemData->GetType() == CItemData::ITEM_TYPE_ARMOR && pItemData->GetSubType() == CItemData::ARMOR_EAR)
	{
		if (CPythonSystem::Instance().IsIgnoring(7))
		{
			return true;
		}
	}
	...
	{
		if (CPythonSystem::Instance().IsIgnoring(8))
		{
			return true;
		}
	}
...
void CPythonPlayer::SendClickItemsPacket(std::vector<DWORD> ids)
{
	if (IsObserverMode())
	{
		return;
	...
	DWORD items[20];
	memset(items, 0, sizeof(DWORD));

	BYTE i = 0;
	for (std::vector<DWORD>::iterator it = ids.begin(); it != ids.end(); ++it)
	{
		if (i == 19)
		{
			break;
		...
		if (!CPythonItem::Instance().GetOwnership(item, &c_szOwnerName))
		{
			continue;
		}

		CItemData* pItemData;
		if (!CItemManager::Instance().GetItemDataPointer(CPythonItem::Instance().GetVirtualNumberOfGroundItem(item), &pItemData))
		{
			Tracenf("CPythonPlayer::SendClickItemPacket(dwIID=%d) : Non-exist item.", item);
			continue;
		}

		if (isIgnored(pItemData))
		{
			continue;
		}
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendItemsPickUpPacket(items);
}
...
void CPythonPlayer::ClearGameWindow(PyObject* ppyObject)
{
	if (m_ppyGameWindow != ppyObject)
	{
		return;
	}
...
void CPythonPlayer::ClearSkillDict()
{
	...
	for (iter = m_vecCubeItems.begin(); iter != m_vecCubeItems.end(); iter++)
	{
		(*iter).second.clear();
	}

	m_vecCubeItems.clear();
#endif
...
void CPythonPlayer::SetItemTransmutate(TItemPos Cell, DWORD trans_id)
{
	if (!Cell.IsValidCell())
	{
		return;
	...
	(const_cast <TItemData*>(GetItemData(Cell)))->transmutation_id = trans_id;
}

DWORD CPythonPlayer::GetItemTransmutate(TItemPos Cell)
{
	if (!Cell.IsValidCell())
	{
		return 0;
	...
	{
		return pItem->transmutation_id;
	}
}
#endif

#ifdef ENABLE_DS_SET
BYTE GetDSGrade(DWORD dwVnum)
{
	return (dwVnum / 1000) % 10;
}

BYTE GetDSStep(DWORD dwVnum)
{
	return (dwVnum / 100) % 10;
}

bool CPythonPlayer::_IsDSPageFull(const BYTE bPageIndex)
{
	...
	for (DWORD dwSlotIndex = dwSlotStartIndex; dwSlotIndex < dwSlotEndIndex; ++dwSlotIndex)
	{
		...
		const uint8_t iGrade = GetDSGrade(dwItemIndex);
		if (iGrade < DRAGON_SOUL_GRADE_LEGENDARY)
			return false;

		const auto iStep = GetDSStep(dwItemIndex);
		if (iStep < DRAGON_SOUL_STEP_LOWEST)
			return false;

		if (iSetType == 0) { iSetType = iStep; }

		if (iSetType != iStep) { return false; }
