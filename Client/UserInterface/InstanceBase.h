...
public:
	...
	static bool RegisterEffect(UINT eEftType, const char* c_szEftAttachBone, const char* c_szEftName, bool isCache, const char* name = "");
...
public:
	void	AttachDetachEffect(DWORD dwEID, bool isHide);
	void 	CheckAttachedEffects(DWORD dwEID);

	void 	SetInstanceHideEffect(bool bFlag);
#endif
...
#ifdef ENABLE_SASH_COSTUME_SYSTEM
	bool					SetSash(DWORD eSash);
	void					ChangeSash(DWORD eSash);
#endif
#ifdef ENABLE_REFINE_ELEMENT
...
#ifdef ENABLE_SHAMAN_SYSTEM
	void SetShaman() { m_dwShaman = true; }
	BOOL IsShaman() { return m_dwShaman; }
#endif
...
public:
	...
	public:
		void SetSkillPreview(UINT uSkill, BYTE bSkillSet);
#endif
...
inline int JobToRace(int job, int sex)
{
	int race = job + sex * 4;
