...
PyObject * itemGetApplyID(PyObject * poSelf, PyObject * poArgs)
{
	char* name;
	...
	return Py_BuildValue("b", CItemManager::Instance().GetApplyTypeByNameEquipment(name));
}
...
PyObject * itemGetItemType(PyObject * poSelf, PyObject * poArgs)
{
	...
	{
		pItemData = CItemManager::Instance().GetSelectedItemDataPointer();
		if (!pItemData)
		{
			return Py_BuildException("no selected item data");
		}
	...
	else
	{
		...
		{
			return Py_BuildException("invalid item data by vnum %d", vnum);
		}
	}
...
PyObject * itemCanAddToQuickSlotItem(PyObject * poSelf, PyObject * poArgs)
{
	...
	case CItemData::ITEM_TYPE_QUEST:
	case CItemData::ITEM_TYPE_TOGGLE:
		return Py_BuildValue("i", TRUE);
	default:
		return Py_BuildValue("i", FALSE);
	}
...
PyObject* itemGetAntiFlag(PyObject* poSelf, PyObject* poArgs)
{
	CItemData* pItemData = CItemManager::Instance().GetSelectedItemDataPointer();
	if (!pItemData)
	{
	...
	return Py_BuildValue("i", pItemData->GetAntiFlag());
}
#endif
...
PyObject* itemGetItemsByName(PyObject* poSelf, PyObject* poArgs)
{
	...
	{
		func_name = "AddOfflineSearchItem";
	}

	std::map<DWORD, std::string> items = CItemManager::Instance().GetItemByName(name);
	for (std::map<DWORD, std::string>::iterator it = items.begin(); it != items.end(); it++)
	{
...
PyObject* itemGetItemEquipmentSetId(PyObject* poSelf, PyObject* poArgs) {
	uint32_t index = 0;
	...
	if (!CItemManager::Instance().GetItemDataPointer(index, &itemData))
		return Py_BuildValue("I", 0);

	return Py_BuildValue("I", itemData->GetEquipmentSetId());
}
...
void initItem()
{
	...
	{
		{ "GetApplyID",						itemGetApplyID,							METH_VARARGS },
		...
		{ "GetItemEquipmentSetId",			itemGetItemEquipmentSetId,				METH_VARARGS },
...
#ifdef TRANSMUTATION_SYSTEM
		{ "GetAntiFlag",					itemGetAntiFlag,						METH_VARARGS },
#endif
	...
	PyModule_AddIntConstant(poModule, "WEAR_UNIQUE3",				c_Equipment_Unique3);
	PyModule_AddIntConstant(poModule, "WEAR_UNIQUE4",				c_Equipment_Unique4);
	...
	PyModule_AddIntConstant(poModule, "EQUIPMENT_AMULET",			c_Equipment_Amulet);
#endif
#endif

#ifdef ENABLE_SPECIAL_STORAGE
	PyModule_AddIntConstant(poModule, "SPECIAL_STORAGE_PAGE_LENGTH", c_Special_Storage_Page_Length);
	PyModule_AddIntConstant(poModule, "SPECIAL_STORAGE_PAGE_BREADTH", c_Special_Storage_Page_Breadth);
	PyModule_AddIntConstant(poModule, "SPECIAL_STORAGE_PAGE_SIZE", c_Special_Storage_Page_Length * c_Special_Storage_Page_Breadth);
	PyModule_AddIntConstant(poModule, "SPECIAL_STORAGE_PAGE_COUNT", c_Special_Storage_Page_Count);
	PyModule_AddIntConstant(poModule, "SPECIAL_STORAGE_COUNT", c_Special_Storage_Count);

	PyModule_AddIntConstant(poModule, "SPECIAL_STORAGE_START_CELL", c_Special_Storage_Slot_Start);
#endif
	...
	PyModule_AddIntConstant(poModule, "ITEM_TYPE_TOGGLE",			CItemData::ITEM_TYPE_TOGGLE);
	PyModule_AddIntConstant(poModule, "TOGGLE_AUTO_RECOVERY_HP",	CItemData::TOGGLE_AUTO_RECOVERY_HP);
	PyModule_AddIntConstant(poModule, "TOGGLE_AUTO_RECOVERY_SP",	CItemData::TOGGLE_AUTO_RECOVERY_SP);
	PyModule_AddIntConstant(poModule, "TOGGLE_AFFECT",				CItemData::TOGGLE_AFFECT);
	PyModule_AddIntConstant(poModule, "TOGGLE_PET",					CItemData::TOGGLE_PET);
#ifdef ENABLE_ALTERNATIVE_MOUNT_SYSTEM
	PyModule_AddIntConstant(poModule, "TOGGLE_MOUNT",				CItemData::TOGGLE_MOUNT);
#endif
#ifdef ENABLE_TOGGLE_SYSTEM
	PyModule_AddIntConstant(poModule, "TOGGLE_RED_SOUL", CItemData::TOGGLE_RED_SOUL);
	PyModule_AddIntConstant(poModule, "TOGGLE_BLUE_SOUL", CItemData::TOGGLE_BLUE_SOUL);
#endif
	PyModule_AddIntConstant(poModule, "TOGGLE_PICKUPER", CItemData::TOGGLE_PICKUPER);
#endif

#ifdef ENABLE_SHAMAN_SYSTEM
	PyModule_AddIntConstant(poModule, "TOGGLE_SHAMAN", CItemData::TOGGLE_SHAMAN);
#endif
	...
	PyModule_AddIntConstant(poModule, "USE_ADD_ATTRIBUTE",			CItemData::USE_ADD_ATTRIBUTE);
	PyModule_AddIntConstant(poModule, "USE_ADD_ATTRIBUTE2",			CItemData::USE_ADD_ATTRIBUTE2);
	PyModule_AddIntConstant(poModule, "USE_CHANGE_ATTRIBUTE",		CItemData::USE_CHANGE_ATTRIBUTE);
	...
	PyModule_AddIntConstant(poModule, "USE_ADD_COSTUME_ATTR_SPECIAL", CItemData::USE_ADD_COSTUME_ATTR_SPECIAL);
	PyModule_AddIntConstant(poModule, "USE_REMOVE_COSTUME_ATTR",	CItemData::USE_REMOVE_COSTUME_ATTR);
#endif

#ifdef ENABLE_BELT_ATTR
	PyModule_AddIntConstant(poModule, "USE_ADD_BELT_ATTR",			CItemData::USE_ADD_BELT_ATTR);
	PyModule_AddIntConstant(poModule, "USE_CHANGE_BELT_ATTR",		CItemData::USE_CHANGE_BELT_ATTR);
#endif

#ifdef ENABLE_ORE_REFACTOR
	PyModule_AddIntConstant(poModule, "USE_REMOVE_ACCESSORY_SOCKET", CItemData::USE_REMOVE_ACCESSORY_SOCKET);
#endif
	...
	PyModule_AddIntConstant(poModule, "ITEM_TYPE_LUCKY_BOX", CItemData::ITEM_TYPE_LUCKY_BOX);
#endif

	PyModule_AddIntConstant(poModule, "APPLY_RESIST_ALL", CItemData::APPLY_RESIST_ALL);
	...
	PyModule_AddIntConstant(poModule, "APPLY_PRECISION", CItemData::APPLY_PRECISION);
	PyModule_AddIntConstant(poModule, "APPLY_DUNGEON_DAMAGE_BONUS", CItemData::APPLY_DUNGEON_DAMAGE_BONUS);
	PyModule_AddIntConstant(poModule, "APPLY_DUNGEON_RECV_DAMAGE_BONUS", CItemData::APPLY_DUNGEON_RECV_DAMAGE_BONUS);
	PyModule_AddIntConstant(poModule, "APPLY_AGGRO_MONSTER_BONUS", CItemData::APPLY_AGGRO_MONSTER_BONUS);
...
#ifdef ENABLE_AMULET_SYSTEM
	PyModule_AddIntConstant(poModule, "ITEM_TYPE_AMULET",			CItemData::ITEM_TYPE_AMULET);
	PyModule_AddIntConstant(poModule, "AMULET_SUB_TYPE_NORMAL",		CItemData::AMULET_NORMAL);
	PyModule_AddIntConstant(poModule, "AMULET_SUB_TYPE_RARE",		CItemData::AMULET_RARE);
	PyModule_AddIntConstant(poModule, "AMULET_SUB_TYPE_LEGENDARY",	CItemData::AMULET_LEGENDARY);
#endif
}
