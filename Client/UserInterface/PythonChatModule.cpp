...
PyObject* chatDestroyWhisper(PyObject* poSelf, PyObject* poArgs)
{
	CPythonChat::Instance().DestroyWhisper();
	return Py_BuildNone();
}
#endif
...
PyObject * chatGetLinkFromHyperlink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (0 == results[0].compare("item"))
	{
		...
		std::string vnum(results[1].c_str());
		std::vector<std::string> vnums;
		split_string(vnum, "|", vnums, false);

		if (vnums.size() == 0)
		{
			vnums.push_back(results[1].c_str());
			vnums.push_back("0");
		}
		else if (vnums.size() == 1)
		{
			vnums.push_back("0");
		}

		if (CItemManager::Instance().GetItemDataPointer(htoi(vnums[0].c_str()), &pItemData))
		{
			char buf[1024] = { 0 };
			char itemlink[256];
			int len;
			...
			len = snprintf(itemlink, sizeof(itemlink), "item:%x|%x:%lu:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x",
						   htoi(vnums[0].c_str()),
						   htoi(vnums[1].c_str()),
						   ...
						   htoi(results[12].c_str()),
						   htoi(results[13].c_str())
						  );

			if (results.size() >= 13)
			{
				for (int i = 14; i < results.size(); i += 2)
				{
					len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%x:%d",
									htoi(results[i].c_str()),
									static_cast<int>(atoi(results[i + 1].c_str())));
					isAttr = true;
				}
			}
...
#ifdef ENABLE_RENEWAL_WHISPER
		{ "DestroyWhisper",			chatDestroyWhisper,			METH_VARARGS },
#endif
