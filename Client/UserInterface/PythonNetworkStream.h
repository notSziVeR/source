...
{
	friend PyObject* offlineshopOpenOfflineShop(PyObject* poSelf, PyObject* poArgs);
...
public:
	...
	bool SendItemsPickUpPacket(DWORD items[20]);
#endif
	...
	bool SendShopBuyPacketMultiple(BYTE slotPos, DWORD count);
#endif
	bool SendShopSellPacket(BYTE bySlot);
	bool SendShopSellPacketNew(WORD wSlot, CountType byCount, BYTE byType);
	...
	bool SendSafeBoxItemMovePacket(BYTE bySourcePos, BYTE byTargetPos, CountType byCount);
	...
	bool SendRefinePacket(BYTE byPos, BYTE byType, bool fast_refine = false);
#else
...
public:
	bool SendRequestTreasureBoxLoot(DWORD dwItemVnum);

private:
	bool RecvRequestTreasureBoxLoot();
#endif

#ifdef ENABLE_TARGET_MONSTER_LOOT
public:
	bool SendLoadTargetInfo(DWORD dwVID);
...
public:
	bool SendRequestDeleteSocket(ESubHeader, WORD);

private:
	bool RecvRequestDeleteSocket();
#endif
...
public:
	bool SendVoiceStart(short* bData);
	bool RecvDataVoiceChat();
#endif

#ifdef ENABLE_ADMIN_MANAGER
public:
	bool SendAdminManagerHead(BYTE bSubHeader);
	bool SendAdminManager(BYTE bSubHeader, const void* c_pvData, int iSize = 0);
...
public:
	friend PyObject* nonplayerGetMonstersByName(PyObject* poSelf, PyObject* poArgs);
#endif

#ifdef OX_EVENT_SYSTEM_ENABLE
public:
	friend PyObject* itemGetItemsByName(PyObject* poSelf, PyObject* poArgs);
#endif
...
public:
	bool	SendMarbleManagerAction(BYTE bSubHeader);

protected:
	bool	RecvMarbleManager();
#endif

#ifdef ENABLE_CUBE_RENEWAL
public:
	bool SendCubeClose();
	bool SendCubeMake(DWORD dwNpcVnum, int iIndex, BYTE bMakeCount, BYTE bIsIncrease);
protected:
	bool RecvCubePacket();
	bool RecvCubeItemPacket();
#endif

#ifdef ENABLE_FIND_LETTERS_EVENT
protected:
	bool RecvFindLettersInfo();
#endif
...
public:
	bool SendLuckyBoxAction(BYTE bAction);
...
protected:
	// Teamler
	bool RecvShowTeamler();
	bool RecvTeamlerStatus();
#endif
...
protected:
	...
	bool RecvShopSearchBuyResult();
	bool RecvShopSearchOwnerMessage();
	bool RecvShopSearchSoldInfo();

public:
	bool SendShopSearchByName(const std::string& itemName, WORD page, BYTE entryCountIdx, BYTE sortType);
	bool SendShopSearchByOptions(const TShopSearchOptions& options, const TShopSearchItemType* itemTypeFlags, const DWORD* vnumList, WORD page, BYTE entryCountIdx, BYTE sortType);
	bool SendShopSearchBuy(TShopSearchItemID itemID, DWORD itemVnum, int64_t itemPrice);
	bool SendShopSearchOwnerMessage(DWORD ownerID);
	bool SendShopSearchRequestSoldInfo(DWORD itemVnum);
#endif

#ifdef __MULTI_LANGUAGE_SYSTEM_FLAG_VISIBILITY__
public:
	bool SendWhisperRequestLanguage(const std::string& targetName);

protected:
	bool RecvWhisperLanguageInfo();
#endif
...
public:
	DWORD GetCurrentPingTime() const { return m_lastPingTime; }

private:
	DWORD m_lastPingTimerSent;
	DWORD m_lastPingTime;
	bool m_waitForPingTimer;
#endif
};
