...
PyObject* backgroundGetMapLocaleName(PyObject* poSelf, PyObject* poArgs)
{
	int iX;
	if (!PyTuple_GetInteger(poArgs, 0, &iX))
	{
	...
	if (pkMapInfo)
	{
		return Py_BuildValue("sii", pkMapInfo->m_strLocaleName.c_str(), pkMapInfo->m_dwBaseX, pkMapInfo->m_dwBaseY);
	}
	else
	{
		return Py_BuildValue("sii", "", 0, 0);
...
void initBackground()
{
	static PyMethodDef s_methods[] =
	{
		...
		{"GetMapLocaleName",					backgroundGetMapLocaleName,					METH_VARARGS},
		{ NULL, NULL, NULL },
	};
