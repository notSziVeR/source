...
PyObject* chrJobToRace(PyObject* poSelf, PyObject* poArgs)
{
	int job;
	if (!PyTuple_GetInteger(poArgs, 0, &job))
	{
		return Py_BuildException();
	}

	int sex;
	if (!PyTuple_GetInteger(poArgs, 1, &sex))
	{
		return Py_BuildException();
	}

	return Py_BuildValue("i", JobToRace(job, sex));
}
...
{
	int iSash;
	if (!PyTuple_GetInteger(poArgs, 0, &iSash))
	{
		return Py_BadArgument();
	}
...
PyObject* chrGetPart(PyObject* poSelf, PyObject* poArgs)
{
	...
	auto pInstance = iVirtualId != 0 ? CPythonCharacterManager::Instance().GetInstancePtr(iVirtualId) : CPythonCharacterManager::Instance().GetMainInstancePtr();
	return Py_BuildValue("i", pInstance ? pInstance->GetPart((CRaceData::EParts)iPart) : 0);
}
...
void initchr()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "JobToRace",					chrJobToRace,							METH_VARARGS },
...
#ifdef ENABLE_ANTY_EXP
	PyModule_AddIntConstant(poModule, "AFFECT_EXP_CURSE",					CInstanceBase::AFFECT_EXP_CURSE);
#endif
...
#ifdef ENABLE_DS_SET
	PyModule_AddIntConstant(poModule, "AFFECT_DS_SET", CInstanceBase::AFFECT_DS_SET);
#endif

	PyModule_AddIntConstant(poModule, "NEW_AFFECT_POLYMORPH", CInstanceBase::NEW_AFFECT_POLYMORPH);

	PyModule_AddIntConstant(poModule, "SEX_FEMALE", 0);
	PyModule_AddIntConstant(poModule, "SEX_MALE", 1);

#ifdef ENABLE_HIDE_COSTUMES
	PyModule_AddIntConstant(poModule, "HIDE_COSTUME_HEAD", CInstanceBase::COSTUME_FLAG_HEAD);
	PyModule_AddIntConstant(poModule, "HIDE_COSTUME_BODY", CInstanceBase::COSTUME_FLAG_BODY);
	PyModule_AddIntConstant(poModule, "HIDE_COSTUME_WEAPON", CInstanceBase::COSTUME_FLAG_WEAPON);
#ifdef ENABLE_SASH_COSTUME_SYSTEM
	PyModule_AddIntConstant(poModule, "HIDE_COSTUME_SASH", CInstanceBase::COSTUME_FLAG_SASH);
#endif
#endif
}
