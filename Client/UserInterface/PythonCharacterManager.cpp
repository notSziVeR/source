...
void CPythonCharacterManager::ShowHideEffectById(DWORD dwAffectId, bool isHide)
{
	CPythonCharacterManager::CharacterIterator itor = CharacterInstanceBegin();
	CPythonCharacterManager::CharacterIterator itorEnd = CharacterInstanceEnd();
	for (; itor != itorEnd; ++itor)
	{
		...
		if (dwAffectId == 0 && !pInstance->IsPC())
		{
			pInstance->SetInstanceHideEffect(isHide);
		}

		if (!pInstance->IsPC() && dwAffectId != 0)
		{
			continue;
		}

		pInstance->AttachDetachEffect(dwAffectId, isHide);
	}
}
#endif
