...
{
	bool resetSelection;
	if (!PyTuple_GetBoolean(poArgs, 0, &resetSelection))
	{
		resetSelection = true;
	}

	CPythonIME::Instance().MoveLeft(resetSelection);
...
PyObject* imeMoveRight(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonIME::Instance().MoveRight(resetSelection);
...
PyObject* imeGetCursorPosition(PyObject* poSelf, PyObject* poArgs)
{
	int cursorPos = CPythonIME::Instance().GetCursorPosition();

	return Py_BuildValue("i", cursorPos);
}

PyObject* imeResetSelectedText(PyObject* poSelf, PyObject* poArgs)
{
	CPythonIME::Instance().ResetTextSelection();

	return Py_BuildNone();
}

PyObject* imeSetSelectedText(PyObject* poSelf, PyObject* poArgs)
{
	CPythonIME::Instance().SetTextSelection();

	return Py_BuildNone();
}

PyObject* imeIsTextSelected(PyObject* poSelf, PyObject* poArgs)
{
	std::tuple<int, int> selectionInfo = CPythonIME::Instance().GetTextSelected();
	return Py_BuildValue("b", std::get<0>(selectionInfo) >= 0 || std::get<1>(selectionInfo) >= 0);
}

PyObject* imeGetSelectedText(PyObject* poSelf, PyObject* poArgs)
{
	std::tuple<int, int> selectionInfo = CPythonIME::Instance().GetTextSelected();
	return Py_BuildValue("ii", std::get<0>(selectionInfo), std::get<1>(selectionInfo));
}
...
{
	CPythonIME::Instance().CopyTextToClipBoard();
	return Py_BuildNone();
}
...
void initime()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetCursorPosition",		imeGetCursorPosition,		METH_VARARGS },
		{ "ResetSelectedText",		imeResetSelectedText,		METH_VARARGS },
		{ "SetSelectedText",		imeSetSelectedText,			METH_VARARGS },
		{ "IsTextSelected",			imeIsTextSelected,			METH_VARARGS },
		{ "GetSelectedText",		imeGetSelectedText,			METH_VARARGS },
