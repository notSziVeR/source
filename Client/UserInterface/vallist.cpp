...
ValueListItem::ValueListItem(const std::string &name, double def, bool constant)
{
	m_name = name;
	m_constant = constant;
	m_value = m_def = def;
...
ValueListItem::ValueListItem(const std::string &name, double *ptr, double def, bool constant)
{
	m_name = name;
	m_constant = constant;
	m_value = m_def = def;
...
// Get the name
const std::string &ValueListItem::GetName() const
{
	return m_name;
}
...
// Add value to list
void ValueList::Add(const std::string &name, double def, bool constant)
{
	...
	{
		// Create value
		std::unique_ptr<ValueListItem> i(new ValueListItem(name, def, constant));
...
void ValueList::AddAddress(const std::string &name, double *ptr, double def, bool constant)
{
	if (GetAddress(name))
	{
		throw (AlreadyExistsException(name));
	...
	{
		// Create value
		std::unique_ptr<ValueListItem> i(new ValueListItem(name, ptr, def, constant));
...
double *ValueList::GetAddress(const std::string &name) const
{
	size_type pos;
...
bool ValueList::IsConstant(const std::string &name) const
{
	size_type pos;
...
// Get an item
void ValueList::Item(size_type pos, std::string *name, double *value) const
{
	if (name)
	{
