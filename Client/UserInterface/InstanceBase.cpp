...
bool CInstanceBase::Create(const SCreateData& c_rkCreateData)
{
...
#ifdef ENABLE_SHAMAN_SYSTEM
	if (c_rkCreateData.m_dwRace == SHAMAN_MOB_VNUM)
	{
		SetInstanceType(CActorInstance::TYPE_PC);
		if (!SetRace(MAIN_RACE_SHAMAN_W))
			return false;
		SetShaman();
	}
	else
...
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
	m_bLocale = c_rkCreateData.m_bLocale;
#endif
...
#ifdef ENABLE_SCALING_INSTANCES
	__RefreshScale();
#endif
	...
	if (c_rkCreateData.m_dwStateFlags & ADD_CHARACTER_STATE_SPAWN)
	{
		...
		if (IsAffect(AFFECT_SPAWN) && !CPythonSystem::Instance().IsHideEffects(CPythonSystem::HIDE_EFFECTS_GENERAL))
		{
	...
	uint32_t dwRank = CPythonNonPlayer::Instance().GetMobRank(c_rkCreateData.m_dwRace);
	uint32_t dwType = CPythonNonPlayer::Instance().GetMobType(c_rkCreateData.m_dwRace);
...
#ifdef ENABLE_HIDE_EFFECTS
	if (CPythonSystem::Instance().IsHideEffects(CPythonSystem::HIDE_EFFECTS_GENERAL))
	{
		m_GraphicThingInstance.SetBattleHitEffect(0);
	}
	else
	{
...
#ifdef ENABLE_HIDE_EFFECTS
	SetInstanceHideEffect(CPythonSystem::Instance().IsHideEffects(CPythonSystem::HIDE_EFFECTS_GENERAL));
#endif
...
#ifdef ENABLE_HIDE_EFFECTS
void CInstanceBase::SetInstanceHideEffect(bool bFlag)
{
	if (bFlag)
	{
...
void CInstanceBase::Update()
{
	...
	if (IsMount())
	{
		if (CPythonSystem::instance().IsHideObjects(CPythonSystem::HIDE_OBJECTS_MOUNTS))
		{
			if (!IsAffect(AFFECT_INVISIBILITY))
			{
	...
	if (IsPet())
	{
		if (CPythonSystem::instance().IsHideObjects(CPythonSystem::HIDE_OBJECTS_PETS))
		{
			if (!IsAffect(AFFECT_INVISIBILITY))
			{
				__SetAffect(AFFECT_INVISIBILITY, true);
...
bool CInstanceBase::IsAttackableInstance(CInstanceBase& rkInstVictim)
{
	...
	{
		const CPythonNonPlayer::TMobTable* pMobTable = CPythonNonPlayer::Instance().GetTable(rkInstVictim.GetVirtualNumber());
		if (pMobTable && pMobTable->dwVnum == SHIP_DEFEND_DUNGEON_ALLIE)
		{
			return false;
		}
	}
...
BOOL CInstanceBase::IsRenderTarget()
{
	return m_GraphicThingInstance.IsRenderTarget();
}
#endif
...
{
	m_awPart[CRaceData::PART_MAIN] = dwArmor;
	DWORD dwShape;
...
UINT CInstanceBase::__GetRefinedEffect(CItemData* pItem)
{
	...
	static const auto METIN_STONE_COUNT = 3;
	auto refine = std::max<uint32_t>(pItem->GetRefine() + pItem->GetSocketCount(),
									 METIN_STONE_COUNT) - METIN_STONE_COUNT;
...
#ifdef ENABLE_SASH_COSTUME_SYSTEM
bool CInstanceBase::SetSash(DWORD eSash)
{
	if (IsPoly() || IsWearingDress() || __IsShapeAnimalWear())
		return false;

	m_GraphicThingInstance.AttachSash(eSash, 0, CRaceData::PART_SASH);
	...
	CItemData* pItemData;
	if (!CItemManager::Instance().GetItemDataPointer(eSash, &pItemData))
	{
		m_GraphicThingInstance.SetAcceScale(1.0f, 1.0f, 1.0f);
		m_awPart[CRaceData::PART_SASH] = 0;
		return true;
	...
	m_awPart[CRaceData::PART_SASH] = eSash;
	return true;
}
#endif
...
#ifdef ENABLE_SASH_COSTUME_SYSTEM
	DWORD eSash = GetPart(CRaceData::PART_SASH);
#endif
...
#ifdef ENABLE_SASH_COSTUME_SYSTEM
void CInstanceBase::ChangeSash(DWORD eSash)
{
	if (eSash == m_GraphicThingInstance.GetPartItemID(CRaceData::PART_SASH))
	{
		return;
	}
...
UINT GetMotionIndexBySkillID(UINT uSkill)
{
	switch (uSkill)
	{
	case 1: case 31: case 61: case 91:
		return 76;
	case 2: case 32: case 62: case 92:
		return 77;
	case 3: case 33: case 63: case 93:
		return 78;
	case 4: case 34: case 64: case 94:
		return 79;
	case 5: case 35: case 65: case 95:
		return 80;
	case 6: case 36: case 66: case 96:
		return 81;

	case 16: case 46: case 76: case 106:
		return 91;
	case 17: case 47: case 77: case 107:
		return 92;
	case 18: case 48: case 78: case 108:
		return 93;
	case 19: case 49: case 79: case 109:
		return 94;
	case 20: case 50: case 80: case 110:
		return 95;
	case 21: case 51: case 81: case 111:
		return 96;
	}
}

void CInstanceBase::SetSkillPreview(UINT uSkill, BYTE bSkillSet)
{
	UINT uMot = GetMotionIndexBySkillID(uSkill);
	//if (bSkillSet)
	//	uMot += SKILL_COSTUME_EFFECT_END + (100 * (bSkillSet - 1));
	m_GraphicThingInstance.InterceptOnceMotion(CRaceMotionData::NAME_SKILL + uMot, 0.1f, uSkill, 1.0f);
}
#endif

#ifdef ENABLE_SCALING_INSTANCES
void CInstanceBase::__RefreshScale()
{
	uint32_t dwRank = CPythonNonPlayer::Instance().GetMobRank(m_dwRace);
	uint32_t dwType = CPythonNonPlayer::Instance().GetMobType(m_dwRace);

	bool bUpdate = false;

	if (dwType == CActorInstance::TYPE_RENDER_TARGET)
		return;

	if (dwType == CActorInstance::TYPE_STONE)
	{
		float fScale = CPythonConfig::instance().GetInteger(CPythonConfig::CLASS_OPTION, "SCALE_STONES", 100);
		fScale /= 100.0f;

		m_GraphicThingInstance.SetScaleWorld(fScale, fScale, fScale);
		bUpdate = true;
	}
	//else if (dwRank >= 4 && dwType == 0)
	//{
	//	float fScale = CPythonConfig::instance().GetInteger(CPythonConfig::CLASS_OPTION, "SCALE_BOSESS", 100);
	//	fScale /= 100.0f;
	//
	//	m_GraphicThingInstance.SetScaleWorld(fScale, fScale, fScale);
	//	bUpdate = true;
