...
public:
	typedef struct STextTail
	{
		...
		CGraphicExpandedImageInstance* pLanguageInstance;
#endif
...
public:
	...
	// For GameMaster
	void RegisterCharacterTextTail_GM(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR& c_rColor, float fAddHeight);
	...
	void SetItemTextTailOwner(DWORD dwVID, const char* c_szName, int iDropType = 0);
...
#ifdef __MULTI_LANGUAGE_SYSTEM_FLAG_VISIBILITY__
	void AttachLocaleFlag(DWORD dwVID, int localeID);
	void DetachLocaleFlag(DWORD dwVID);
#endif
