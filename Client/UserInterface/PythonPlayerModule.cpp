...
PyObject* playerClearGameWindow(PyObject* poSelf, PyObject* poArgs)
{
	PyObject* pyHandle;
	if (!PyTuple_GetObject(poArgs, 0, &pyHandle))
	{
	...
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.ClearGameWindow(pyHandle);
	return Py_BuildNone();
}
...
{
	auto dwEXP = CPythonPlayer::Instance().GetStatus(POINT_EXP);
...
PyObject* playerGetGayaCount(PyObject* poSelf, PyObject* poArgs)
{
...
PyObject * playerGetItemLink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		DWORD trans_id = CPythonPlayer::Instance().GetItemTransmutate(Cell);
#ifdef ENABLE_REFINE_ELEMENT
		DWORD refineElement = CPythonPlayer::Instance().GetItemRefineElement(Cell);
		len = snprintf(itemlink, sizeof(itemlink), "item:%x|%d:%d:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x:%x",
#else
		...
		len = snprintf(itemlink, sizeof(itemlink), "item:%x:%x:%x:%x:%x",
			...
			pPlayerItem->alSockets[6], pPlayerItem->alSockets[7], pPlayerItem->alSockets[8],
			pPlayerItem->alSockets[9]
		);
...
PyObject * playerIsSpecialStorageSlot(PyObject* poSelf, PyObject* poArgs)
{
	int iSlotIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iSlotIndex))
	{
	...
	char Flag = CPythonPlayer::Instance().IsSpecialStorageSlot(TItemPos(INVENTORY, iSlotIndex));
...
#ifdef TRANSMUTATION_SYSTEM
PyObject* playerGetItemTransmutate(PyObject* poSelf, PyObject* poArgs)
{
	switch (PyTuple_Size(poArgs))
	{
	case 1:
	{
		...
		int ItemNum = CPythonPlayer::Instance().GetItemTransmutate(TItemPos(INVENTORY, iSlotIndex));
		return Py_BuildValue("i", ItemNum);
	}
	case 2:
	{
		...
		int ItemNum = CPythonPlayer::Instance().GetItemTransmutate(Cell);
...
PyObject * playerGetCubeListSize(PyObject* poSelf, PyObject* poArgs)
{
	...
	return Py_BuildValue("i", CPythonPlayer::Instance().GetCubeListSize(iNpcVnum));
}

PyObject* playerCountSpecifyItem(PyObject* poSelf, PyObject* poArgs)
{
	int iVnum;
	if (!PyTuple_GetInteger(poArgs, 0, &iVnum))
	{
	...
	int itemCount = CPythonPlayer::Instance().CountSpecifyItem(iVnum);
...
PyObject * playerGetCubeItem(PyObject* poSelf, PyObject* poArgs)
{
	...
	const TCubeData * itemCube = CPythonPlayer::Instance().GetCubeItem(iNpcVnum, iPos);

	if (itemCube)
	{
		PyObject* poList = PyList_New(0);

		for (int i = 0; i < itemCube->materialV.size(); ++i)
		{
			PyObject* oMaterial = PyTuple_New(2);

			PyTuple_SetItem(oMaterial, 0, Py_BuildValue("i", itemCube->materialV[i].dwVnum));
			PyTuple_SetItem(oMaterial, 1, Py_BuildValue("i", itemCube->materialV[i].iCount));

			PyList_Append(poList, oMaterial);
		}

		return Py_BuildValue("iiKi(ii)bi(ii)O",


							 itemCube->iIndex, itemCube->iPercent, itemCube->dwGold, itemCube->bCategory,

							 itemCube->tReward.dwVnum, itemCube->tReward.iCount, itemCube->bCopyBonus,
							 itemCube->iIncPercent,
							 itemCube->tIncItem.dwVnum, itemCube->tIncItem.iCount,
							 poList);
	}
...
PyObject* playerGetLuckyBoxItemInfo(PyObject* poSelf, PyObject* poArgs)
{
	int iPos;
	...
	const TLuckyBoxItem* c_pItemData;

	if (CPythonPlayer::Instance().GetLuckyBoxItem(iPos, &c_pItemData))
	{
		return Py_BuildValue("iii", c_pItemData->dwVnum, c_pItemData->bCount, c_pItemData->bIsReward);
	}

	return Py_BuildValue("iii", 0, 0, 0);
}

PyObject* playerGetLuckyBoxVnum(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetLuckyBoxVnum());
}

PyObject* playerGetLuckyBoxPrice(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetLuckyBoxPrice());
}
#endif
...
void initPlayer()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "ClearGameWindow",			playerClearGameWindow,				METH_VARARGS },
...
#ifdef ENABLE_GAYA_SYSTEM
		{ "GetGayaCount",				playerGetGayaCount,					METH_VARARGS },
#endif
...
#ifdef ENABLE_SPECIAL_STORAGE
		{ "IsSpecialStorageSlot",			playerIsSpecialStorageSlot,			METH_VARARGS },
#endif
		// Refine
...
#ifdef TRANSMUTATION_SYSTEM
		{ "GetItemTransmutate",			playerGetItemTransmutate,			METH_VARARGS },
#endif

#ifdef ENABLE_CUBE_RENEWAL
		{ "GetCubeListSize",			playerGetCubeListSize,				METH_VARARGS },
		{ "GetCubeItem",				playerGetCubeItem,					METH_VARARGS },
		{ "CountSpecifyItem",			playerCountSpecifyItem,				METH_VARARGS },
#endif
...
#ifdef ENABLE_LUCKY_BOX
		{ "GetLuckyBoxItemInfo",		playerGetLuckyBoxItemInfo,			METH_VARARGS },
		{ "GetLuckyBoxVnum",			playerGetLuckyBoxVnum,				METH_VARARGS },
		{ "GetLuckyBoxPrice",			playerGetLuckyBoxPrice,				METH_VARARGS },
#endif
...
#ifdef ENABLE_SPECIAL_STORAGE
	PyModule_AddIntConstant(poModule, "SPECIAL_STORAGE_START_CELL", c_Belt_Inventory_Slot_End);
#endif
	...
	PyModule_AddIntConstant(poModule, "DUMMY_EXCHANGE",						DUMMY_EXCHANGE);
...
#ifdef ENABLE_CUBE_RENEWAL
	PyModule_AddIntConstant(poModule, "CUBE_WEAPONPVP",			CUBE_WEAPONPVP );
	PyModule_AddIntConstant(poModule, "CUBE_ARMORPVM",			CUBE_ARMORPVM );
	PyModule_AddIntConstant(poModule, "CUBE_ARMORPVP",			CUBE_ARMORPVP );
	PyModule_AddIntConstant(poModule, "CUBE_ACCESSORYPVM",		CUBE_ACCESSORYPVM );
	PyModule_AddIntConstant(poModule, "CUBE_ACCESSORYPVP",		CUBE_ACCESSORYPVP );
	PyModule_AddIntConstant(poModule, "CUBE_SHIELDSPVM",		CUBE_SHIELDSPVM);
	PyModule_AddIntConstant(poModule, "CUBE_SHIELDSPVP",		CUBE_SHIELDSPVP);
	PyModule_AddIntConstant(poModule, "CUBE_HELMETPVM",			CUBE_HELMETPVM);
	PyModule_AddIntConstant(poModule, "CUBE_HELMETPVP",			CUBE_HELMETPVP);
	PyModule_AddIntConstant(poModule, "CUBE_BRACELETPVM",		CUBE_BRACELETPVM);
	PyModule_AddIntConstant(poModule, "CUBE_BRACELETPVP",		CUBE_BRACELETPVP);
	PyModule_AddIntConstant(poModule, "CUBE_NECKLACEPVM",		CUBE_NECKLACEPVM);
	PyModule_AddIntConstant(poModule, "CUBE_NECKLACEPVP",		CUBE_NECKLACEPVP);
	PyModule_AddIntConstant(poModule, "CUBE_EARRINGSPVM",		CUBE_EARRINGSPVM);
	PyModule_AddIntConstant(poModule, "CUBE_EARRINGSPVP",		CUBE_EARRINGSPVP);
	PyModule_AddIntConstant(poModule, "CUBE_SHOESPVM",			CUBE_SHOESPVM);
	PyModule_AddIntConstant(poModule, "CUBE_SHOESPVP",			CUBE_SHOESPVP);
	PyModule_AddIntConstant(poModule, "CUBE_BELTPVM",			CUBE_BELTPVM);
	PyModule_AddIntConstant(poModule, "CUBE_BELTPVP",			CUBE_BELTPVP);
	...
	PyModule_AddIntConstant(poModule, "CUBE_MOUNT",				CUBE_MOUNT);
	PyModule_AddIntConstant(poModule, "CUBE_SOULS",				CUBE_SOULS);
	PyModule_AddIntConstant(poModule, "CUBE_SOULS_SCROLLS",		CUBE_SOULS_SCROLLS);
	PyModule_AddIntConstant(poModule, "CUBE_REFINE_SCROLLS",	CUBE_REFINE_SCROLLS);
	PyModule_AddIntConstant(poModule, "CUBE_CATEGORY_MAX",		CUBE_CATEGORY_MAX);
#endif				

#ifdef SASH_ABSORPTION_ENABLE
	// SASH
	PyModule_AddIntConstant(poModule, "SASH_TYPE_SOCKET", SASH_TYPE_SOCKET);
	PyModule_AddIntConstant(poModule, "SASH_REFINE_CHANCE_ITEM_VALUE", SASH_REFINE_CHANCE_ITEM_VALUE);
	PyModule_AddIntConstant(poModule, "SASH_ABSORPTION_SOCKET", SASH_ABSORPTION_SOCKET);
	PyModule_AddIntConstant(poModule, "SASH_ABSORPTION_RELEASE_ITEM", SASH_ABSORPTION_RELEASE_ITEM);
	PyModule_AddIntConstant(poModule, "SASH_ABSORPTION_BASE_VALUE", SASH_ABSORPTION_BASE_VALUE);
	PyModule_AddIntConstant(poModule, "SASH_ABSORPTION_VALUE_MAX", SASH_ABSORPTION_VALUE_MAX);
	PyModule_AddIntConstant(poModule, "SASH_ABSORPTION_LEGENDARY_MIN", SASH_ABSORPTION_LEGENDARY_MIN);
	PyModule_AddIntConstant(poModule, "SASH_ABSORPTION_LEGENDARY_MAX", SASH_ABSORPTION_LEGENDARY_MAX);
	PyModule_AddIntConstant(poModule, "SASH_TYPE_LEGENDARY", SASH_TYPE_LEGENDARY);
#endif
}
