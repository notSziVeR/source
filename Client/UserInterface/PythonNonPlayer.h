...
public:
	...
	std::map<DWORD, std::string> GetMonsterByName(std::string MonsterName, bool bBoss = false, bool bMetin = false, const std::unordered_set<DWORD>& s_banned_list = {});
#endif

	// Function for outer
	void				GetMatchableMobList(int iLevel, int iInterval, TMobTableList * pMobTableList);

	short				GetRaceFlag(DWORD dwVnum);
	bool				GetEnchant(DWORD dwVnum, BYTE enchant);

	uint32_t			GetMobRank(uint32_t vnum);
	uint32_t			GetMobType(uint32_t vnum);
