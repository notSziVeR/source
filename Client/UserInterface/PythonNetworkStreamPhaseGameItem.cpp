...
bool CPythonNetworkStream::SendSafeBoxItemMovePacket(BYTE bySourcePos, BYTE byTargetPos, CountType byCount)
{
...
bool CPythonNetworkStream::RecvSafeBoxSetPacket()
{
	...
	kItemData.transmutation_id = kItemSet.transmutation_id;
#endif
...
#ifdef TRANSMUTATION_SYSTEM
	kItemData.transmutation_id = packet_item_set.transmutation_id;
#endif
...
#ifdef TRANSMUTATION_SYSTEM
	rkPlayer.SetItemTransmutate(packet_item_update.Cell, packet_item_update.transmutation_id);
#endif
...
bool CPythonNetworkStream::RecvItemOwnership()
{
	...
	int iDropType = static_cast<int>(EItemDropTypes::TYPE_ENEMY);
	CInstanceBase* pInstance = CPythonCharacterManager::Instance().GetSelectedInstancePtr();
	if (pInstance && strcmp(pInstance->GetNameString(), p.szName) == 0)
	{
		iDropType = static_cast<int>(EItemDropTypes::TYPE_MINE);
	}
	else if (CPythonPlayer::Instance().IsPartyMemberByName(p.szName))
	{
		iDropType = static_cast<int>(EItemDropTypes::TYPE_PARTY);
	}

	CPythonItem::Instance().SetOwnership(p.dwVID, p.szName, iDropType);
	return true;
}
...
bool CPythonNetworkStream::SendShopBuyPacketMultiple(BYTE bPos, DWORD iCount)
{
	if (!__CanActMainInstance())
	{
		return true;
	...
	TPacketCGShop PacketShop;
	PacketShop.header = HEADER_CG_SHOP;
	PacketShop.subheader = SHOP_SUBHEADER_CG_BUY_MULTIPLE;

	TMultipleBuy multiple;
	multiple.bySlot = bPos;
	multiple.byCount = iCount;
	...
	if (!Send(sizeof(TMultipleBuy), &multiple))
	{
		Tracef("SendShopBuyPacket Multiple Error\n");
		return false;
	}
...
bool CPythonNetworkStream::SendShopSellPacketNew(WORD wSlot, CountType byCount, BYTE byType)
{
	if (!__CanActMainInstance())
	{
		return true;
	...
	if (!Send(sizeof(TSellPacket), &second))
...
bool CPythonNetworkStream::SendItemMultiUsePacket(TItemPos pos, BYTE bCount)
{
	...
	TPacketCGItemMultiUse itemMultiUsePacket;
	itemMultiUsePacket.header = HEADER_CG_ITEM_MULTI_USE;
	itemMultiUsePacket.pos = pos;
	itemMultiUsePacket.count = bCount;

	if (!Send(sizeof(TPacketCGItemMultiUse), &itemMultiUsePacket))
	{
...
bool CPythonNetworkStream::SendItemMovePacket(TItemPos pos, TItemPos change_pos, CountType num)
{
	if (!__CanActMainInstance())
	{
		return true;
...
bool CPythonNetworkStream::SendItemDestroyPacket(TItemPos pos)
{
	...
	TPacketCGItemDestroyPacket itemDestroyPacket;
	itemDestroyPacket.header = HEADER_CG_ITEM_DESTROY;
	itemDestroyPacket.pos = pos;

	if (!Send(sizeof(TPacketCGItemDestroyPacket), &itemDestroyPacket))
	{
...
bool CPythonNetworkStream::SendItemsPickUpPacket(DWORD items[20])
{
	if (!__CanActMainInstance())
	{
		return true;
	...
	TPacketCGItemsPickUp	itemPickUpPacket;
	itemPickUpPacket.header = HEADER_CG_ITEMS_PICKUP;
	memcpy(itemPickUpPacket.items, items, sizeof(itemPickUpPacket.items));

	if (!Send(sizeof(TPacketCGItemsPickUp), &itemPickUpPacket))
	{
		Tracen("SendItemsPickUpPacket Error");
		return false;
	}
...
#ifdef TRANSMUTATION_SYSTEM
		kItemData.transmutation_id = packItem.transmutation_id;
#endif

#ifdef ENABLE_REFINE_ELEMENT
		kItemData.dwRefineElement = packItem.dwRefineElement;
#endif
...
#ifdef ENABLE_SWITCHBOT
		if (packItem.Cell.window_type == SWITCHBOT)
		{
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RefreshSwitchbotWindow", Py_BuildValue("()"));
			return true;
		}
