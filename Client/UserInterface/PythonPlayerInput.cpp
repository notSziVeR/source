...
void CPythonPlayer::PickCloseItem()
{
	...
	if (CPythonSystem::Instance().IsVectorPickupMode())
	{
		CInstanceBase* pkInstMain = NEW_GetMainActorPtr();
		if (!pkInstMain)
		{
		...
		CPythonItem& rkItem = CPythonItem::Instance();
		std::vector<DWORD> itemVidList = rkItem.GetCloseItems(kPPosMain, __GetPickableDistance());;
...
void CPythonPlayer::SetTarget(DWORD dwVID, BOOL bForceChange)
{
	...
	if (pkInstTarget)
	{
		...
		if (pkInstTarget->IsShaman() == false)
#endif
...
void CPythonPlayer::__OnClickActor(CInstanceBase& rkInstMain, DWORD dwPickedActorID, bool isAuto)
{
...
#ifdef ENABLE_SHAMAN_SYSTEM
	{
		if (rkInstVictim.IsShaman()) {
			PyCallClassMemberFunc(m_ppyGameWindow, "__ShamanSystem_Open", Py_BuildValue("()"));
			return;
		}
	}
#endif
...
#ifdef SHIP_DUNGEON_ENABLE
	const CPythonNonPlayer::TMobTable* pMobTable = CPythonNonPlayer::Instance().GetTable(pkInstReserved->GetVirtualNumber());
	if (pMobTable && pMobTable->dwVnum == SHIP_DEFEND_DUNGEON_ALLIE)
	{
		if (pkInstMain->NEW_IsClickableDistanceDestInstance(*pkInstReserved))
		{
