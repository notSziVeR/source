...
// Parse an expression std::string
Node *Parser::Parse(const std::string &exstr)
{
	BuildTokens(exstr);
...
Node *Parser::ParseRegion(Parser::size_type start, Parser::size_type end)
{
	...
	if (multiexpr)
	{
		std::unique_ptr<Node> n(new MultiNode(m_expr));
	...
	{
		// Assignment next
		std::unique_ptr<Node> n(new AssignNode(m_expr));
	...
	else if (addsubindex != (size_type) - 1)
	{
		...
		{
			// Addition
			std::unique_ptr<Node> n(new AddNode(m_expr));
		...
		{
			// Subtraction
			std::unique_ptr<Node> n(new SubtractNode(m_expr));
			n->Parse(*this, start, end, addsubindex);
			return n.release();
		}
	}
	else if (muldivindex != (size_type) - 1)
	{
		...
		{
			// Multiplication
			std::unique_ptr<Node> n(new MultiplyNode(m_expr));
		...
		{
			// Division
			std::unique_ptr<Node> n(new DivideNode(m_expr));
			n->Parse(*this, start, end, muldivindex);
			return n.release();
		}
	}
	else if (posnegindex == start)
	{
		...
		{
			std::unique_ptr<Node> n(new NegateNode(m_expr));
	...
	{
		// Exponent
		std::unique_ptr<Node> n(new ExponentNode(m_expr));
	...
	else if (fgopen == start + 1)
	{
		...
		if (fgclose == end)
		{
			...
			std::unique_ptr<FunctionNode> n(flist->Create(ident, m_expr));
	...
	else if (start == end)
	{
		...
		{
			// Variable/constant
			std::unique_ptr<Node> n(new VariableNode(m_expr));
			n->Parse(*this, start, end);
			return n.release();
		}
		else
		{
			// Value
			std::unique_ptr<Node> n(new ValueNode(m_expr));
			n->Parse(*this, start, end);
			return n.release();
		}
	}
...
// Build tokens
void Parser::BuildTokens(const std::string &exstr)
{
	m_tokens.clear();
