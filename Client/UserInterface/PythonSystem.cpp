...
void CPythonSystem::SetDefaultConfig()
{
	...
	m_Config.dwEffectsFlag = 0;
#endif
#ifdef ENABLE_HIDE_OBJECTS
	m_Config.dwObjectsFlag = 0;
#endif
#ifdef ENABLE_NEW_FILTERS
	m_Config.PickUpSolo = false;
	m_Config.IgnoreWeapon = false;
	m_Config.IgnoreArmor = false;
	m_Config.IgnoreHelmet = false;
	m_Config.IgnoreShield = false;
	m_Config.IgnoreBracelets = false;
	m_Config.IgnoreShoes = false;
	m_Config.IgnoreNecklets = false;
	m_Config.IgnoreEarrings = false;
	m_Config.IgnoreOthers = false;
#endif
}
...
#ifdef ENABLE_HIDE_EFFECTS
void CPythonSystem::SetHideEffectsFlag(DWORD iFlag, bool isHide)
{
	if (isHide)
	{
		SET_BIT(m_Config.dwEffectsFlag, iFlag);
	}
	else
	{
		REMOVE_BIT(m_Config.dwEffectsFlag, iFlag);
	}
}

bool CPythonSystem::IsHideEffects(DWORD dwHideFlag)
{
	return IS_SET(m_Config.dwEffectsFlag, dwHideFlag) != 0;
}
#endif

#ifdef ENABLE_HIDE_OBJECTS
void CPythonSystem::SetHideObjectsFlag(DWORD iFlag, bool isHide)
{
	if (isHide)
	{
		SET_BIT(m_Config.dwObjectsFlag, iFlag);
	}
	else
	{
		REMOVE_BIT(m_Config.dwObjectsFlag, iFlag);
	}
}

bool CPythonSystem::IsHideObjects(DWORD dwHideFlag)
{
	return IS_SET(m_Config.dwObjectsFlag, dwHideFlag) != 0;
}
#endif

#ifdef ENABLE_NEW_FILTERS
bool CPythonSystem::IsIgnoring(int iFlag)
{
	switch (iFlag)
	{
	case 0:
		return m_Config.IgnoreWeapon;
	case 1:
		return m_Config.IgnoreArmor;
	case 2:
		return m_Config.IgnoreHelmet;
	case 3:
		return m_Config.IgnoreShield;
	case 4:
		return m_Config.IgnoreBracelets;
	case 5:
		return m_Config.IgnoreShoes;
	case 6:
		return m_Config.IgnoreNecklets;
	case 7:
		return m_Config.IgnoreEarrings;
	case 8:
		return m_Config.IgnoreOthers;
	}

	return false;
}

void CPythonSystem::SetIgnoring(int iFlag, bool value)
{
	switch (iFlag)
	{
	case 0:
		m_Config.IgnoreWeapon = value;
		break;
	case 1:
		m_Config.IgnoreArmor = value;
		break;
	case 2:
		m_Config.IgnoreHelmet = value;
		break;
	case 3:
		m_Config.IgnoreShield = value;
		break;
	case 4:
		m_Config.IgnoreBracelets = value;
		break;
	case 5:
		m_Config.IgnoreShoes = value;
		break;
	case 6:
		m_Config.IgnoreNecklets = value;
		break;
	case 7:
		m_Config.IgnoreEarrings = value;
		break;
	case 8:
		m_Config.IgnoreOthers = value;
		break;
	}
}

bool CPythonSystem::IsVectorPickupMode()
{
	return (m_Config.PickUpSolo == false);
}

void CPythonSystem::SwitchPickupMode()
{
	m_Config.PickUpSolo = !m_Config.PickUpSolo;
}
#endif
...
#endif
#ifdef ENABLE_HIDE_EFFECTS
		else if (!stricmp(command, "HIDE_EFFECTS"))
		{
			m_Config.dwEffectsFlag = atoi(value);
		}
#endif
#ifdef ENABLE_HIDE_OBJECTS
		else if (!stricmp(command, "HIDE_OBJECTS"))
		{
			m_Config.dwObjectsFlag = atoi(value);
		}
#endif
#ifdef ENABLE_NEW_FILTERS
		else if (!stricmp(command, "PickUpSolo"))
		{
			m_Config.PickUpSolo = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, "IgnoreWeapon"))
		{
			m_Config.IgnoreWeapon = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, "IgnoreArmor"))
		{
			m_Config.IgnoreArmor = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, "IgnoreHelmet"))
		{
			m_Config.IgnoreHelmet = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, "IgnoreShield"))
		{
			m_Config.IgnoreShield = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, "IgnoreBracelets"))
		{
			m_Config.IgnoreBracelets = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, "IgnoreShoes"))
		{
			m_Config.IgnoreShoes = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, "IgnoreNecklets"))
		{
			m_Config.IgnoreNecklets = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, "IgnoreEarrings"))
		{
			m_Config.IgnoreEarrings = atoi(value) == 1 ? true : false;
		}
		else if (!stricmp(command, "IgnoreOthers"))
		{
			m_Config.IgnoreOthers = atoi(value) == 1 ? true : false;
		}
#endif
	}
...
#endif
#ifdef ENABLE_HIDE_EFFECTS
	fprintf(fp, "HIDE_EFFECTS			%d\n", m_Config.dwEffectsFlag);
#endif
#ifdef ENABLE_HIDE_OBJECTS
	fprintf(fp, "HIDE_OBJECTS			%d\n", m_Config.dwObjectsFlag);
#endif
#ifdef ENABLE_NEW_FILTERS
	if (m_Config.PickUpSolo == 1)
	{
		fprintf(fp, "PickUpSolo			%d\n", m_Config.PickUpSolo);
	}

	if (m_Config.IgnoreWeapon == 1)
	{
		fprintf(fp, "IgnoreWeapon		%d\n", m_Config.IgnoreWeapon);
	}
	if (m_Config.IgnoreArmor == 1)
	{
		fprintf(fp, "IgnoreArmor		%d\n", m_Config.IgnoreArmor);
	}
	if (m_Config.IgnoreHelmet == 1)
	{
		fprintf(fp, "IgnoreHelmet		%d\n", m_Config.IgnoreHelmet);
	}
	if (m_Config.IgnoreShield == 1)
	{
		fprintf(fp, "IgnoreShield		%d\n", m_Config.IgnoreShield);
	}
	if (m_Config.IgnoreBracelets == 1)
	{
		fprintf(fp, "IgnoreBracelets	%d\n", m_Config.IgnoreBracelets);
	}
	if (m_Config.IgnoreShoes == 1)
	{
		fprintf(fp, "IgnoreShoes		%d\n", m_Config.IgnoreShoes);
	}
	if (m_Config.IgnoreNecklets == 1)
	{
		fprintf(fp, "IgnoreNecklets		%d\n", m_Config.IgnoreNecklets);
	}
	if (m_Config.IgnoreEarrings == 1)
	{
		fprintf(fp, "IgnoreEarrings		%d\n", m_Config.IgnoreEarrings);
	}
	if (m_Config.IgnoreOthers == 1)
	{
		fprintf(fp, "IgnoreOthers		%d\n", m_Config.IgnoreOthers);
	}
#endif
