...
{
	return Py_BuildValue("s", CLocaleManager::instance().GetName().c_str());
}
...
{
	return Py_BuildValue("i", CLocaleManager::instance().GetCodePage());
}
// END_OF_LOCALE
...
PyObject* appGetCursorMode(PyObject* poSelf, PyObject* poArgs)
{
	CPythonApplication::Instance().GetCursorMode();
	return Py_BuildNone();
}
...
PyObject* appGetLocalDay(PyObject* poSelf, PyObject* poArgs)
{
	int dayChange;
	if (!PyTuple_GetInteger(poArgs, 0, &dayChange))
	{
		dayChange = 0;
	}

	time_t now = time(NULL) + dayChange * 24 * 60 * 60;
	tm* date = localtime(&now);

	return Py_BuildValue("i", date->tm_mday);
}

PyObject* appGetLocalMonth(PyObject* poSelf, PyObject* poArgs)
{
	int dayChange;
	if (!PyTuple_GetInteger(poArgs, 0, &dayChange))
	{
	...
	return Py_BuildValue("i", date->tm_mon);
}
...
PyObject* appGetLanguageByID(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bLanguageID;
	if (!PyTuple_GetByte(poArgs, 0, &bLanguageID))
		return Py_BadArgument();
	...
	bool shortName = true;
	if (!PyTuple_GetBoolean(poArgs, 1, &shortName))
		return Py_BadArgument();

	const auto &bLocale = CLocaleManager::instance().GetLanguageNameByID(bLanguageID, shortName);
	return Py_BuildValue("s", bLocale);
}
#endif
...
PyObject* appRestart(PyObject* poSelf, PyObject* poArgs)
{
	...
	ShellExecute(0, "open", szFileName, 0, 0, SW_SHOWNORMAL);
	CPythonApplication::Instance().Exit();
	return Py_BuildNone();
}
...
void initapp()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetCursorMode",				appGetCursorMode,				METH_VARARGS },
		...
		{ "GetLocalDay",				appGetLocalDay,					METH_VARARGS },
		{ "GetLocalMonth",				appGetLocalMonth,				METH_VARARGS },

#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
		{ "GetLanguage",				appGetLanguage,					METH_VARARGS },
		{ "SetLanguage",				appSetLanguage,					METH_VARARGS },
		{ "GetLanguageByID",			appGetLanguageByID,				METH_VARARGS },
#endif
	...
	PyModule_AddIntConstant(poModule, "DIK_LMBUTTON",	WM_LBUTTONDOWN);      /* Left Mouse Button */
	PyModule_AddIntConstant(poModule, "DIK_RMBUTTON",	WM_RBUTTONDOWN);      /* Right Mouse Button */
	...
	PyModule_AddIntConstant(poModule, "CURSOR_MODE_HARDWARE",	CPythonApplication::CURSOR_MODE_HARDWARE);
	...
	PyModule_AddIntConstant(poModule, "ENABLE_RENEWAL_EXCHANGE", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_RENEWAL_EXCHANGE", false);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_SASH_COSTUME_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_SASH_COSTUME_SYSTEM", false);
#endif

#ifdef SASH_ABSORPTION_ENABLE
	PyModule_AddIntConstant(poModule, "SASH_ABSORPTION_ENABLE", true);
#else
	PyModule_AddIntConstant(poModule, "SASH_ABSORPTION_ENABLE", false);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_MONSTER_CARD", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_MONSTER_CARD", false);
#endif
...
#ifdef USE_OPENID
	PyModule_AddIntConstant(poModule, "USE_OPENID", true);
	if (openid_test)
	{
...
#else
	PyModule_AddIntConstant(poModule, "USE_OPENID", false);
	PyModule_AddIntConstant(poModule, "OPENID_TEST", false);
#endif /* USE_OPENID */

#ifdef ENABLE_HIGHLIGHT_NEW_ITEM
	PyModule_AddIntConstant(poModule, "ENABLE_HIGHLIGHT_NEW_ITEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_HIGHLIGHT_NEW_ITEM", false);
#endif

#ifdef ENABLE_MOUSEWHEEL_EVENT
	PyModule_AddIntConstant(poModule, "ENABLE_MOUSEWHEEL_EVENT", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_MOUSEWHEEL_EVENT", false);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_EMOJI_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_EMOJI_SYSTEM", false);
#endif

#ifdef ENABLE_MULTI_USE_PACKET
	PyModule_AddIntConstant(poModule, "ENABLE_MULTI_USE_PACKET", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_MULTI_USE_PACKET", false);
#endif

#ifdef ENABLE_DESTROY_ITEM_PACKET
	PyModule_AddIntConstant(poModule, "ENABLE_DESTROY_ITEM_PACKET", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_DESTROY_ITEM_PACKET", false);
#endif

#ifdef ENABLE_TREASURE_BOX_LOOT
	PyModule_AddIntConstant(poModule, "ENABLE_TREASURE_BOX_LOOT", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_TREASURE_BOX_LOOT", false);
#endif

#ifdef ENABLE_TARGET_MONSTER_LOOT
	PyModule_AddIntConstant(poModule, "ENABLE_TARGET_MONSTER_LOOT", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_TARGET_MONSTER_LOOT", false);
#endif

#ifdef ENABLE_DELETE_SINGLE_STONE
	PyModule_AddIntConstant(poModule, "ENABLE_DELETE_SINGLE_STONE", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_DELETE_SINGLE_STONE", false);
#endif

#ifdef ENABLE_CPP_PSM
	PyModule_AddIntConstant(poModule, "ENABLE_CPP_PSM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_CPP_PSM", false);
#endif

#ifdef ENABLE_DS_REFINE_ALL
	PyModule_AddIntConstant(poModule, "ENABLE_DS_REFINE_ALL", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_DS_REFINE_ALL", false);
#endif

#ifdef ENABLE_SPECIAL_STORAGE
	PyModule_AddIntConstant(poModule, "ENABLE_SPECIAL_STORAGE", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_SPECIAL_STORAGE", false);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_GAYA_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_GAYA_SYSTEM", true);
#endif

#ifdef ENABLE_RENEWAL_DEAD_PACKET
	PyModule_AddIntConstant(poModule, "ENABLE_RENEWAL_DEAD_PACKET", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_RENEWAL_DEAD_PACKET", false);
#endif
...
#ifdef ENABLE_RENEWAL_CHARACTER_WINDOW
	PyModule_AddIntConstant(poModule, "ENABLE_RENEWAL_CHARACTER_WINDOW", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_RENEWAL_CHARACTER_WINDOW", false);
#endif

#ifdef ENABLE_RENEWAL_SHOP_SELLING
	PyModule_AddIntConstant(poModule, "ENABLE_RENEWAL_SHOP_SELLING", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_RENEWAL_SHOP_SELLING", false);
#endif

#ifdef ENABLE_RENEWAL_WHISPER
	PyModule_AddIntConstant(poModule, "ENABLE_RENEWAL_WHISPER", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_RENEWAL_WHISPER", false);
#endif

#ifdef ENABLE_VOICE_CHAT
	PyModule_AddIntConstant(poModule, "ENABLE_VOICE_CHAT", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_VOICE_CHAT", false);
#endif
...
#ifdef ENABLE_LOGIN_COUNT_DOWN_UI_MODIFY
	PyModule_AddIntConstant(poModule, "ENABLE_LOGIN_COUNT_DOWN_UI_MODIFY", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_LOGIN_COUNT_DOWN_UI_MODIFY", false);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_MOUSE_WHEEL_EVENT", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_MOUSE_WHEEL_EVENT", false);
#endif

#ifdef ENABLE_FAST_REFINE_OPTION
	PyModule_AddIntConstant(poModule, "ENABLE_FAST_REFINE_OPTION", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_FAST_REFINE_OPTION", false);
#endif

#ifdef ENABLE_CONFIG_MODULE
	PyModule_AddIntConstant(poModule, "ENABLE_CONFIG_MODULE", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_CONFIG_MODULE", false);
#endif

#ifdef ENABLE_CHANGE_CHANNEL
	PyModule_AddIntConstant(poModule, "ENABLE_CHANGE_CHANNEL", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_CHANGE_CHANNEL", false);
#endif

#ifdef ENABLE_DOG_MODE
	PyModule_AddIntConstant(poModule, "ENABLE_DOG_MODE", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_DOG_MODE", false);
#endif

#ifdef ENABLE_HIDE_EFFECTS
	PyModule_AddIntConstant(poModule, "ENABLE_HIDE_EFFECTS", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_HIDE_EFFECTS", false);
#endif

#ifdef ENABLE_HIDE_OBJECTS
	PyModule_AddIntConstant(poModule, "ENABLE_HIDE_OBJECTS", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_HIDE_OBJECTS", false);
#endif

#ifdef ENABLE_NEW_FILTERS
	PyModule_AddIntConstant(poModule, "ENABLE_NEW_FILTERS", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_NEW_FILTERS", false);
#endif

#ifdef ENABLE_12ZI_ELEMENT_ADD
	PyModule_AddIntConstant(poModule, "ENABLE_12ZI_ELEMENT_ADD", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_12ZI_ELEMENT_ADD", false);
#endif

#ifdef ENABLE_12ZI_ELEMENT_SLOT
	PyModule_AddIntConstant(poModule, "ENABLE_12ZI_ELEMENT_SLOT", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_12ZI_ELEMENT_SLOT", false);
#endif

#ifdef OFFLINE_MESSAGE_SYSTEM
	PyModule_AddIntConstant(poModule, "OFFLINE_MESSAGE_ENABLE", true);
#else
	PyModule_AddIntConstant(poModule, "OFFLINE_MESSAGE_ENABLE", false);
#endif

#ifdef ENABLE_ADMIN_MANAGER
	PyModule_AddIntConstant(poModule, "ENABLE_ADMIN_MANAGER", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_ADMIN_MANAGER", false);
#endif

#ifdef ZUO_PANEL_ENABLE
	PyModule_AddIntConstant(poModule, "ZUO_PANEL_ENABLE", true);
#else
	PyModule_AddIntConstant(poModule, "ZUO_PANEL_ENABLE", false);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_MARBLE_CREATOR_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_MARBLE_CREATOR_SYSTEM", false);
#endif

#ifdef SHIP_DUNGEON_ENABLE
	PyModule_AddIntConstant(poModule, "SHIP_DEFEND_DUNGEON", true);
#else
	PyModule_AddIntConstant(poModule, "SHIP_DEFEND_DUNGEON", false);
#endif

#ifdef BATTLE_PASS_ENABLE
	PyModule_AddIntConstant(poModule, "BATTLE_PASS_ENABLE", true);
#else
	PyModule_AddIntConstant(poModule, "BATTLE_PASS_ENABLE", false);
#endif

#ifdef TRANSMUTATION_SYSTEM
	PyModule_AddIntConstant(poModule, "ENABLE_TRANSMUTATION_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_TRANSMUTATION_SYSTEM", false);
#endif
	...
	PyModule_AddIntConstant(poModule, "ENABLE_CUBE_RENEWAL_COPY_BONUS", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_CUBE_RENEWAL_COPY_BONUS", false);
#endif

#ifdef ENABLE_FIND_LETTERS_EVENT
	PyModule_AddIntConstant(poModule, "ENABLE_FIND_LETTERS_EVENT", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_FIND_LETTERS_EVENT", false);
#endif
...
#ifdef ENABLE_DUNGEON_MISSION_INFORMATION
	PyModule_AddIntConstant(poModule, "ENABLE_DUNGEON_MISSION_INFORMATION", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_DUNGEON_MISSION_INFORMATION", false);
#endif

#ifdef ENABLE_LUCKY_BOX
	PyModule_AddIntConstant(poModule, "ENABLE_LUCKY_BOX", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_LUCKY_BOX", false);
#endif

#ifdef ENABLE_TEAMLER_STATUS
	PyModule_AddIntConstant(poModule, "ENABLE_TEAMLER_STATUS", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_TEAMLER_STATUS", false);
#endif

#ifdef OX_EVENT_SYSTEM_ENABLE
	PyModule_AddIntConstant(poModule, "OX_EVENT_SYSTEM_ENABLE", true);
#else
	PyModule_AddIntConstant(poModule, "OX_EVENT_SYSTEM_ENABLE", false);
#endif

#ifdef ENABLE_PASSIVE_SKILLS_HELPER
	PyModule_AddIntConstant(poModule, "ENABLE_PASSIVE_SKILLS_HELPER", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_PASSIVE_SKILLS_HELPER", false);
#endif

#ifdef ENABLE_SAVE_POSITIONS_SYSTEM
	PyModule_AddIntConstant(poModule, "ENABLE_SAVE_POSITIONS_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_SAVE_POSITIONS_SYSTEM", false);
#endif

#ifdef ENABLE_TELEPORT_SYSTEM
	PyModule_AddIntConstant(poModule, "ENABLE_TELEPORT_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_TELEPORT_SYSTEM", false);
#endif

#ifdef ENABLE_TOGGLE_SYSTEM
	PyModule_AddIntConstant(poModule, "ENABLE_TOGGLE_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_TOGGLE_SYSTEM", false);
#endif
...
#ifdef ENABLE_TOGGLE_SYSTEM
	PyModule_AddIntConstant(poModule, "ENABLE_RENDER_TARGET_EXTENSION", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_RENDER_TARGET_EXTENSION", false);
#endif

#ifdef ENABLE_OFFLINE_SHOP
	PyModule_AddIntConstant(poModule, "ENABLE_OFFLINE_SHOP", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_OFFLINE_SHOP", false);
#endif

#ifdef ENABLE_NEW_COSTUME_BONUS
	PyModule_AddIntConstant(poModule, "ENABLE_NEW_COSTUME_BONUS", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_NEW_COSTUME_BONUS", false);
#endif

#ifdef ENABLE_BELT_ATTR
	PyModule_AddIntConstant(poModule, "ENABLE_BELT_ATTR", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_BELT_ATTR", false);
#endif

#ifdef ENABLE_PING_TIME
	PyModule_AddIntConstant(poModule, "ENABLE_PING_TIME", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_PING_TIME", false);
#endif

#ifdef __INGAME_ITEMSHOP_ENABLE__
	PyModule_AddIntConstant(poModule, "INGAME_ITEMSHOP_ENABLE", true);
#else
	PyModule_AddIntConstant(poModule, "INGAME_ITEMSHOP_ENABLE", false);
#endif

#ifdef __TOMBOLA_EVENT_ENABLE__
	PyModule_AddIntConstant(poModule, "TOMBOLA_EVENT_ENABLE", true);
#else
	PyModule_AddIntConstant(poModule, "TOMBOLA_EVENT_ENABLE", false);
#endif

#ifdef __DUNGEON_INFO_ENABLE__
	PyModule_AddIntConstant(poModule, "DUNGEON_INFO_ENABLE", true);
#else
	PyModule_AddIntConstant(poModule, "DUNGEON_INFO_ENABLE", false);
#endif

#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
	PyModule_AddIntConstant(poModule, "ENABLE_MULTI_LANGUAGE_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_MULTI_LANGUAGE_SYSTEM", false);
#endif

#ifdef __MULTI_LANGUAGE_SYSTEM_FLAG_VISIBILITY__
	PyModule_AddIntConstant(poModule, "MULTI_LANGUAGE_SYSTEM_FLAG_VISIBILITY", true);
#else
	PyModule_AddIntConstant(poModule, "MULTI_LANGUAGE_SYSTEM_FLAG_VISIBILITY", false);
#endif
...
#ifdef ENABLE_ORE_REFACTOR
	PyModule_AddIntConstant(poModule, "ENABLE_ORE_REFACTOR", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_ORE_REFACTOR", false);
#endif

#ifdef ENABLE_AMULET_SYSTEM
	PyModule_AddIntConstant(poModule, "ENABLE_AMULET_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_AMULET_SYSTEM", false);
#endif

#ifdef ENABLE_SHAMAN_SYSTEM
	PyModule_AddIntConstant(poModule, "ENABLE_SHAMAN_SYSTEM", true);
#else
	PyModule_AddIntConstant(poModule, "ENABLE_SHAMAN_SYSTEM", false);
#endif

#ifdef __POPUP_SYSTEM_ENABLE__
	PyModule_AddIntConstant(poModule, "POPUP_SYSTEM_ENABLE", true);
#else
	PyModule_AddIntConstant(poModule, "POPUP_SYSTEM_ENABLE", false);
#endif
}
