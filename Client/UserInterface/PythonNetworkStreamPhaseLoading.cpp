...
bool CPythonNetworkStream::RecvMainCharacter()
{
	...
	if (strlen(MainChrPacket.szBGMName))
	{
		__SetFieldMusicFileInfo(MainChrPacket.szBGMName, MainChrPacket.fBGMVol);
	}

	long lX = MainChrPacket.lX, lY = MainChrPacket.lY;
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_LOAD], "LoadData", Py_BuildValue("(ii)", lX, lY));
