...
void CPythonNewWeb::Initialize()
{
	...
	m_kWebPreferences.accept_language = Awesomium::WSLit("en"); // CLocaleManager::instance().GetLanguageShortName()
...
#ifdef PRTNWORKS_UNLFPS
	if (CUnlockedFPS::Instance().GetMaxFPS() > 99)
	{
		m_iTimeout++;
		if (m_iTimeout <= MAX(CUnlockedFPS::Instance().GetUpdateFPS() / 100.0f, 1))
		{
			return;
		}
		m_iTimeout = 0;
