		...
		case Token::TypeCloseParenthesis:
		{
			...
			if (plevel == 0)
			{
				if (pos > last)
				{
					...
					{
						// Create node
						std::unique_ptr<Node> n(parser.ParseRegion(last, pos - 1));
					  ...
					  Parser::size_type v1)
{
	...
	if (last < end + 1)
	{
		std::unique_ptr<Node> n(parser.ParseRegion(last, end));
		m_nodes.push_back(n.get());
		n.release();
	}
}
					   ...
					   Parser::size_type v1)
{
	...
	std::unique_ptr<Node> n(parser.ParseRegion(v1 + 1, end));
					...
					Parser::size_type v1)
{
	...
	// Parse sides
	std::unique_ptr<Node> left(parser.ParseRegion(start, v1 - 1));
	std::unique_ptr<Node> right(parser.ParseRegion(v1 + 1, end));
