...
PyObject* exchangeGetItemTransmutateFromTarget(PyObject* poTarget, PyObject* poArgs)
{
	int pos;
	if (!PyTuple_GetInteger(poArgs, 0, &pos))
	{
	...
	return Py_BuildValue("i", CPythonExchange::Instance().GetItemTransmutateFromTarget(pos));
}

PyObject* exchangeGetItemTransmutateFromSelf(PyObject* poTarget, PyObject* poArgs)
{
	int pos;
	if (!PyTuple_GetInteger(poArgs, 0, &pos))
	{
	...
	return Py_BuildValue("i", CPythonExchange::Instance().GetItemTransmutateFromSelf(pos));
}
#endif
...
#ifdef TRANSMUTATION_SYSTEM
		{"GetItemTransmutateFromTarget",	exchangeGetItemTransmutateFromTarget,	METH_VARARGS},
		{"GetItemTransmutateFromSelf",		exchangeGetItemTransmutateFromSelf,		METH_VARARGS},
#endif
