...
void CPythonNetworkStream::ServerCommand(char * c_szCommand)
{
	...
	else if (!strcmpi(szCmd, "RefineFailedType"))
	{
		...
		const BYTE bAutoReopen = atoi(TokenVector[2].c_str());

		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_RefineFailedTypeMessage", Py_BuildValue("(ii)", bType, bAutoReopen));
	}
#endif
