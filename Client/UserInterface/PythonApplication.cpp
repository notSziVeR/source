...
bool LoadLocaleData(const char* localePath)
{
	...
	std::string settingsDir = "game/settings/{}";

	if (!rkItemMgr.LoadItemList(fmt::format(settingsDir, "ItemList.txt").c_str()))
	{
		TraceError("LoadLocaleData - LoadItemList(%s) Error", fmt::format(settingsDir, "ItemList.txt").c_str());
	}

	if (!rkItemMgr.LoadItemTable(fmt::format("game/proto/item_proto_{}", CLocaleManager::instance().GetLanguageShortName()).c_str()))
	{
		TraceError("LoadLocaleData - LoadItemProto(%s) Error", fmt::format("game/proto/item_proto_{}", CLocaleManager::instance().GetLanguageShortName()).c_str());
		return false;
	}

	if (!rkItemMgr.LoadItemDesc(fmt::format("{}/ItemDesc.txt", CLocaleManager::instance().GetLocalePath().c_str()).c_str()))
	{
		Tracenf("LoadLocaleData - LoadItemDesc(%s) Error", fmt::format("{}/ItemDesc.txt", CLocaleManager::instance().GetLocalePath().c_str()).c_str());
	}

	if (!rkNPCMgr.LoadNonPlayerData(fmt::format("game/proto/mob_proto_{}", CLocaleManager::instance().GetLanguageShortName()).c_str()))
	{
		TraceError("LoadLocaleData - LoadMobProto(%s) Error", fmt::format("game/proto/mob_proto_{}", CLocaleManager::instance().GetLanguageShortName()).c_str());
		return false;
	}

#ifdef ENABLE_SASH_COSTUME_SYSTEM
	if (!rkItemMgr.LoadItemScale(fmt::format(settingsDir, "ItemScale.txt").c_str()))
	{
		TraceError("LoadLocaleData - LoadItemScale(%s) Error", fmt::format(settingsDir, "ItemScale.txt").c_str());
		return false;
	}
#endif

	if (!rkSkillMgr.RegisterSkillDesc(fmt::format("{}/SkillDesc.txt", CLocaleManager::instance().GetLocalePath().c_str()).c_str()))
	{
		TraceError("LoadLocaleData - RegisterSkillDesc(%s) Error", fmt::format("{}/SkillDesc.txt", CLocaleManager::instance().GetLocalePath().c_str()).c_str());
		return false;
	}

	if (!rkSkillMgr.RegisterSkillTable(fmt::format(settingsDir, "SkillTable.txt").c_str()))
	{
		TraceError("LoadLocaleData - RegisterSkillTable(%s) Error", fmt::format(settingsDir, "SkillTable.txt").c_str());
		return false;
	}

	if (!rkNetStream.LoadInsultList(fmt::format("{}/insult.txt", CLocaleManager::instance().GetLocalePath().c_str()).c_str()))
	{
		Tracenf("CPythonApplication - CPythonNetworkStream::LoadInsultList(%s)", fmt::format("{}/insult.txt", CLocaleManager::instance().GetLocalePath().c_str()).c_str());
	}
