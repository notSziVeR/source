...
void CPythonApplication::OnMouseMove(int x, int y)
{
	...
	POINT Point;
	if ((pkCmrCur && CPythonBackground::Instance().IsMapReady() && pkCmrCur->Drag(x, y, &Point)) ||
			(rkWndMgr.UpdateLockedCursor(x, y, &Point)))
	{
		x = Point.x;
		y = Point.y;
...
void CPythonApplication::RunIMEUpdateSelection()
{
	UI::CWindowManager& rkWndMgr = UI::CWindowManager::Instance();
	rkWndMgr.RunIMEUpdateSelectionEvent();
}
