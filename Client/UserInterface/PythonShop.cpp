...
void CPythonShop::SetItemData(DWORD dwIndex, const TShopItemData & c_rShopItemData)
{
	...
	TShopItemExData tmpData;
	ZeroMemory(&tmpData, sizeof(tmpData));
	memcpy(&tmpData, &c_rShopItemData, sizeof(TShopItemData));

	SetItemData(tabIdx, dwSlotPos, tmpData);
}
...
BOOL CPythonShop::GetItemData(DWORD dwIndex, const TShopItemData ** c_ppItemData)
{
	...
	const TShopItemExData* tmpData;
	bool ret = GetItemData(tabIdx, dwSlotPos, &tmpData);

	*c_ppItemData = (const TShopItemData*) tmpData;
	return ret;
}

void CPythonShop::SetItemData(BYTE tabIdx, DWORD dwSlotPos, const TShopItemExData & c_rShopItemData)
{
...
BOOL CPythonShop::GetItemData(BYTE tabIdx, DWORD dwSlotPos, const TShopItemExData ** c_ppItemData)
{
	if (tabIdx >= SHOP_TAB_COUNT_MAX || dwSlotPos >= SHOP_HOST_ITEM_MAX_NUM)
	{
		TraceError("Out of Index. tabIdx(%d) must be less than %d. dwSlotPos(%d) must be less than %d", tabIdx, SHOP_TAB_COUNT_MAX, dwSlotPos, SHOP_HOST_ITEM_MAX_NUM);
...
void CPythonShop::ClearPrivateShopStock()
{
	...
	void CPythonShop::AddPrivateShopItemStock(TItemPos ItemPos, BYTE dwDisplayPos, int64_t dwPrice)
#else
...
void CPythonShop::Clear()
{
	...
	{
		memset (m_aShoptabs[i].items, 0, sizeof(TShopItemExData) * SHOP_HOST_ITEM_MAX_NUM);
	}
}
...
PyObject * shopGetItemPriceVnum(PyObject * poSelf, PyObject * poArgs)
{
	...
	BYTE tabIdx = iIndex / SHOP_HOST_ITEM_MAX_NUM;
	DWORD dwSlotPos = iIndex % SHOP_HOST_ITEM_MAX_NUM;

	const TShopItemExData * c_pItemData;
	if (CPythonShop::Instance().GetItemData(tabIdx, dwSlotPos, &c_pItemData))
	{
		return Py_BuildValue("i", c_pItemData->coin_vnum);
	}
...
PyObject* shopGetItemTransmutate(PyObject* poSelf, PyObject* poArgs)
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
	{
	...
	if (CPythonShop::Instance().GetItemData(iIndex, &c_pItemData))
	{
		return Py_BuildValue("i", c_pItemData->transmutation_id);
	}
...
#ifdef TRANSMUTATION_SYSTEM
		{ "GetItemTransmutate",			shopGetItemTransmutate,			METH_VARARGS },
#endif
