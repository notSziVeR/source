...
// For GameMaster
extern std::string s_game_master_tag;
static const float f_game_master_padding = 12.0f;
static bool IsGameMasterTitle(const std::string& s_name)
{
	return (s_name == s_game_master_tag);
}
...
const std::array<D3DXCOLOR, to_underlying(EItemDropTypes::TYPE_PARTY)> a_drop_colours = { D3DXCOLOR(1.00f, 0.0f, 0.0f, 1.0f), D3DXCOLOR(1.0f, 0.93f, 0.17f, 1.0f), D3DXCOLOR(0.50f, 0.75f, 1.0f, 1.0f) };
...
void CPythonTextTail::ArrangeTextTail()
{
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		fxAdd = 4.0f;
		int curX = pTextTail->x - (iNameWidth / 2);
		...
		if (pTitle)
		{
			...
			if (IsGameMasterTitle(pTitle->GetValueStringReference()))
			{
				fxAdd = 0.0f;
				pTitle->SetPosition(pTextTail->x + fxAdd + (iTitleWidth / 2), pTextTail->y - f_game_master_padding, pTextTail->z);
			}
			else
			{
				pTitle->SetPosition(curX, pTextTail->y, pTextTail->z);
			}
			pTitle->Update();

			curX -= iTitleWidth + 4.0f;
		}
		...
		if (pLevel)
		{
			...
			pLevel->SetPosition(curX, pTextTail->y, pTextTail->z);

			pLevel->Update();

			curX -= iLevelWidth + 4.0f;
		}


#ifdef __MULTI_LANGUAGE_SYSTEM_FLAG_VISIBILITY__
		if (CGraphicExpandedImageInstance* pFlag = pTextTail->pLanguageInstance)
		{
			int flagYPos = pTextTail->y - (iNameHeight + pFlag->GetHeight()) / 2 + 1.5;
			pFlag->SetPosition(curX - pFlag->GetWidth(), flagYPos);
		}
#endif
...
#ifdef __MULTI_LANGUAGE_SYSTEM_FLAG_VISIBILITY__
		if (pTextTail->pLanguageInstance) // && CPythonSystem::Instance().IsLocaleFlagEnabled())
		{
...
#ifdef ENABLE_EMOJI_SYSTEM
		pTextTail->pTextInstance->SetValue(chatStr.c_str());
#else
...
void CPythonTextTail::SetItemTextTailOwner(DWORD dwVID, const char* c_szName, int iDropType)
{
	...
	if (strlen(c_szName) > 0)
	{
		...
		pTextTail->pOwnerTextInstance->SetColor(a_drop_colours[iDropType]);
...
void CPythonTextTail::DeleteTextTail(TTextTail * pTextTail)
{
	...
	{
		CGraphicExpandedImageInstance::Delete(pTextTail->pLanguageInstance);
		pTextTail->pLanguageInstance = NULL;
	}
#endif
...
#ifdef __MULTI_LANGUAGE_SYSTEM_FLAG_VISIBILITY__
void CPythonTextTail::AttachLocaleFlag(DWORD dwVID, int localeID)
{
	...
	bool isFlag = localeID >= 0 && localeID < LANGUAGE_MAX_NUM;
	CGraphicExpandedImageInstance*& prFlag = pTextTail->pLanguageInstance;
	if ((prFlag != NULL) != isFlag)
	{
		if (prFlag)
		{
			CGraphicExpandedImageInstance::Delete(prFlag);
			prFlag = NULL;
		}
		else
		{
	...
	if (isFlag)
	{
		...
		snprintf(szFileName, sizeof(szFileName), "assets/ui/language_flags/%s.tga", CLocaleManager::instance().GetLanguageNameByID(localeID));

		CGraphicImage* pImg = (CGraphicImage*)CResourceManager::Instance().GetResourcePointer(szFileName);
		if (!pImg)
		{
			TraceError("There is no image for language flag: %s", CLocaleManager::instance().GetLanguageNameByID(localeID));
		}
		else
			prFlag->SetImagePointer(pImg);
	}
}

void CPythonTextTail::DetachLocaleFlag(DWORD dwVID)
{
	AttachLocaleFlag(dwVID, -1);
}
#endif
...
// For GameMaster
void CPythonTextTail::RegisterCharacterTextTail_GM(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR& c_rColor, float fAddHeight)
{
	CInstanceBase* pCharacterInstance = CPythonCharacterManager::Instance().GetInstancePtr(dwVirtualID);
