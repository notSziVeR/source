...
public:
	...
	void	ClearGameWindow(PyObject* ppyObject);
...
#ifdef EXTANDED_GOLD_AMOUNT
	void	SetStatus(DWORD dwType, int64_t lValue);
	int64_t	GetStatus(DWORD dwType);
#else
...
#endif
	CountType	GetItemCount(TItemPos Cell);
	CountType	GetItemCountByVnum(DWORD dwVnum);
	...
	void SendClickItemsPacket(std::vector<DWORD> ids);
#endif


#ifdef ENABLE_LUCKY_BOX
	void 	ClearLuckyBoxVector() { m_LuckyBoxItems.clear(); }

	void 	SetLuckyBoxItem(const TLuckyBoxItem& rItemInstance);
	bool 	GetLuckyBoxItem(BYTE bItemPos, const TLuckyBoxItem** ppItemInfo);

	void 	SetLuckyBoxVnum(DWORD dwBoxVnum) { m_pLuckyBoxVnum = dwBoxVnum; }
	DWORD 	GetLuckyBoxVnum() { return m_pLuckyBoxVnum; }

	void 	SetLuckyBoxPrice(DWORD dwPrice) { m_pLuckyBoxPrice = dwPrice; }
	DWORD 	GetLuckyBoxPrice() { return m_pLuckyBoxPrice; }
#endif
	...
	DWORD	GetItemTransmutate(TItemPos Cell);
	void	SetItemTransmutate(TItemPos Cell, DWORD trans_id);
#endif
	...
	bool	IsSpecialStorageSlot(TItemPos Cell);
#endif
...
#ifdef ENABLE_DS_SET
public:
	bool	_IsDSPageFull(BYTE bPageIndex);
#endif
...
protected:
	...
	// LuckyBox
#ifdef ENABLE_LUCKY_BOX
	std::vector<TLuckyBoxItem>		m_LuckyBoxItems;
	DWORD 							m_pLuckyBoxVnum;
	DWORD 							m_pLuckyBoxPrice;
#endif
...
public:
	void 	SetCubeItem(const TCubeData & c_rkItemCubeInst, DWORD dwNpcVnum);
	int 	GetCubeListSize(DWORD dwNpcVnum);
	const TCubeData * GetCubeItem(DWORD dwNpcVnum, int iIndex) const;
	int		CountSpecifyItem(DWORD dwVnum);
protected:
	typedef std::vector<TCubeData>	TCubeVector;
	typedef std::map<DWORD, TCubeVector> TCubeMap;

	TCubeMap m_vecCubeItems;
#endif
