...
std::string s_game_master_tag = "[Game Master]";
static D3DXCOLOR d3x_game_master_colour = 0xffFF8000;
...
void CInstanceBase::ProcessDamage()
{
	...
	else if ((flag & DAMAGE_FIRE) && bTarget)
	{
		strDamageType = "fire_";
...
#ifdef ENABLE_HIDE_EFFECTS
	m_kStoneSmoke.m_dwSmokeID = eSmoke;
	if (CPythonSystem::Instance().IsHideEffects(CPythonSystem::HIDE_EFFECTS_GENERAL))
	{
		__DetachEffect(m_kStoneSmoke.m_dwEftID);
	}
...
UINT CInstanceBase::GetNameColorIndex()
{
	...
	else if (IsNPC())
	{
		...
		if (this->GetRace() >= 30000 && this->GetRace() <= 30008)
		{
...
void CInstanceBase::AttachTextTail()
{
	...
	if (IsGameMaster())
	{
		CPythonTextTail::Instance().AttachTitle(GetVirtualID(), s_game_master_tag.c_str(), d3x_game_master_colour);
		CPythonTextTail::Instance().RegisterCharacterTextTail_GM(m_dwGuildID, dwVID, s_kD3DXClrTextTail, fTextTailHeight);
		return;
	}
	...
	if (GetLocale() >= 0 && GetLocale() < LANGUAGE_MAX_NUM)
		CPythonTextTail::Instance().AttachLocaleFlag(dwVID, GetLocale());
	else
		CPythonTextTail::Instance().DetachLocaleFlag(dwVID);
#endif

	// CHARACTER_LEVEL
	if (m_dwLevel && (IsPC() || IsEnemy() || IsStone()))
	{
...
void CInstanceBase::UpdateTextTailLevel(DWORD level)
{
	...
	CPythonTextTail::Instance().AttachLevel(GetVirtualID(), szText, *color);
}
...
#ifdef ENABLE_HIDE_EFFECTS
		if (CPythonSystem::Instance().IsHideEffects(CPythonSystem::HIDE_EFFECTS_SKILLS))
		{
...
#ifdef ENABLE_HIDE_EFFECTS
void CInstanceBase::AttachDetachEffect(DWORD dwEID, bool isHide)
{
	// hard code
	if (dwEID >= EFFECT_NUM)
	{
	...
	if (isHide)
	{
		...
		else if (dwEID == AFFECT_GEOMGYEONG)
		{
			if (m_kWarrior.m_dwGeomgyeongEffect)
			{
				m_GraphicThingInstance.SetReachScale(1.0f);
		...
		{
			if (m_adwCRCAffectEffect[dwEID])
			{
				__DetachEffect(m_adwCRCAffectEffect[dwEID]);
			}
		}
	}
	else
	{
		if (dwEID == 0)
		{
			...
			{
				__DetachEffect(m_kStoneSmoke.m_dwEftID);
				m_kStoneSmoke.m_dwEftID = m_GraphicThingInstance.AttachSmokeEffect(m_kStoneSmoke.m_dwSmokeID);
			}
		}
		else if (dwEID == AFFECT_GEOMGYEONG)
		{
		...
		else
		{
			...
			{
				__DetachEffect(m_adwCRCAffectEffect[dwEID]);
				m_adwCRCAffectEffect[dwEID] = __AttachEffect(EFFECT_AFFECT + dwEID);
			}
		}
	}
}

void CInstanceBase::CheckAttachedEffects(DWORD dwEID)
{
	if (CPythonSystem::Instance().IsHideEffects(CPythonSystem::HIDE_EFFECTS_BUFFS))
	{
		BYTE buffId[5] = { 22, 23, 24, 29, 30 };
		for (int k = 0; k < 5; k++)
		{
			if (dwEID == buffId[k])
			{
	...
	if (CPythonSystem::Instance().IsHideEffects(CPythonSystem::HIDE_EFFECTS_SKILLS))
	{
		BYTE skillId[8] = { 16, 19, 20, 21, 25, 43, 44 };
		for (int k = 0; k < 8; k++)
		{
			if (dwEID == skillId[k])
			{
