...
DWORD	CPythonItem::__GetUseSoundType(const CItemData& c_rkItemData)
{
	...
	void CPythonItem::CreateItem(DWORD dwVirtualID, DWORD dwVirtualNumber, float x, float y, float z, WORD count, bool bDrop)
#else
...
#ifdef ENABLE_DROP_ITEM_COUNT
	std::string sItemFullDesc(pItemData->GetName());
	if (count > 1)
	{
		sItemFullDesc += CFF::format(std::string(" [") + std::to_string(count) + std::string("x]"), "#ffe3ad");
	}
#endif
...
void CPythonItem::SetOwnership(DWORD dwVID, const char* c_pszName, int iDropType)
{
	...
	CPythonTextTail& rkTextTail = CPythonTextTail::Instance();
	rkTextTail.SetItemTextTailOwner(dwVID, c_pszName, iDropType);
}
...
std::vector<DWORD> CPythonItem::GetCloseItems(const TPixelPosition& c_rPixelPosition, DWORD dwDistance)
{
