...
public:
...
#ifdef __ENABLE_BOSS_ON_ATLAS__
	bool GetAtlasInfo(float fScreenX, float fScreenY, std::string& rReturnString, std::string& rDurationString, float* pReturnPosX, float* pReturnPosY, DWORD* pdwTextColor, DWORD* pdwGuildID);
#else
	...
	void UpdatePartyMember(DWORD dwPID, DWORD dwX, DWORD dwY);
	...
	// NPC List
#ifdef __ENABLE_BOSS_ON_ATLAS__
	void ClearAtlasMarkInfo(bool clearStone = false);
#else
...
#ifdef __ENABLE_BOSS_ON_ATLAS__
	void AddBossList(BYTE byType, DWORD dwID, float fX, float fY, std::string strText, DWORD dwChrVID);
	void RemoveBossAtlas(DWORD dwID);
#endif
...
protected:
...
#ifdef __ENABLE_BOSS_ON_ATLAS__
	CGraphicImageInstance			m_BossImageMark;
#endif
	...
	TInstanceMarkPositionVector		m_vecOfflineShopPositions;
#endif
#ifdef ENABLE_PARTY_ATLAS
...
public:
	...
	bool GetAdminManagerAtlasInfo(float fScreenX, float fScreenY, std::string& rReturnString, float* pReturnPosX, float* pReturnPosY, DWORD* pdwTextColor, DWORD* pdwStoneDropVnum);
	...
	void ShowAdminManagerFlag(DWORD dwFlag);
	void HideAdminManagerFlag(DWORD dwFlag);
	bool IsAdminManagerFlagShown(DWORD dwFlag) const;

protected:
	bool m_bAdminManagerAtlas;
	bool m_bShowAdminManagerAtlas;
	DWORD m_dwShowAdminManagerFlag;
	CGraphicExpandedImageInstance m_AdminManagerAtlasImageInstance;
	D3DXMATRIX m_matWorldAdminManagerAtlas;
	...
	DWORD m_dwAdminManagerAtlasBaseX;
	DWORD m_dwAdminManagerAtlasBaseY;
