...
{
	for (CPythonCharacterManager::CharacterIterator it = CPythonCharacterManager::Instance().CharacterInstanceBegin(),
			ti = CPythonCharacterManager::Instance().CharacterInstanceEnd();
			it != ti; ++it)
	{
		CInstanceBase* pkInst = *it;
		if (pkInst && !pkInst->IsPC() && !pkInst->IsNPC() && !pkInst->IsDead())
		{
			pkInst->Destroy();
		}
	}
...
PyObject* systemSetHideEffectsFlag(PyObject* poSelf, PyObject* poArgs)
{
	int iFlag;
	if (!PyTuple_GetInteger(poArgs, 0, &iFlag))
	{
	...
	int isHide;
	if (!PyTuple_GetInteger(poArgs, 1, &isHide))
	{
		return Py_BuildException();
	}

	CPythonSystem::Instance().SetHideEffectsFlag(iFlag, isHide ? true : false);

	return Py_BuildNone();
}

PyObject* systemIsHideEffects(PyObject* poSelf, PyObject* poArgs)
{
	int iFlag;
	if (!PyTuple_GetInteger(poArgs, 0, &iFlag))
	{
	...
	return Py_BuildValue("i", CPythonSystem::Instance().IsHideEffects(iFlag));
}
#endif

#ifdef ENABLE_HIDE_OBJECTS
PyObject* systemSetHideObjectsFlag(PyObject* poSelf, PyObject* poArgs)
{
	int iFlag;
	if (!PyTuple_GetInteger(poArgs, 0, &iFlag))
	{
	...
	CPythonSystem::Instance().SetHideObjectsFlag(iFlag, isHide ? true : false);

	return Py_BuildNone();
}

PyObject* systemIsHideObjects(PyObject* poSelf, PyObject* poArgs)
{
	int iFlag;
	if (!PyTuple_GetInteger(poArgs, 0, &iFlag))
	{
	...
	return Py_BuildValue("i", CPythonSystem::Instance().IsHideObjects(iFlag));
}
#endif

#ifdef ENABLE_NEW_FILTERS
PyObject* systemIsIgnoring(PyObject* poSelf, PyObject* poArgs)
{
	int iFlag;
	if (!PyTuple_GetInteger(poArgs, 0, &iFlag))
	{
	...
	return Py_BuildValue("i", CPythonSystem::Instance().IsIgnoring(iFlag));
}

PyObject* systemSetIgnoring(PyObject* poSelf, PyObject* poArgs)
{
	int iFlag;
	if (!PyTuple_GetInteger(poArgs, 0, &iFlag))
	{
	...
	CPythonSystem::Instance().SetIgnoring(iFlag, value == 1 ? true : false);
	return Py_BuildNone();
}

PyObject* systemSwitchPickupMode(PyObject* poSelf, PyObject* poArgs)
{
	CPythonSystem::Instance().SwitchPickupMode();
	return Py_BuildNone();
}

PyObject* systemIsVectorPickupMode(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonSystem::Instance().IsVectorPickupMode());
}
#endif
...
#ifdef ENABLE_HIDE_EFFECTS
		{ "SetHideEffectsFlag",			systemSetHideEffectsFlag,		METH_VARARGS },
		{ "IsHideEffects",				systemIsHideEffects,			METH_VARARGS },
#endif
#ifdef ENABLE_HIDE_OBJECTS
		{ "SetHideObjectsFlag",			systemSetHideObjectsFlag,		METH_VARARGS },
		{ "IsHideObjects",				systemIsHideObjects,			METH_VARARGS },
#endif
#ifdef ENABLE_NEW_FILTERS
		{ "SwitchPickupMode",			systemSwitchPickupMode,			METH_VARARGS },
		{ "IsVectorPickupMode",			systemIsVectorPickupMode,		METH_VARARGS },
		{ "IsIgnoring",					systemIsIgnoring,				METH_VARARGS },
		{ "SetIgnoring",				systemSetIgnoring,				METH_VARARGS },
#endif
...
#ifdef ENABLE_HIDE_EFFECTS
	PyModule_AddIntConstant(poModule, "HIDE_EFFECTS_GENERAL", CPythonSystem::HIDE_EFFECTS_GENERAL);
	PyModule_AddIntConstant(poModule, "HIDE_EFFECTS_SKILLS", CPythonSystem::HIDE_EFFECTS_SKILLS);
	PyModule_AddIntConstant(poModule, "HIDE_EFFECTS_BUFFS", CPythonSystem::HIDE_EFFECTS_BUFFS);
#endif
#ifdef ENABLE_HIDE_OBJECTS
	PyModule_AddIntConstant(poModule, "HIDE_OBJECTS_MOUNTS", CPythonSystem::HIDE_OBJECTS_MOUNTS);
	PyModule_AddIntConstant(poModule, "HIDE_OBJECTS_PETS", CPythonSystem::HIDE_OBJECTS_PETS);
	PyModule_AddIntConstant(poModule, "HIDE_OBJECTS_SHOPS", CPythonSystem::HIDE_OBJECTS_SHOPS);
#endif
}
