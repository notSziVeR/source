...
WORD DSManager::GetBasePosition(const TItemTable* pItemTable) const
{
	...
	GetDragonSoulInfo(pItemTable->dwVnum, type, grade_idx, step_idx, strength_idx);

	BYTE col_type = pItemTable->bSubType;
...
bool DSManager::PullOut(LPCHARACTER ch, TItemPos DestCell, LPITEM& pItem, LPITEM pExtractor)
{
	...
	bool DSManager::DoRefineGrade(LPCHARACTER ch, TItemPos(&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE], bool returnTrueInCaseOfFailedRefine)
#else
...
#ifdef __BATTLE_PASS_ENABLE__
	CBattlePassManager::instance().TriggerEvent(ch, BattlePassNS::SObjective{ BattlePassNS::EObjectives::TYPE_ALL_SPEND_MONEY, 0, static_cast<uint64_t>(fee) });
#endif
...
bool DSManager::DoAllRefineGrade(LPCHARACTER ch, BYTE dsKind, BYTE dsGrade)
{
	...
	if (dsGrade >= DRAGON_SOUL_GRADE_MAX) {
		return false;
	}
...
bool DSManager::GetDSSetGrade(CHARACTER* ch, uint8_t& iSetGrade)
{
	...
	for (uint8_t iSlotIndex = iStartSlotIndex; iSlotIndex < iEndSlotIndex; ++iSlotIndex)
	{
		...
		if (iSetGrade == 0) { iSetGrade = iStep; }

		if (iSetGrade != iStep) { return false; }
	}
