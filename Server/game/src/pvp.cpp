...
{
#ifdef __SHIP_DUNGEON_ENABLE__
	if (pkChr->IsNPC() && pkVictim->IsNPC() && CShipDefendDungeonManager::instance().FindDungeonByVID(pkChr) &&
			CShipDefendDungeonManager::instance().FindDungeonByVID(pkVictim) == CShipDefendDungeonManager::instance().FindDungeonByVID(pkChr))
	{
		return true;
	}
#endif
