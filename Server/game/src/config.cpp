...
CountType	g_bItemCountLimit = 2000;
...
int g_server_id = 0;
string g_strWebMallURL = "itemshop.xantios.pl";

#ifdef __ADMIN_MANAGER__
	string g_AccountDatabase = "account";
#endif
...
char	openid_uri[256];
	...
	uint64_t max_gold = 2000000000000LL;
#endif
...
int MAX_ATTACK_RANGE = 500;

uint32_t	g_version;

#ifdef __ENABLE_OFFLINE_SHOP__
	extern std::vector<uint32_t> g_offlineShopAllowedMaps;
	extern uint32_t g_offlineShopMaxPerPlayer;
	extern uint32_t g_offlineShopMaxDistance;
#endif

int STONE_PERCENT_VALUE = 40;
int ACCESSORY_PERCENT_VALUE = 40;
int BOOK_DELAY_MIN = 3600;
int BOOK_DELAY_MAX = 7200;
...
static bool __LoadConnectConfigFile(const char* configName)
{
	...
	*log_db = '\0';
	...
	AccountDB::instance().SetLocale(MYSQL_CHARSET);

	AccountDB::instance().ConnectAsync(db_host[1], mysql_db_port[1], db_user[1], db_pwd[1], db_db[1], MYSQL_CHARSET);
...
static bool __LoadDefaultConfigFile(const char* configName)
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("version")
		{
			str_to_number(g_version, value_string);
			continue;
		}
		...
		TOKEN("offline_shop_map_allow")
		{
			...
			boost::split(tokens, value_string, boost::is_any_of(" "));

			for (const auto& token : tokens)
			{
				g_offlineShopAllowedMaps.push_back(atoi(token.c_str()));
			}

			continue;
		}

		TOKEN("offline_shop_max_per_player")
		{
			str_to_number(g_offlineShopMaxPerPlayer, value_string);
			continue;
		}

		TOKEN("offline_shop_max_distance")
		{
			str_to_number(g_offlineShopMaxDistance, value_string);
			continue;
		}
#endif

		TOKEN("STONE_PERCENT_VALUE")
		{
			str_to_number(STONE_PERCENT_VALUE, value_string);
			continue;
		}

		TOKEN("ACCESSORY_PERCENT_VALUE")
		{
			str_to_number(ACCESSORY_PERCENT_VALUE, value_string);
			continue;
		}

		TOKEN("BOOK_DELAY_MIN")
		{
			str_to_number(BOOK_DELAY_MIN, value_string);
			continue;
		}

		TOKEN("BOOK_DELAY_MAX")
		{
			str_to_number(BOOK_DELAY_MAX, value_string);
			continue;
		}
	}
...
static bool __LoadGeneralConfigFile(const char* configName)
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("GRAND_NEXT_DELAY_MIN")
		{
			DWORD flag = 0;
			str_to_number(flag, value_string);
			g_dwSkillBookNextReadMin = flag;
			fprintf(stdout, "GRAND_NEXT_DELAY_MIN: %u\n", g_dwSkillBookNextReadMin);
			continue;
		}

		TOKEN("GRAND_NEXT_DELAY_MAX")
		{
			DWORD flag = 0;
			str_to_number(flag, value_string);
			g_dwSkillBookNextReadMax = flag;
			fprintf(stdout, "GRAND_NEXT_DELAY_MAX: %u\n", g_dwSkillBookNextReadMax);
			continue;
		}
		...
		TOKEN("max_gold")
		{
			max_gold = boost::lexical_cast<uint64_t>(value_string);

			max_gold = m2::minmax<uint64_t>(0, max_gold, ULLONG_MAX);

			fprintf(stderr, "MAX_YANG: %llu\n", max_gold);
		...
		TOKEN("HIT_LIMITER_ENABLE")
		{
			...
			fprintf(stdout, "HIT_LIMITER_ENABLE: %d\n", CHitLimiter::g_limiter_enable);
			continue;
		}
		...
		TOKEN("HIT_LIMITER_BUFFER_VALUE")
		{
			...
			fprintf(stdout, "HIT_LIMITER_BUFFER_VALUE: %d\n", CHitLimiter::HIT_LIMIT_BUFF);
			continue;
		}
#endif

		TOKEN("max_attack_range")
		{
			str_to_number(MAX_ATTACK_RANGE, value_string);
			continue;
		}
	}
	fclose(fp);
...
void config_init(const string& st_localeServiceName)
{
	...
	{
		char szFileName[256];
		snprintf(szFileName, sizeof(szFileName), "%s/conf/GENERAL_%s", GetBasePath().c_str(), st_configFileName.c_str());
	...
	{
		char szFileName[256];
		snprintf(szFileName, sizeof(szFileName), "%s/conf/GENERAL_%s_CHANNEL_%d", GetBasePath().c_str(), st_configFileName.c_str(), g_bChannel);
		if (__LoadGeneralConfigFile(szFileName))
		{
			fprintf(stderr, "GENERAL CONFIG LOAD OK [%s]\n", szFileName);
		}
	...
	{
		char szFileName[256];
		snprintf(szFileName, sizeof(szFileName), "%s/conf/GENERAL_%s_CHANNEL_%d_HOSTNAME_%s", GetBasePath().c_str(), st_configFileName.c_str(), g_bChannel, g_stHostname.c_str());
		if (__LoadGeneralConfigFile(szFileName))
		{
			fprintf(stderr, "GENERAL CONFIG LOAD OK [%s]\n", szFileName);
		}
	...
	TOKEN("speedhack_log")
	{
		...
		if (0 == tmp)
		{
			g_bDisableMovspeedHacklog = false;
		}
		else
		{
			g_bDisableMovspeedHacklog = true;
		}


	}

	TOKEN("movespeed_threshold")
	{
		double tmp = 0.75;
	...
	{
		char szFileName[256];
		snprintf(szFileName, sizeof(szFileName), "%s/conf/GENERAL_%s", GetBasePath().c_str(), st_cmdFileName.c_str());
	...
	{
		char szFileName[256];
		snprintf(szFileName, sizeof(szFileName), "%s/conf/GENERAL_%s_CHANNEL_%d", GetBasePath().c_str(), st_cmdFileName.c_str(), g_bChannel);
		if (__LoadDefaultCMDFile(szFileName))
		{
			fprintf(stdout, "GENERAL CMD LOAD OK [%s]\n", szFileName);
		}
	...
	{
		char szFileName[256];
		snprintf(szFileName, sizeof(szFileName), "%s/conf/GENERAL_%s_CHANNEL_%d_HOSTNAME_%s", GetBasePath().c_str(), st_cmdFileName.c_str(), g_bChannel, g_stHostname.c_str());
		if (__LoadDefaultCMDFile(szFileName))
		{
			fprintf(stdout, "GENERAL CMD LOAD OK [%s]\n", szFileName);
		}
