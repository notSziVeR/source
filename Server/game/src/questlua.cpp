...
bool LoadLuaFile(lua_State* L, std::string filename)
{
	auto error = luaL_loadfile(L, filename.c_str());
	if (error)
	{
		sys_err("LUA: Failed to load file %s.", filename.c_str());

		if (!lua_isstring(L, lua_gettop(L)))
		{
			sys_err("LUA: No error information.");
		}
		else
		{
			sys_err("LUA: %s", lua_tostring(L, lua_gettop(L)));
		}

		return false;
	}

	auto callError = lua_pcall(L, 0, LUA_MULTRET, 0);
	if (callError)
	{
		sys_err("LUA: Failed to execute file %s.", filename.c_str());
...
#error "lua version not found"
	...
	RegisterZuoEventFunctionTable();
#endif
#ifdef __SHIP_DUNGEON_ENABLE__
	RegisterShipDefendDungeonFunctionTable();
#endif
...
#define ENABLE_TRANSLATE_LUA
	...
	{
		GetBasePath() + "/settings.lua",
		GetQuestPath() + "/questlib.lua",
#ifdef ENABLE_TRANSLATE_LUA
		GetBasePath() + "/translate.lua",
#endif
		GetQuestPath() + "/locale.lua"
	};

	for (const auto& luaFile : luaFiles)
	{
		if (!LoadLuaFile(L, luaFile))
		{
			return false;
		}
	}
