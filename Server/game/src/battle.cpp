...
bool battle_is_attackable(LPCHARACTER ch, LPCHARACTER victim)
{
	...
	if (ch->FindAffect(AFFECT_INVISIBILITY) || victim->FindAffect(AFFECT_INVISIBILITY))
	{
		return false;
	}
...
int CalcMagicDamage(LPCHARACTER pkAttacker, LPCHARACTER pkVictim)
{
	...
	// Magic Dmg Calc Fix
	iDam += MAX(0, pkAttacker->GetPoint(POINT_MAGIC_ATT_GRADE) - pkVictim->GetPoint(POINT_MAGIC_DEF_GRADE));
...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk, bool bSkillType)
{
	if (!pkVictim->IsPC())
	{
	...
	if (pkVictim->IsNPC())
	{
		...
		if ((pkAttacker->GetJob() == JOB_ASSASSIN || pkAttacker->GetJob() == JOB_SHAMAN) && !bSkillType)
			iAtk += (iAtk * 25) / 100;

		if ((pkVictim->IsBoss() || pkVictim->IsStone()) && !bSkillType)
		{
			iAtk += (iAtk * 100) / 100;
		}
	}
	...
	{
		iAtk -= (iAtk * pkVictim->GetPoint(POINT_RESIST_ALL)) / 100;
	...
	if (pkAttacker->IsNPC() && pkVictim->IsPC())
	{
		...
		else if (pkAttacker->IsBoss())
		{
			iAtk -= iAtk * pkVictim->GetPoint(POINT_RESIST_BOSS) / 100;
		}

		if (pkAttacker->IsMonster())
		{
...
#ifdef __SASH_ABSORPTION_ENABLE__
	if (pkAttacker && pkAttacker->GetWear(WEAR_COSTUME_SASH) && SashSystemHelper::HasAbsorption(pkAttacker->GetWear(WEAR_COSTUME_SASH)))
	{
		auto a_pProto = ITEM_MANAGER::instance().GetTable_NEW(SashSystemHelper::GetAbsorptionBase(pkAttacker->GetWear(WEAR_COSTUME_SASH)));
		if (a_pProto && a_pProto->bType == ITEM_WEAPON)
		{
			iDamMin += std::max((int)1, static_cast<int>(static_cast<float>(a_pProto->alValues[3] + a_pProto->alValues[5]) * SashSystemHelper::GetAbsorptionValue(pkAttacker->GetWear(WEAR_COSTUME_SASH)) / 100.0f));
			iDamMax += std::max((int)1, static_cast<int>(static_cast<float>(a_pProto->alValues[4] + a_pProto->alValues[5]) * SashSystemHelper::GetAbsorptionValue(pkAttacker->GetWear(WEAR_COSTUME_SASH)) / 100.0f));
		}
	}
#endif
