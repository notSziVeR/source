...
void CGuild::SendListOneToAll(DWORD pid)
{
	...
	void CGuild::SendListPacket(LPCHARACTER ch, bool bSend)
#else
...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
	...
	if (ch->FindAffect(AFFECT_EXP_CURSE) && amount > 0)
	{
		return false;
	}
#endif
...
void CGuild::Invite( LPCHARACTER pchInviter, LPCHARACTER pchInvitee )
{
	...
	case GERR_ALREADYJOIN:
		pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Guild] This person is already a member of another Guild."));
		return;
	case GERR_GUILDISFULL:
		pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Guild] The maximum guild capacity has been reached."));
		return;
...
void CGuild::InviteAccept( LPCHARACTER pchInvitee )
{
	...
	case GERR_COMMISSIONPENALTY:
		...
		return;
	case GERR_ALREADYJOIN:
		pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Guild] This person is already a member of another Guild."));
		return;
	case GERR_GUILDISFULL:
		pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Guild] The maximum guild capacity has been reached."));
		return;
	case GERR_GUILD_IS_IN_WAR :
...
#ifdef __ENABLE_FIX_CHANGE_NAME__
void CGuild::ChangeMemberName(DWORD pid, std::string new_name, LPCHARACTER me)
{
	TGuildMember* member = nullptr;
	if ((member = GetMember(pid)))
	{
		...
		{
			// Sending P2P Packet
			TPacketGGGuildNameUpdate p;
			p.bHeader = HEADER_GG_GUILD_PLAYER_NAME;
			p.dwGuild = GetID();
			p.pid = pid;
			strlcpy(p.szName, new_name.c_str(), sizeof(p.szName));

			P2P_MANAGER::instance().Send(&p, sizeof(TPacketGGGuildNameUpdate));
		}
	}
}
#endif
