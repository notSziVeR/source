...
static const DWORD s_adwSubSkillVnums[] =
{
	...
	SKILL_PASSIVE_CRAFT_STONES,
#endif
#ifdef __ENABLE_AMULET_SYSTEM__
	SKILL_PASSIVE_CRAFT_AMULETS,
#endif
};
...
#ifdef __ADMIN_MANAGER__
	CAdminManager::instance().Observer_OnPlayerSkillGroupChange(this);
#endif
}
...
#ifdef __ADMIN_MANAGER__
	CAdminManager::Instance().OnPlayerSkillChange(this, dwVnum);
#endif
...
bool CHARACTER::LearnGrandMasterSkill(DWORD dwSkillVnum, bool bGreater)
{
	...
	auto usingNoDelay = false;
	if ((get_global_time() < GetSkillNextReadTime(dwSkillVnum)) && !bGreater)
	{
	...
	auto fIt = m_ConstantSkillPercents.find(SKILL_GRAND_MASTER);
	if (fIt != m_ConstantSkillPercents.end())
	{
		iPercent = bGreater ? ((fIt->second).first * 2) - 5 : (fIt->second).first;
		iPercentAdd = (fIt->second).second;
	}

	if (FindAffect(AFFECT_SKILL_BOOK_BONUS))
	{
		iPercent += iPercentAdd;

		RemoveAffect(AFFECT_SKILL_BOOK_BONUS);
	}

	auto getTimeSeconds = number(g_dwSkillBookNextReadMin, g_dwSkillBookNextReadMax);
	if (usingNoDelay)
		getTimeSeconds *= 2;

	auto nextTime = get_global_time() + getTimeSeconds;
...
#ifdef __ENABLE_PARTY_BUFFS__
class FPartyBuffCompute
{
public:
	FPartyBuffCompute(LPCHARACTER _pkCaster, DWORD _dwVnum, BYTE _bPointOn, int _iAmount, DWORD _dwAffectFlag, int _iDur,
					  long _kDurationSPCostPoly, bool _bAdded, std::string _strSkillEffect)
	{
		pkCaster = _pkCaster;
		dwVnum = _dwVnum;
		bPointOn = _bPointOn;
		iAmount = _iAmount;
		dwAffectFlag = _dwAffectFlag;
		iDur = _iDur;
		kDurationSPCostPoly = _kDurationSPCostPoly;
		bAdded = _bAdded;
		strSkillEffect = _strSkillEffect;
	}
	...
	void operator () (LPCHARACTER pkVictim)
	{
		...
		if (DISTANCE_APPROX(pkCaster->GetX() - pkVictim->GetX(), pkCaster->GetY() - pkVictim->GetY()) > 4000)
		{
			return;
		}

		pkVictim->RemoveAffect(dwVnum);
		pkVictim->AddAffect(dwVnum, bPointOn, iAmount, dwAffectFlag, iDur, 0, false);
		// pkVictim->SpecificEffectPacket(strSkillEffect.c_str());
	}

	LPCHARACTER pkCaster;
	DWORD dwVnum;
	BYTE bPointOn;
	int iAmount;
	DWORD dwAffectFlag;
	int iDur;
	long kDurationSPCostPoly;
	bool bAdded;
	std::string strSkillEffect;
};
#endif
...
// #define ENABLE_SKILLBOK_NEED_EXP
...
bool CHARACTER::LearnSkillByBook(DWORD dwSkillVnum, BYTE bProb)
{
...
#ifdef ENABLE_SKILLBOK_NEED_EXP
	DWORD need_exp = 0;
...
#endif // ENABLE_SKILLBOK_NEED_EXP
	...
	else
	{
		...
		if (pPC)
		{
			...
			auto fIt = m_ConstantSkillPercents.find(SKILL_MASTER);
			if (fIt != m_ConstantSkillPercents.end())
			{
				iPercent = (fIt->second).first;
				iPercentAdd = (fIt->second).second;
			}
...
void CHARACTER::ResetSkill()
{
	...
	SkillLevelPacket();
#ifdef __ADMIN_MANAGER__
	CAdminManager::Instance().OnPlayerSkillChange(this);
#endif
}
...
struct FFindNearVictim
{
	...
	bool SetOwnVictim(LPCHARACTER victim)
	{
		if (!m_excepts_set.empty())
		{
			if (m_excepts_set.find(victim) != m_excepts_set.end())
			{
				return false;
			}
		}

		if (m_pkChrCenter == victim)
		{
			return false;
		}

		if (!battle_is_attackable(m_pkChrAttacker, victim))
		{
			return false;
		}

		if (abs(m_pkChrCenter->GetX() - victim->GetX()) > 1000 || abs(m_pkChrCenter->GetY() - victim->GetY()) > 1000)
		{
			return false;
		}

		float fDist = DISTANCE_APPROX(m_pkChrCenter->GetX() - victim->GetX(), m_pkChrCenter->GetY() - victim->GetY());
...
#ifdef __SASH_ABSORPTION_ENABLE__
			if (ch && ch->GetWear(WEAR_COSTUME_SASH) && SashSystemHelper::HasAbsorption(ch->GetWear(WEAR_COSTUME_SASH)))
			{
				auto a_pProto = ITEM_MANAGER::instance().GetTable_NEW(SashSystemHelper::GetAbsorptionBase(ch->GetWear(WEAR_COSTUME_SASH)));
				if (a_pProto && a_pProto->bType == ITEM_WEAPON)
				{
					iWep += std::max((int)1, number(static_cast<int>(static_cast<float>(a_pProto->alValues[3] + a_pProto->alValues[5]) * SashSystemHelper::GetAbsorptionValue(ch->GetWear(WEAR_COSTUME_SASH)) / 100.0f),
						static_cast<int>(static_cast<float>(a_pProto->alValues[4] + a_pProto->alValues[5]) * SashSystemHelper::GetAbsorptionValue(ch->GetWear(WEAR_COSTUME_SASH)) / 100.0f)));

					iMtk += std::max((int)1, number(static_cast<int>(static_cast<float>(a_pProto->alValues[1] + a_pProto->alValues[5]) * SashSystemHelper::GetAbsorptionValue(ch->GetWear(WEAR_COSTUME_SASH)) / 100.0f),
						static_cast<int>(static_cast<float>(a_pProto->alValues[2] + a_pProto->alValues[5]) * SashSystemHelper::GetAbsorptionValue(ch->GetWear(WEAR_COSTUME_SASH)) / 100.0f)));
				}
			}
#endif
...
struct FuncSplashDamage
{
	...
	void operator () (LPENTITY ent)
	{
...
#if defined(__ENABLE_WOLFMAN_CHARACTER__) && defined(__USE_ITEM_CLAW_AS_DAGGER__)
					iDam = iDam * (100 - pkChrVictim->GetPoint(POINT_RESIST_DAGGER)) / 100;
#endif
					break;
#endif
		...
		case SKILL_ATTR_TYPE_MAGIC:
			...
			iDam = CalcAttBonus(m_pkChr, pkChrVictim, iDam, true);
			iDam += MAX(0, m_pkChr->GetPoint(POINT_MAGIC_ATT_GRADE) - pkChrVictim->GetPoint(POINT_MAGIC_DEF_GRADE));

#ifdef __SASH_ABSORPTION_ENABLE__
			if (m_pkChr->GetWear(WEAR_COSTUME_SASH) && SashSystemHelper::HasAbsorption(m_pkChr->GetWear(WEAR_COSTUME_SASH)))
			{
				auto a_pProto = ITEM_MANAGER::instance().GetTable_NEW(SashSystemHelper::GetAbsorptionBase(m_pkChr->GetWear(WEAR_COSTUME_SASH)));
				if (a_pProto && a_pProto->bType == ITEM_WEAPON)
				{
					iDam += std::max((int)1, number(static_cast<int>(static_cast<float>(a_pProto->alValues[1] + a_pProto->alValues[5]) * SashSystemHelper::GetAbsorptionValue(m_pkChr->GetWear(WEAR_COSTUME_SASH)) / 100.0f),
						static_cast<int>(static_cast<float>(a_pProto->alValues[2] + a_pProto->alValues[5]) * SashSystemHelper::GetAbsorptionValue(m_pkChr->GetWear(WEAR_COSTUME_SASH)) / 100.0f)));
				}
			}
#endif	
...
#ifndef __MONSTER_HELPER__
		if (!pkChrVictim->Damage(m_pkChr, iDam, dt) && !pkChrVictim->IsStun())
#else
		if (!pkChrVictim->Damage(m_pkChr, iDam, dt, true) && !pkChrVictim->IsStun())
#endif
		{
...
int CHARACTER::ComputeSkillAtPosition(DWORD dwVnum, const PIXEL_POSITION& posTarget, BYTE bSkillLevel)
{
	...
	if (IS_SET(pkSk->dwFlag, SKILL_FLAG_ATTACK | SKILL_FLAG_USE_MELEE_DAMAGE | SKILL_FLAG_USE_MAGIC_DAMAGE))
	{
		...
		// ADD_GRANDMASTER_SKILL
		if (GetUsedSkillMasterType(pkSk->dwVnum) >= SKILL_PERFECT_MASTER && pkSk->bPointOn3 != POINT_NONE)
		{
...
// #endif
			...
			!(pkSk->dwVnum == SKILL_MUYEONG && pkVictim == this) && !(pkSk->IsChargeSkill() && pkVictim == this))
	{
		...
		// ADD_GRANDMASTER_SKILL
		if (pkSk->bPointOn3 != POINT_NONE && !pkSk->IsChargeSkill() && GetUsedSkillMasterType(pkSk->dwVnum) >= SKILL_PERFECT_MASTER)
		{
	...
	else
	{
		...
		if (iDur > 0)
		{
			...
			if (pkSk->bPointOn2 != POINT_NONE)
			{
				...
				// END_OF_ADD_GRANDMASTER_SKILL
#ifdef __ENABLE_PARTY_BUFFS__
				bool bIsShamanBuff = false;
				bool bShamanBuffExecuted = false;
				std::string strSkillEffect;

				CParty* pkParty = GetParty();

				switch (pkSk->dwVnum)
				{
				case SKILL_KWAESOK:
					bIsShamanBuff = true;
					strSkillEffect = "d:\\ymir work\\pc\\shaman\\effect\\10kwaesok_shoot.mse";
					break;
				}

				if (bIsShamanBuff && pkParty)
				{
					if (pkParty->IsMember(pkVictim->GetPlayerID()))
					{
						if (test_server)
						{
							ChatPacket(CHAT_TYPE_INFO, "<ShamanPartyBuff> Execute");
						}

						FPartyBuffCompute f(this, pkSk->dwVnum, pkSk->bPointOn, iAmount, affact_flag, iDur,
											(long)pkSk->kDurationSPCostPoly.Eval(), false, strSkillEffect);
						pkParty->ForEachOnMapMember(f, GetMapIndex());
						bShamanBuffExecuted = true;
					}
				}

				if (!bShamanBuffExecuted)
					pkVictim->AddAffect(pkSk->dwVnum, pkSk->bPointOn, iAmount, affact_flag, iDur,
										(long)pkSk->kDurationSPCostPoly.Eval(), false);
#else
			...
			else
			{
				...
				case SKILL_HOSIN:
					bIsShamanBuff = true;
					strSkillEffect = "d:\\ymir work\\pc\\shaman\\effect\\3hosin_shoot.mse";
					break;
				case SKILL_REFLECT:
					bIsShamanBuff = true;
					strSkillEffect = "d:\\ymir work\\pc\\shaman\\effect\\boho.mse";
					break;
				case SKILL_GICHEON:
					bIsShamanBuff = true;
					strSkillEffect = "d:\\ymir work\\pc\\shaman\\effect\\6gicheon_blow.mse";
					break;
				case SKILL_JEUNGRYEOK:
					bIsShamanBuff = true;
					strSkillEffect = "d:\\ymir work\\pc\\shaman\\effect\\jeungryeok.mse";
					break;
				}
				...
				if (bIsShamanBuff && pkParty)
				{
					if (pkParty->IsMember(pkVictim->GetPlayerID()))
					{
						...
						FPartyBuffCompute f(this, pkSk->dwVnum, pkSk->bPointOn, iAmount, pkSk->dwAffectFlag, iDur,
											(long)pkSk->kDurationSPCostPoly.Eval(), !bAdded, strSkillEffect);
						pkParty->ForEachOnMapMember(f, GetMapIndex());
						bShamanBuffExecuted = true;
					}
				}
				...
				if (!bShamanBuffExecuted)
					pkVictim->AddAffect(pkSk->dwVnum, pkSk->bPointOn, iAmount, pkSk->dwAffectFlag, iDur,
										(long)pkSk->kDurationSPCostPoly.Eval(), !bAdded);
#else
				pkVictim->AddAffect(pkSk->dwVnum,
									pkSk->bPointOn,
									iAmount,
...
bool CHARACTER::UseSkill(DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
...
#ifdef __ENABLE_FIX_USING_SKILLS_WITHOUT_WEAPON__
	LPITEM pWeapon = GetWear(WEAR_WEAPON);

	if ((dwVnum == SKILL_GEOMKYUNG || dwVnum == SKILL_GWIGEOM) && !pWeapon)
	{
		ChatPacket(CHAT_TYPE_NOTICE, "Error! Cannot do this without weapon");
		return false;
	}
#endif
	...
	if (m_SkillMap[dwVnum])
	{
		delete m_SkillMap[dwVnum];
	}

	m_SkillMap[dwVnum] = new t_vec_Targets();
#endif
	...
	{
#ifdef __ADMIN_MANAGER__
		DWORD dwCoolTime = ComputeCooltime(iCooltime * 1000);
...
#endif
		{
			if (test_server)
			{
				ChatPacket(CHAT_TYPE_NOTICE, "cooltime not finished %s %d", pkSk->szName, ComputeCooltime(iCooltime * 1000));
			}
...
#ifdef __ADMIN_MANAGER__
		CAdminManager::instance().OnPlayerSkillCooldown(this, dwVnum, dwCoolTime);
#endif
	}
...
EVENTFUNC(skill_muyoung_event)
{
	...
	if (ch->GetSectree())
	{
		if (ch->GetTarget())
		{
			if (!f.SetOwnVictim(ch->GetTarget()))
			{
...
std::string SecondToDHMS(int time, LPCHARACTER player)
{
	std::stringstream ss;

	if (time < 60)
	{
		ss << time << " " << LC_TEXT("Seconds");
		return ss.str();
	}


	auto const second = int(time % 60);
	auto const minute = int((time / 60) % 60);
	auto const hour = int((time / 60) / 60) % 24;
	auto const day = int(int((time / 60) / 60) / 24);
...
{
	ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("You have to wait %s to learn this."), SecondToDHMS(ms, this).c_str());
}
...
#if defined(__ENABLE_WOLFMAN_CHARACTER__) && !defined(__USE_WOLFMAN_BOOKS__)
...
#ifdef __ENABLE_SKILLS_INFORMATION__
void CHARACTER::BroadcastSkillInformation()
{
	quest::CQuestManager& q = quest::CQuestManager::instance();
	quest::PC* pPC = q.GetPC(GetPlayerID());
	...
	if (pPC)
	{
		ChatPacket(CHAT_TYPE_COMMAND, fmt::format("SkillInformationExorcism {}", FindAffect(AFFECT_SKILL_NO_BOOK_DELAY) ? 1 : 0));

		ChatPacket(CHAT_TYPE_COMMAND, "SkillInformationClear");

		for (int i = 0; i < SKILL_COUNT; i++)
		{
			if (GetSkillGroup())
			{
				auto skillV = SkillList[GetJob()][GetSkillGroup() - 1][i];

				auto skillG = GetSkillMasterType(skillV);
				if (skillG >= SKILL_MASTER && skillG != SKILL_PERFECT_MASTER)
				{
					std::string sReadCount = "traning_master_skill." + std::to_string(skillV) + ".read_count";
					auto iReadCount = pPC->GetFlag(sReadCount);
					auto iNeedBookCount = GetSkillLevel(skillV) - 20;

					auto iPercent = 0;
					auto iPercentAdd = 0;

					auto fIt = m_ConstantSkillPercents.find(skillG);
					if (fIt != m_ConstantSkillPercents.end())
					{
						iPercent = (fIt->second).first;
						iPercentAdd = (fIt->second).second;
					...
					auto isMaster = skillG == SKILL_MASTER;

					ChatPacket(CHAT_TYPE_COMMAND, fmt::format("SkillInformation {} {} {} {} {} {} {} {}", 1, skillV, skillG, isMaster ? iReadCount : 0, isMaster ? iNeedBookCount : 0, iPercent, FindAffect(AFFECT_SKILL_BOOK_BONUS) ? iPercentAdd : 0, GetSkillNextReadTime(skillV) ? std::max<time_t>(GetSkillNextReadTime(skillV) - get_global_time(), 0) : 0));
				}
			}
		}

		const auto PASSIVE_LIST = { SKILL_LEADERSHIP, SKILL_POLYMORPH
#ifdef __ENABLE_AMULET_SYSTEM__
			, SKILL_PASSIVE_CRAFT_AMULETS
#endif
		};

		for (const auto& skill : PASSIVE_LIST)
		{
			auto skillG = GetSkillMasterType(skill);
			std::string sReadCount = "traning_master_skill." + std::to_string(skill) + ".read_count";
			auto iReadCount = pPC->GetFlag(sReadCount);
			auto iNeedBookCount = GetSkillLevel(skill) - 20;

			ChatPacket(CHAT_TYPE_COMMAND, fmt::format("SkillInformation {} {} {} {} {} {} {} {}", 0, skill, skillG, iReadCount, iNeedBookCount, 40, FindAffect(AFFECT_SKILL_BOOK_BONUS) ? 40 : 0, GetSkillNextReadTime(skill) ? std::max<time_t>(GetSkillNextReadTime(skill) - get_global_time(), 0) : 0));
		}
	}
}
#endif
