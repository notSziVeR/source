...
ALUA(pc_learn_grand_master_skill)
{
	...
	if (!lua_isnumber(L, 1))
	{
		sys_err("LearnGrandMasterSkill wrong type of argument!");
		return 0;
	}

	auto bGreater = (item && item->GetVnum() == 50569);
	lua_pushboolean(L, ch->LearnGrandMasterSkill((long)lua_tonumber(L, 1), bGreater));
	return 1;
}
...
ALUA(pc_give_gold)
{
	...
	auto iAmount = static_cast<int64_t>(lua_tonumber(L, 1));
#else
...
#else
	bool bEnoughInventoryForItem = ch->GetEmptyInventory(pTable->bSize, CItem::GetVirtualWindow(pTable)) != -1;
#endif
...
{
#ifdef __EXTANDED_GOLD_AMOUNT__
	auto gold = static_cast<int64_t>(lua_tonumber(L, -1));
#else
...
#else
	for (int iCell = 0; iCell < INVENTORY_AND_EQUIP_SLOT_MAX; iCell++)
#endif
	{
		...
		if (item->GetCell() >= INVENTORY_MAX_NUM && item->GetCell() < SPECIAL_STORAGE_START_CELL)
		{
			continue;
		}
#endif
...
ALUA(pc_change_name)
{
	...
	/* Fucking brainless faggot
	delete messenger list
	MessengerManager::instance().RemoveAllList(ch->GetName());
	*/

#ifdef __ENABLE_FIX_CHANGE_NAME__
	// Updating messenger list
	MessengerManager::instance().UpdateAllList(ch->GetName(), szName);

	// Updating guild if player has one
	if (ch->GetGuild())
	{
		ch->GetGuild()->ChangeMemberName(ch->GetPlayerID(), szName, ch);
	}
#endif
...
#ifdef __OFFLINE_MESSAGE_SYSTEM__
	COfflineMessage::instance().UpdateName(std::string(ch->GetName()), szName);
#endif
...
#else
	int iEmptyCell = ch->GetEmptyInventory(item->GetSize(), item->GetVirtualWindow());
#endif
...
#ifdef __ENABLE_INSTANT_EQUIP__
int pc_instant_equip(lua_State* L)
{
	if (!lua_isnumber(L, 1))
	{
		return 1;
	...
	LPCHARACTER bCh = CQuestManager::instance().GetCurrentCharacterPtr();

	InstantEquipHelper::__instantEquip(bCh, iVnum);
	return 1;
}
#endif

ALUA(pc_get_killed_cords)
{
	LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	long relativeX = ch->GetX() - offsetX - radius;
	long relativeY = ch->GetY() - offsetY - radius;

	double angle = 180.0 + atan2(relativeY, relativeX) * 180.0 / 3.1415926;
	double dist = sqrt(relativeX * relativeX + relativeY * relativeY);

	int regionIndex = angle / (360 / 8);
	if (dist < radius / 2)
	{
		regionIndex += 8;
	}
	lua_pushnumber(L, regionIndex + 1);
	return 1;
}
...
#ifdef __ENABLE_INSTANT_EQUIP__
		{ "_instant_equip",				pc_instant_equip },
#endif

		{ "dt_cords",	pc_get_killed_cords },
