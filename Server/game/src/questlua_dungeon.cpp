...
#ifdef __ENABLE_DUNGEON_MISSION_INFORMATION__
...
ALUA(dungeon_join)
{
	...
	if (!pDungeon)
	{
		...
		sys_log(0, "A request from PID: %d regarding map %d has been sent over cores. Waiting for response.", ch->GetPlayerID(), lMapIndex);
...
ALUA(dungeon_can_return)
{
	...
	if (ch->GetQuestFlag("dungeon_return.index") == 0)
	{
		lua_pushboolean(L, false);
	}
	else
	{
		lua_pushboolean(L, ch->CanReturnToDungeon());
	}
	return 1;
}

ALUA(dungeon_warp_back)
{
	LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	if (!ch || ch->GetDungeon())
	{
	...
	if (ch->GetQuestFlag("dungeon_return.index") == 0 || !ch->CanReturnToDungeon())
	{
		return 0;
	}

	LPDUNGEON dng = CDungeonManager::instance().FindDungeonByPID(ch->GetPlayerID());
#ifndef __CROSS_CHANNEL_DUNGEON_WARP__
	if (!dng)
	{
		return 0;
	...
	{
		long x, y, map_index;
		std::tie(x, y, map_index) = dng->GetDungeonCoords(ch);
	...
	p.bRequest = true;
	p.lX = ch->GetQuestFlag("dungeon_return.x");
	p.lY = ch->GetQuestFlag("dungeon_return.y");
	p.lMapIndex = static_cast<long>(ch->GetQuestFlag("dungeon_return.index")) / 10000;
	p.aPids[0] = ch->GetPlayerID();

	P2P_MANAGER::instance().Send(&p, sizeof(TPacketGGCreateDungeonInstance));
	sys_log(0, "A request from PID: %d regarding map %d has been sent over cores. Waiting for response.", ch->GetPlayerID(), ch->GetMapIndex());
#endif

	return 0;
}

ALUA(dungeon_block_return)
{
	LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	if (!ch)
	{
...
#ifdef __ENABLE_DUNGEON_MISSION_INFORMATION__
int dungeon_init_gui(lua_State* L)
{
	CQuestManager& q = CQuestManager::instance();

	// Title, Time, Actual Mission
	if (!lua_isstring(L, 1) || !lua_isstring(L, 2) || !lua_isstring(L, 3))
	{
		sys_err("Invalid Argument");
		return 0;
	...
	std::string time(lua_tostring(L, 2));
	std::string state(lua_tostring(L, 3));
	...
	// Clearing Flags
	ch->SetQuestFlag("dungeon_gui.monster_count", 0);
	ch->SetQuestFlag("dungeon_gui.stone_count", 0);
	ch->SetQuestFlag("dungeon_gui.boss_count", 0);

	// Sending Datas
	ch->ChatPacket(CHAT_TYPE_COMMAND, "Dungeon_AddDatas name %s", name.c_str());
	ch->ChatPacket(CHAT_TYPE_COMMAND, "Dungeon_AddDatas time %s", time.size() > 0 ? time.c_str() : "0");
	ch->ChatPacket(CHAT_TYPE_COMMAND, "Dungeon_AddDatas state %s", state.c_str());
	return 0;
}

int dungeon_update_gui(lua_State* L)
{
	CQuestManager& q = CQuestManager::instance();
	...
	// Sending Datas
	ch->ChatPacket(CHAT_TYPE_COMMAND, "Dungeon_AddDatas %s %s", key.c_str(), value.c_str());
	return 0;
}
#endif

ALUA(dungeon_get_player_pids)
{
	...
	dungeon->ForEachMember([&pids](LPCHARACTER player)
	{
		if (!player)
		{
			return;
		...
		pids.push_back(player->GetPlayerID());
	});
...
ALUA(dungeon_can_warp_to_party)
{
	LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	if (!ch)
	{
	...
	if (lua_isnumber(L, 1))
	{
		map_index = lua_tonumber(L, 1);
	}

	LPCHARACTER sec_ch = CHARACTER_MANAGER::instance().FindByPID((ch->GetParty()->GetLeaderPID() != ch->GetPlayerID()) ? ch->GetParty()->GetLeaderPID() : ch->GetParty()->GetRandomPID(ch->GetPlayerID(), true));
	...
	if (!sec_ch->GetDungeon())
	{
		lua_pushboolean(L, false);
		return 1;
	}

	if (map_index && (sec_ch->GetMapIndex() / 10000) != map_index)
	{
		lua_pushboolean(L, false);
		return 1;
	}
...
ALUA(dungeon_warp_to_party)
{
	...
	ch->WarpSet(sec_ch->GetX(), sec_ch->GetY(), sec_ch->GetMapIndex());
	return 0;
}
...
int dungeon_get_dungeon_pids(lua_State* L)
{
	LPDUNGEON pDungeon = CQuestManager::instance().GetCurrentDungeon();
	lua_newtable(L);
	...
	LPSECTREE_MAP pkMap = SECTREE_MANAGER::instance().GetMap(pDungeon->GetMapIndex());
	int iCount = 0;

	auto lSearch = [&L, &iCount] (LPENTITY ent) {
		if (ent->IsType(ENTITY_CHARACTER))
		{
			LPCHARACTER ch = (LPCHARACTER) ent;
...
#ifdef __DUNGEON_RETURN_ENABLE__
		{ "can_return",			dungeon_can_return		},
		{ "warp_back",			dungeon_warp_back		},
		{ "block_return",		dungeon_block_return	},
#endif

#ifdef __ENABLE_DUNGEON_MISSION_INFORMATION__
		{ "initialize_information",			dungeon_init_gui		},
		{ "update_information",				dungeon_update_gui		},
#endif

		{ "get_player_pids", dungeon_get_player_pids },
		{ "warp_to_party",	dungeon_warp_to_party	},
		{ "can_warp_to_party",	dungeon_can_warp_to_party	},

#ifdef __DUNGEON_INFO_ENABLE__
		{ "finish",	dungeon_finish	},
#endif

		{ "get_dungeon_pids",		dungeon_get_dungeon_pids },
