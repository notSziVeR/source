...
void PC::SetFlag(const string& name, long value, bool bSkipSave)
{
	if ( test_server )
	{
...
long PC::GetFlag(const string & name)
{
	TFlagMap::iterator it = m_FlagMap.find(name);
...
void PC::SaveFlag(const string & name, long value)
{
...
void PC::SendQuestInfoPacket()
{
	...
	// Determine the extra size of the quest packet (from details sent afterwards)
	int extraSize = 0;
	if (m_iSendToClient & QUEST_SEND_ISBEGIN) { extraSize += 1; }
	if (m_iSendToClient & QUEST_SEND_TITLE) { extraSize += 30 + 1; }
#ifdef ENABLE_QUEST_CATEGORY
	if (m_iSendToClient & QUEST_SEND_CATEGORY_ID) { extraSize += +4; }
#endif
	if (m_iSendToClient & QUEST_SEND_CLOCK_NAME) { extraSize += 16 + 1; }
	if (m_iSendToClient & QUEST_SEND_CLOCK_VALUE) { extraSize += 4; }
	if (m_iSendToClient & QUEST_SEND_COUNTER_NAME) { extraSize += 16 + 1; }
	if (m_iSendToClient & QUEST_SEND_COUNTER_VALUE) { extraSize += 4; }
	if (m_iSendToClient & QUEST_SEND_ICON_FILE) { extraSize += 24 + 1; }
