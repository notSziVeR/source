...
static bool MonsterCanBeTracked(const DWORD& dwVnum, const int & iType)
{
	if (iType != REGEN_TYPE_MOB) return false;

	// Seeking for type
	auto pMobManagerData = CMobManager::instance().Get(dwVnum);
	if (!pMobManagerData) return false;

	auto pMobData = pMobManagerData->m_table;

	if (pMobData.bType == CHAR_TYPE_MONSTER && pMobData.bRank == MOB_RANK_BOSS)
		return true;
...
#ifdef __REGEN_RESPAWN_ON_BLOCKED_AREA__
			if (!ch)
			{
				ch = CHARACTER_MANAGER::instance().SpawnMobRandomPosition(regen->vnum, regen->lMapIndex);
			}
...
#ifdef __ENABLE_MOB_TRAKCER__
				if (MonsterCanBeTracked(regen->vnum, regen->type))
				{
					regen->tracker_id = MobTrackerManager::instance().RegisterInstance(lMapIndex, regen->vnum, (regen->sx + regen->ex) / 2 - base_x, (regen->sy + regen->ey) / 2 - base_y, regen->time);
				}
#endif
