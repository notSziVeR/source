...
LPDESC DESC_MANAGER::AcceptP2PDesc(LPFDWATCH fdw, socket_t bind_fd)
{
	...
	// 2019.06.07 - P2P Unwanted Acceptor Exploit
	if (std::find(g_stAdminPageIP.begin(), g_stAdminPageIP.end(), host) == g_stAdminPageIP.end() &&
			host != std::string(g_szInternalIP) && host != std::string(g_szPublicIP))
	{
		sys_err("Spoof attempt. Host: %s", host);
		socket_close(fd);
		return NULL;
	}
...
struct name_with_desc_func
{
	...
	{
#if defined(__ENABLE_FIX_FIND_DESC__)
...
int DESC_MANAGER::GetInvalidLoginRemainTime(const std::string& c_rstIP)
{
	auto it = m_map_InvalidLoginsByIP.find(c_rstIP);
	if (it == m_map_InvalidLoginsByIP.end())
	{
		return 0;
	}

	const TInvalidLoginInfo& info = it->second;
	if (info.invalid_counter < INVALID_LOGIN_MAX_COUNT)
	{
		return 0;
	}

	int remainTime = (int)(info.invalid_time + INVALID_LOGIN_RESET_TIME) - (int)get_global_time();
	if (remainTime <= 0)
	{
		return 0;
	}
...
bool DESC_MANAGER::IsInvalidLogin(const std::string& c_rstIP)
{
	return GetInvalidLoginRemainTime(c_rstIP) > 0;
}

void DESC_MANAGER::AppendInvalidLogin(const std::string& c_rstIP)
{
	TInvalidLoginInfo* info;

	auto it = m_map_InvalidLoginsByIP.find(c_rstIP);
	if (it == m_map_InvalidLoginsByIP.end())
	{
		info = &m_map_InvalidLoginsByIP[c_rstIP];
		info->invalid_time = get_global_time();
		info->invalid_counter = 0;
	}
	else
	{
		info = &it->second;
	...
	if (get_global_time() >= info->invalid_time + INVALID_LOGIN_RESET_TIME)
	{
		info->invalid_time = get_global_time();
		info->invalid_counter = 0;
	}

	info->invalid_counter++;
}
