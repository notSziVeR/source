...
public:
	...
	bool	GetRefineGradeValues(BYTE ds_type, BYTE grade_idx, OUT CountType& need_count, OUT int& fee, OUT std::vector<float>& vec_probs);
	bool	GetRefineStepValues(BYTE ds_type, BYTE step_idx, OUT CountType& need_count, OUT int& fee, OUT std::vector<float>& vec_probs);
	...
	int GetDSSetValue(uint8_t iAttributeIndex, uint16_t iApplyType, uint8_t iType, uint8_t iSetGrade);
	float GetDSSetWeight(uint8_t iDSType, uint8_t iSetGrade);
	uint8_t GetDSBasicApplyCount(uint8_t iDSType);
	int GetDSBasicApplyValue(uint8_t iDSType, int applyType);
	int GetDSAdditionalApplyValue(uint8_t iDSType, int applyType);
#endif
