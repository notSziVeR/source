...
public:
	...
	const std::vector<SMobItemGroupInfo>& GetVec()
	{
		return m_vecItems;
	}
#endif
...
public:
	...
	void			SaveSingleItem(LPITEM item, DWORD alternativeOwner = 0);
	void			GetPlayerItem(LPITEM item, TPlayerItem* result);

	LPITEM                  CreateItem(DWORD vnum, CountType count = 1, DWORD dwID = 0, bool bTryMagic = false, int iRarePct = -1, bool bSkipSave = false, bool bGameMasterCreated = false, bool bSkipCreateUtils = false);
#ifndef DEBUG_ALLOC
	...
	TItemTable*				GetTable_NEW(DWORD r_dwVnum);
	...
	TVecDropInfo* FindDropInfo(DWORD dwVNum);
#endif
...
protected:
	...
	void			DropBoost(LPCHARACTER pkChr, LPCHARACTER pkKiller, DWORD& iPercent);
...
#ifdef __ENABLE_TARGET_MONSTER_LOOT__
	std::map<DWORD, TVecDropInfo*> m_map_pkDropInfoByVNum;
#endif
...
#ifdef __BELT_ATTR__
	const static int MAX_BELT_ATTR_NUM = ATTRIBUTE_COUNT_BELT_ATTR;
#endif

	bool ReadItemVnumMaskTable(const char * c_pszFileName);

	void GetClosestItems(LPCHARACTER pkChr, std::unordered_set<DWORD>& us_items);
private:
#ifdef M2_USE_POOL
