...
public:
	...
	void	EraseAttenderByPID(const DWORD& dwPID);
	bool	CheckAttenderByPID(const DWORD& dwPID);
	void	SaveCoords(LPCHARACTER ch, bool bClear = false);
...
public:
	void RegisterPartyForDungeon(CParty* pParty);
#endif
};
...
#ifdef __DUNGEON_RETURN_ENABLE__
public:
	LPDUNGEON FindDungeonByPID(const DWORD& dwPID, const DWORD & dwMapIndex = 0);
#endif
};
