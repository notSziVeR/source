...
bool CInputProcessor::Process(LPDESC lpDesc, const void * c_pvOrig, int iBytes, int & r_iBytesProceed)
{
	...
	for (m_iBufferLeft = iBytes; m_iBufferLeft > 0;)
	{
		...
		//sys_log(0, "<DEBUG> PACKET_INFO :: USER %s (%x) :: Header, Size, Sequence :: [%d, %d]",
...
void LoginFailure(LPDESC d, const char* c_pszStatus, int iValue, std::string&& sBanReason)
{
	if (!d)
	{
		return;
	...
	strlcpy(failurePacket.szReason, sBanReason.c_str(), sizeof(failurePacket.szReason));
	failurePacket.iValue = iValue;
...
int CInputHandshake::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	{
#ifdef __ENABLE_PORT_SECURITY__
		...
		{
			d->SetPhase(PHASE_CLOSE); //setphase+ret0
			return 0;
		}
