...
void CSwitchbot::SwitchItems()
{
	for (BYTE bSlot = 0; bSlot < SWITCHBOT_SLOT_COUNT; ++bSlot)
	{
		...
		if (CheckItem(pkItem, bSlot))
		{
			...
			if (desc)
			{
#ifndef __POPUP_SYSTEM_ENABLE__
				char buf[255];
...
#else
				CPopupManager::instance().ProcessPopup(pkOwner, NSPopupMessage::ECPopupTypes::SWITCHBOT_END, pkItem);
#endif
			}
		...
		else
		{
			...
			if (SWITCHBOT_PRICE_TYPE == 1)
			{
				...
				for (const auto& itemVnum : c_arSwitchingItems)
				{
					...
					{
						if ((pkItem->GetType() == ITEM_WEAPON) || (pkItem->GetType() == ITEM_ARMOR && pkItem->GetSubType() != ARMOR_PENDANT))
						{
							bool bCanUse = true;
			...
			if (SWITCHBOT_PRICE_TYPE == 1)
			{
				...
				for (const auto& itemVnum : c_arSwitchingItems)
				{
					...
					{
#ifdef __BATTLE_PASS_ENABLE__
						CBattlePassManager::instance().TriggerEvent(pkOwner, BattlePassNS::SObjective{ BattlePassNS::EObjectives::TYPE_USE, pkItem->GetVnum(), 1 });
#endif
