...
bool DragonSoulTable::GetRefineGradeValues(BYTE ds_type, BYTE grade_idx, OUT CountType& need_count, OUT int& fee, OUT std::vector<float>& vec_probs)
{
...
bool DragonSoulTable::GetRefineStepValues(BYTE ds_type, BYTE step_idx, OUT CountType& need_count, OUT int& fee, OUT std::vector<float>& vec_probs)
{
	if (step_idx >= DRAGON_SOUL_STEP_MAX)
	{
...
int DragonSoulTable::GetDSSetValue(uint8_t iAttributeIndex, uint16_t iApplyType, uint8_t iType, uint8_t iSetGrade)
{
...
int DragonSoulTable::GetDSBasicApplyValue(uint8_t iDSType, int applyType)
{
	int applyValue = 0;
...
int DragonSoulTable::GetDSAdditionalApplyValue(uint8_t iDSType, int applyType)
{
	int applyValue = 0;
