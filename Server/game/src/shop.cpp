...
void CShop::SetShopItems(TShopItemTable* pTable, BYTE bItemCount)
{
	...
	int64_t CShop::Buy(LPCHARACTER ch, BYTE pos, CountType wAmount)
#else
...
#endif
{
...
#else
			iEmptyPos = ch->GetEmptyInventory(item->GetSize(), item->GetVirtualWindow());
#endif
		else
		{
			// find stack
...
#endif
			{
				if (LPITEM curItem = ch->GetInventoryItem(i))
				{
					...
					if (curItem && curItem->CanStackWith(item) &&
							item->GetCount() + curItem->GetCount() <= g_bItemCountLimit)
					{
						iEmptyPos = i;
						break;
					}
			...
			// find empty pos if item cannot be stacked with anything in inventory / storage
			if (iEmptyPos < 0)
#ifndef __SPECIAL_STORAGE_ENABLE__
				iEmptyPos = ch->GetEmptyInventory(item->GetSize());
#else
...
#ifdef __EXTANDED_GOLD_AMOUNT__
			dwTax = dwPrice * static_cast<int64_t>(iVal) / 100;
#else
...
#ifdef __EXTANDED_GOLD_AMOUNT__
			sys_log(0, "Shop trade: %lu to %lu, price %lld", m_pkPC->GetPlayerID(), ch->GetPlayerID(), dwPrice);
			DWORD tradeID = LogManager::instance().ExchangeLog(EXCHANGE_TYPE_SHOP, m_pkPC->GetPlayerID(), ch->GetPlayerID(), m_pkPC->GetX(), m_pkPC->GetY(), dwPrice);
			LogManager::instance().ExchangeItemLog(tradeID, item, "A");
#else
	...
	else
	{
		...
		{
			if (LPITEM targetItem = ch->GetItem(TItemPos(emptyWindow, iEmptyPos)))
			{
				char szBuyStackHint[256];
				snprintf(szBuyStackHint, sizeof(szBuyStackHint), "%s [new ID -> %u]", item->GetBaseName(), targetItem->GetID());
				LogManager::instance().ItemLog(ch, item, "BUY_STACK", szBuyStackHint);
		...
		ITEM_MANAGER::instance().FlushDelayedSave(item);
		LogManager::instance().ItemLog(ch, item, "BUY", item->GetBaseName());
...
bool CShop::AddGuest(LPCHARACTER ch, DWORD owner_vid, bool bOtherEmpire)
{
	...
	for (DWORD i = 0; i < m_itemVector.size() && i < SHOP_HOST_ITEM_MAX_NUM; ++i)
	{
		...
		if (item.pkItem)
		{
			...
			pack2.items[i].transmutate_id = item.pkItem->GetTransmutate();
#endif
...
#ifdef __TRANSMUTATION_SYSTEM__
			pack2.item.transmutate_id = m_itemVector[pos].pkItem->GetTransmutate();
#endif
