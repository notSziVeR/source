...
void CInputAuth::Login(LPDESC d, const char * c_pData)
{
	...
	#ifdef __HARDWARE_BAN__
		char Hardware[HDID_MAX_LEN + 1];
		std::string sDecodedHWID = CheckHardware::instance().DecodeHWID(std::string(std::begin(pinfo->HDId), std::end(pinfo->HDId)));
		if (!sDecodedHWID.size() || sDecodedHWID[0] == '\0')
		{
			LoginFailure(d, "HARDBAN");
			return;
		}

		strlcpy(Hardware, sDecodedHWID.c_str(), sizeof(Hardware));
	#endif
	...
	#ifdef __HARDWARE_BAN__
		// char szHardware[HDID_MAX_LEN * 2 + 1];
		// DBManager::instance().EscapeString(szHardware, sizeof(szHardware), Hardware, strlen(szHardware));
		delete DBManager::instance().DirectQuery("UPDATE account SET hardware_id = '%s' WHERE login='%s'", Hardware, szLogin);
	#endif
...
#if defined(__HARDWARE_BAN__)
										  "UNIX_TIMESTAMP(create_time), hardware_id, ban_reason"
#else
										  "UNIX_TIMESTAMP(create_time), ban_reason"
#endif
