...
void MessengerManager::RequestToAdd(LPCHARACTER ch, LPCHARACTER target)
{
	...
	{
		if (ch->GetActionRestrictions()->AddCounter(CActionRestricts::RESTRICT_MESSENGER) == false) { return; }
	}
#endif
...
void MessengerManager::UpdateOnList(MessengerManager::keyA account, MessengerManager::keyA companion, MessengerManager::keyA new_name)
{
	if (companion.empty())
	{
		return;
	...
	DBManager::instance().Query("UPDATE messenger_list%s SET companion = '%s' WHERE account='%s' AND companion = '%s'",
								get_table_postfix(), new_name.c_str(), account.c_str(), companionEscaped);


	m_Relation[account].erase(companion);
	m_Relation[account].insert(new_name);

#ifdef __ENABLE_BLOCK_SYSTEM__
	if (!CheckIgnored(account, companion))
	{
		return;
	}

	m_FuckingCunts[account].erase(companion);
	m_FuckingCunts[account].insert(new_name);
#endif
}

void MessengerManager::UpdateAllList(MessengerManager::keyA account, MessengerManager::keyA name_new)
{
	std::set<keyT>	company(m_Relation[account]);

	DBManager::instance().Query("UPDATE messenger_list%s SET account = '%s' WHERE account='%s'",
								get_table_postfix(), name_new.c_str(), account.c_str());

	for (std::set<keyT>::iterator iter = company.begin();
			iter != company.end();
			iter++)
	{
		this->UpdateOnList(*iter, account, name_new);
	}

	// Copying Data
	m_Relation[name_new] = m_Relation[account];
	m_InverseRelation[name_new] = m_InverseRelation[account];
#ifdef __ENABLE_BLOCK_SYSTEM__
	m_FuckingCunts[name_new] = m_FuckingCunts[account];
#endif
