...
bool CHARACTER::SetQuickslot(BYTE pos, TQuickslot & rSlot)
{
	...
	case QUICKSLOT_TYPE_ITEM:
		if (false == srcCell.IsDefaultInventoryPosition() && false == srcCell.IsBeltInventoryPosition())
		{
 		...
 		//Prevent unwanted items into quick slot
		if (LPITEM pItem = GetItem(srcCell))
		{
			const auto eTypes = {ITEM_USE, ITEM_QUEST, ITEM_TOGGLE};
			if (std::find_if(eTypes.begin(), eTypes.end(), [pItem](const BYTE& eType) { return pItem->GetType() == eType; }) == eTypes.end()) return false;
		}
		break;
