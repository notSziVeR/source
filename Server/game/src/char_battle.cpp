...
uint64_t AdjustExpByLevel(const LPCHARACTER ch, const uint64_t exp)
{
...
{
#ifdef __SHIP_DUNGEON_ENABLE__
	if (pkVictim->GetDungeon() && IsPC() && !pkVictim->IsPC() &&
			CShipDefendDungeonManager::instance().FindDungeonByPID(this) && CShipDefendDungeonManager::instance().FindDungeonByVID(pkVictim) &&
			CShipDefendDungeonManager::instance().FindDungeonByPID(this) == CShipDefendDungeonManager::instance().FindDungeonByVID(pkVictim)
			&& CShipDefendDungeonManager::instance().FindDungeonByPID(this)->FindFieldMonster(pkVictim))
	{
		return;
	}
#endif
...
bool CHARACTER::Attack(LPCHARACTER pkVictim, BYTE bType)
{
	if (test_server)
	{
		sys_log(0, "[TEST_SERVER] Attacker - %d - %d", GetRaceNum(), pkVictim->GetRaceNum());
		sys_log(0, "[TEST_SERVER] Attack : %s race %d, type %d, MobBattleType %d", GetName(), GetRaceNum(), bType, !GetMobBattleType() ? 0 : GetMobAttackRange());
	}
	...
	if (IsPC())
	{
		...
		/* Start of fix nr. 2
		if (gHackCheckEnable && bType == 0 && dwCurrentTime - this->GetLastComboTime() > this->GetValidComboInterval() + 200)
		{
			if (test_server)
			{
				this->ChatPacket(CHAT_TYPE_INFO, "You need to attack. Time: %d Valid: %d", dwCurrentTime - this->GetLastComboTime(), this->GetValidComboInterval() + 200);
			}

			LogManager::instance().HackLog("WAIT_HACK", this);
			return false;
		}
		*/
		...
		/*
		if (DISTANCE_APPROX(pkVictim->GetX() - GetX(), pkVictim->GetY() - GetY()) > MAX_ATTACK_RANGE)
		{
			return false;
		}
		*/
		...
		//GetAbuseController()->SuspectAttack(pkVictim->GetXYZ());
...
#ifdef __SHIP_DUNGEON_ENABLE__
		if (!(IsPC() && !pkVictim->IsPC() && CShipDefendDungeonManager::instance().FindDungeonByPID(this) && CShipDefendDungeonManager::instance().FindDungeonByVID(pkVictim) &&
				CShipDefendDungeonManager::instance().FindDungeonByPID(this) == CShipDefendDungeonManager::instance().FindDungeonByVID(pkVictim)
				&& CShipDefendDungeonManager::instance().FindDungeonByPID(this)->FindFieldMonster(pkVictim)))
		{
			pkVictim->OnMove();
...
void CHARACTER::DeathPenalty(BYTE bTown)
{
	...
	CCubeRenewal::instance().CubeClose(this);
#else
	Cube_close(this);
#endif
...
void CHARACTER::Stun()
{
	...
	m_pkStunEvent = event_create(StunEvent, info, 1/*PASSES_PER_SEC(3)*/);
}
	...
	else
	{
		if (ch->IsMonster() == true)
		{
			if (ch->IsRevive() == false && ch->HasReviverInParty() == true)
			{
				...
				ch->SetRevive(true);
#ifdef __ADMIN_MANAGER__
				CAdminManager::instance().OnMonsterStateChange(ch);
#endif
				return 0;
			}
		}
...
void CHARACTER::RewardGold(LPCHARACTER pkAttacker)
{
	...
	auto iGoldPercent = MobRankStats[GetMobRank()].iGoldPercent;
...
void CHARACTER::Reward(bool bItemDrop)
{
	...
	if (ITEM_MANAGER::instance().CreateDropItem(this, pkAttacker, s_vec_item))
	{
		...
		else if (s_vec_item.size() == 1)
		{
			item = s_vec_item[0];
#ifdef __MONSTER_HELPER__
			if (!MonsterHelper::PerformPickup(pkAttacker, item))
			{
				item->AddToGround(GetMapIndex(), pos);

				if (CBattleArena::instance().IsBattleArenaMap(pkAttacker->GetMapIndex()) == false)
				{
					item->SetOwnership(pkAttacker, 30);
				}
			...
			else
			{
				...
				sys_log(0, "PICKUP_ITEM: %s %d %d from %s", item->GetName(), pos.x, pos.y, GetName());
		...
		else
		{
			...
			if (v.empty())
			{
				...
				while (iItemIdx >= 0)
				{
					...
					else
					{
						...
						sys_log(0, "PICKUP_ITEM: %s %d %d by %s", item->GetName(), pos.x, pos.y, GetName());
						M2_DESTROY_ITEM(item);
					}
#else
					item->AddToGround(GetMapIndex(), pos);
			...
			else
			{
				...
				while (iItemIdx >= 0)
				{
					...
					if (!MonsterHelper::PerformPickup(ch, item))
					{
						item->AddToGround(GetMapIndex(), pos);
...
void CHARACTER::ItemDropPenalty(LPCHARACTER pkKiller)
{
	...
	if (bDropInventory) // Drop Inventory
	{
		...
		{
			if (GetInventoryItem(i) && (GetInventoryItem(i)->GetCell() >= INVENTORY_MAX_NUM && GetInventoryItem(i)->GetCell() < SPECIAL_STORAGE_START_CELL))
			{
				continue;
			}
		...
		if (!vec_bSlots.empty())
		{
			...
			for (i = 0; i < iQty; ++i)
			{
				...
				if (!pkItem || IS_SET(pkItem->GetAntiFlag(), ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_PKDROP))
				{
					continue;
				}
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
...
#ifndef __DISABLE_STOP_RIDING_WHEN_DIE__
	{
	...
	{
		auto MAX_LEVEL_DIFFERENCE = 15;
		//If killer > 15 then return;
		if ((pkKiller->GetLevel() - GetLevel()) < MAX_LEVEL_DIFFERENCE)
		{
			CBattlePassManager::instance().TriggerEvent(pkKiller, BattlePassNS::SObjective{ (IsPC() ? BattlePassNS::EObjectives::TYPE_KILL_PC : BattlePassNS::EObjectives::TYPE_KILL), GetRaceNum(), 1 });
			if (IsMonster())
				CBattlePassManager::instance().TriggerEvent(pkKiller, BattlePassNS::SObjective{ BattlePassNS::EObjectives::TYPE_KILL_MONSTERS, 0, 1 });

			if (IsBoss())
				CBattlePassManager::instance().TriggerEvent(pkKiller, BattlePassNS::SObjective{ BattlePassNS::EObjectives::TYPE_KILL_BOSSESS, 0, 1 });

			if (IsStone())
				CBattlePassManager::instance().TriggerEvent(pkKiller, BattlePassNS::SObjective{ BattlePassNS::EObjectives::TYPE_KILL_STONES, 0, 1 });
		}
	}
#endif
	...
	if (pkKiller && pkKiller->IsPC())
	{
		CMissionManager::instance().TriggerEvent(pkKiller, MissionNS::SObjective{ GetRaceNum(), 1 });
	}
#endif
...
#ifdef __LEGENDARY_MONSTER_HELPER_ENABLE__
		MonsterSpecialHelper::RegisterLegendaryBossKill(pkKiller, this);
#endif
	}
...
#ifdef __ENABLE_BOSS_NOTIFICATION__
	const auto BOSS_VNUMS =
	{
		591, 691, 2191, 792,
		1304, 1901, 2091, 2206, 2306,
		2307, 2492, 3191, 3596, 3790, 6407
	};
	...
	for (const auto& raceID : BOSS_VNUMS)
	{
		...
		{
			char buf[256];
			snprintf(buf, sizeof(buf), LC_TEXT("|cFFd0ffcc|H|h[Channel: %d] %s was killed by %s. Congratulations!"), g_bChannel, this->GetMobTable().szLocaleName, pkKiller->GetName());
			SendNotice(buf);
			break;
		}
	}
...
#ifdef __ENABLE_PASSIVE_SKILLS_HELPER__
			PSkillsManager::RegisterHighscore(pkKiller, SKILL_PASSIVE_STONES);
#endif
		}
		else
		{
...
#ifdef __ENABLE_PASSIVE_SKILLS_HELPER__
			PSkillsManager::RegisterHighscore(pkKiller, SKILL_PASSIVE_BOSSESS);
#endif
		}
	}
...
#ifdef __ENABLE_PASSIVE_SKILLS_HELPER__
	if (pkKiller && pkKiller->IsPC() && IsMonster())
	{
		PSkillsManager::RegisterHighscore(pkKiller, SKILL_PASSIVE_MONSTERS);
	}
#endif

#ifdef __ENABLE_MARBLE_CREATOR_SYSTEM__
	if (pkKiller && pkKiller->IsPC() && GetRaceNum() == pkKiller->GetMarbleActiveMission())
	{
		CMarbleSystemManager::instance().CounterKills(pkKiller, this);
	}
#endif

#ifndef __RENEWAL_DEAD_PACKET__
	TPacketGCDead pack;
	...
	if (isDuel == false)
	{
		...
		if (IsPC())
		{
			...
			m_pkDeadEvent = event_create(dead_event, pEventInfo, PASSES_PER_SEC(600));
		}
		else
		{
	...
	if (!IsPC())
	{
		CZuoEventManager::instance().DeadMonster(this);
	}
#endif
	...
	if (IsPC())
	{
		CAdminManager::instance().OnPlayerStateChange(this);
	}
	else
	{
		CAdminManager::instance().OnMonsterStateChange(this);
	}
#endif
...
#ifdef __MONSTER_HELPER__
	MonsterHelper::RemoveSpecialInstance(GetVID());
#endif
	...
	if (IsBoss())
	{
		...
		if (pkRegen == nullptr) return;

		MobTrackerManager::instance().ApplyDelay(GetMapIndex(), pkRegen->tracker_id);
	}
#endif
}
...
void CHARACTER::SendDamagePacket(LPCHARACTER pAttacker, int Damage, BYTE DamageFlag)
{
	if (IsPC() == true || (pAttacker->IsPC() == true && pAttacker->GetTarget() == this))
	{
		...
		/// send to nearby teamler
		for (auto it : m_map_view)
		{
			...
			if (ch == this || ch == pAttacker)
			{
				continue;
			}

			if (ch->IsTeamSendDamageFlag())
			{
				ch->GetDesc()->Packet(&damageInfo, sizeof(TPacketGCDamageInfo));
			}
		}
#endif
		/*
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type, bool bSkill) // returns true if dead
{
...
#ifdef __SHIP_DUNGEON_ENABLE__
	if (pAttacker && pAttacker->GetRaceNum() == ShipDefendHelper::ENEMY_HYDRA_VNUM && this->IsMonster())
	{
		return false;
	}

	if (CShipDefendDungeonManager::instance().CheckForAllie(pAttacker, this))
	{
		return false;
	}
#endif
	...
	{
		pAttacker->StartHitTracking();
		pAttacker->RegisterPCDamage();
	}
#endif

#ifdef __HIT_LIMITER_ENABLE__
	if (pAttacker && pAttacker->IsPC() && !CHitLimiter::RegisterHit(pAttacker, this))
	{
		return false;
	}
#endif
	...
	else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
	{
		...
		if (IsAffectFlag(AFF_TERROR))
		{
			dam = (int)(dam * (100 - GetSkillLevel(SKILL_TERROR) - 10) / 100);
		}
...
#ifdef __MONSTER_HELPER__
	if (!MonsterHelper::CanAttackSpecialInstance(pAttacker, this, !bSkill ? type : DAMAGE_TYPE_MAGIC, damageFlag))
	{
		return false;
	}
#endif
	...
	if (pAttacker && (IsNPC() || pAttacker->IsNPC()) && (GetDungeon() || pAttacker->GetDungeon()))
	{
		if (pAttacker->GetPoint(POINT_DUNGEON_DAMAGE_BONUS))
		{
			dam = dam * (100 + pAttacker->GetPoint(POINT_DUNGEON_DAMAGE_BONUS)) / 100;
		}
		if (GetPoint(POINT_DUNGEON_RECV_DAMAGE_BONUS))
		{
			dam -= (dam * GetPoint(POINT_DUNGEON_RECV_DAMAGE_BONUS)) / 100;
		}
	}
	...
	if (pAttacker)
	{
		...
		if (pAttacker && pAttacker->IsPolymorphed())
		{
			if (IsPC())
			{
			...
			else
			{
				...
				switch (pAttacker->GetSkillMasterType(SKILL_POLYMORPH))
				{
					case SKILL_NORMAL:
						dwPercent = 5;
						break;
...
#ifdef __SHIP_DUNGEON_ENABLE__
	CShipDefendDungeonManager::instance().RegisterHitRecord(pAttacker, this);
#endif

#ifdef __DUNGEON_INFO_ENABLE__
	if (pAttacker && pAttacker->IsPC() && pAttacker->GetDungeon())
		CDungeonInfoManager::instance().BroadcastHighscoreRecord(pAttacker, pAttacker->GetMapIndex(), EDIHighScoreTypes::GREATEST_DAMAGE, dam, this);
#endif

	OnDamage(pAttacker, type, dam);

	return false;
}

void CHARACTER::OnDamage(LPCHARACTER pAttacker, EDamageType eDamageType, int iRealDamage)
{
	if (pAttacker && iRealDamage > 0 && IsNPC())
	{
		TDamageMap::iterator it = m_map_kDamage.find(pAttacker->GetVID());

		if (it == m_map_kDamage.end())
		{
			m_map_kDamage.insert(TDamageMap::value_type(pAttacker->GetVID(), TBattleInfo(iRealDamage, 0)));
			it = m_map_kDamage.find(pAttacker->GetVID());
		}
		else
		{
			it->second.iTotalDamage += iRealDamage;
		}

		StartRecoveryEvent();

		UpdateAggrPointEx(pAttacker, eDamageType, iRealDamage, it->second);
	}
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
	...
	if (to->IsEquipUniqueItem(EXPERIENCE_RING_30))
	{
		rateFactor += EXPERIENCE_RING_30_PERCENT;
	}

	if (to->IsEquipUniqueItem(EXPERIENCE_RING_60))
	{
		rateFactor += EXPERIENCE_RING_60_PERCENT;
	}
	...
	if (test_server)
	{
		to->ChatPacket(CHAT_TYPE_INFO, "base_exp(%d) * rate(%Lf) = exp(%d)", retExp, rateFactor / 100.0L, (int)(retExp * rateFactor / 100));
	}
	retExp *= (rateFactor / 100.0L);
	...
	if (retExp > to->GetNextExp() / 10)
	{
		retExp = to->GetNextExp() / 10;
	}

	// it recalculate the given exp if the player level is greater than the exp_table size (useless)
	retExp = AdjustExpByLevel(to, retExp);
	if (test_server)
	{
		to->ChatPacket(CHAT_TYPE_INFO, "exp+minGNE+adjust(%d)", retExp);
	}

	// set
	to->PointChange(POINT_EXP, retExp, true);
	...
	// marriage
	{
		...
		if (you)
		{
			...
			uint64_t dwUpdatePoint = (2000.0L / to->GetLevel() / to->GetLevel() / 3) * retExp;
...
LPCHARACTER CHARACTER::DistributeExp()
{
...
#ifdef __ENABLE_FIX_DOUBLE_EXP_LEADER_EXPLOIT__
		if (!pAttacker || !pAttacker->IsPC())
		{
			continue;
		}
...

	{
		...
		for (it = damage_info_table.begin(); it != damage_info_table.end(); ++it)
		{
			...
			if (fPercent > 1.0f)
			{
				sys_err("DistributeExp percent over 1.0 (fPercent %f name %s)", fPercent, di.pAttacker ? di.pAttacker->GetName() : "Unknown");
				fPercent = 1.0f;
			}
...
int CHARACTER::GetArrowAndBow(LPITEM* ppkBow, LPITEM* ppkArrow, CountType iArrowCount/* = 1 */)
{
	LPITEM pkBow;
...
void CHARACTER::UseArrow(LPITEM pkArrow, CountType dwArrowCount)
{
	auto iCount = pkArrow->GetCount();
...
public:
	...
	void operator () (DWORD dwTargetVID)
	{
		...
		if (m_bType > 1)
		{
			...
			if (m_me->IsPC())
			{
				auto pkSkillMap = m_me->m_SkillMap[m_bType];
				if (pkSkillMap)
				{
					if (std::find(pkSkillMap->begin(), pkSkillMap->end(), pkVictim->GetVID()) != pkSkillMap->end())
					{
						if (test_server)
						{
							m_me->ChatPacket(CHAT_TYPE_INFO, "You can't use this skill on this target more times.");
						}

						LogManager::instance().HackLog("SKILL_HACK", m_me);
						return;
					}
				}
				pkSkillMap->push_back(pkVictim->GetVID());
			}
		}
#else
	void operator () (DWORD dwTargetVID)
...
#ifdef __ENABLE_SOUL_SYSTEM__ // Arrow ninja
...
#ifndef __ANTY_SKILL_FLYING_TARGET__
...
#ifdef __ABUSE_CONTROLLER_ENABLE__
			/* Start of fix nr. 1 */
			if (std::find(m_vec_dwFlyTargets.begin(), m_vec_dwFlyTargets.end(), dwTargetVID) != m_vec_dwFlyTargets.end())
			{
				sys_log(0, "Name: %s | VID %d exists in vector FlyTargets", GetName(), dwTargetVID);
				return;
			}
			/* End of fix nr. 1 */
#endif
...
void CHARACTER::SetVictim(LPCHARACTER pkVictim)
{
	...
	else
	{
		...
		{
			MonsterLog("°ř°Ý ´ë»óŔ» ĽłÁ¤: %s", pkVictim->GetName());
		}
...
void CHARACTER::UpdateAggrPointEx(LPCHARACTER pAttacker, EDamageType type, int dam, CHARACTER::TBattleInfo & info)
{
	...
	if (pAttacker->GetPoint(POINT_AGGRO_MONSTER_BONUS))
	{
		dam = (int)(dam * (100 + pAttacker->GetPoint(POINT_AGGRO_MONSTER_BONUS)) / 100);
	}
...
void CHARACTER::ChangeVictimByAggro(int iNewAggro, LPCHARACTER pNewVictim)
{
	...
	// No, you are not fucking changing any victim ffs
#ifdef __SHIP_DUNGEON_ENABLE__
	if (pNewVictim->GetDungeon() && pNewVictim->IsPC() && !IsPC() &&
			CShipDefendDungeonManager::instance().FindDungeonByPID(pNewVictim) && CShipDefendDungeonManager::instance().FindDungeonByVID(this) &&
			CShipDefendDungeonManager::instance().FindDungeonByPID(pNewVictim) == CShipDefendDungeonManager::instance().FindDungeonByVID(this)
			&& CShipDefendDungeonManager::instance().FindDungeonByPID(pNewVictim)->FindFieldMonster(this))
	{
		return;
	}
#endif
		...
		for (it = m_map_kDamage.begin(); it != m_map_kDamage.end(); ++it)
		{
			if (it->second.iAggro > iNewAggro)
			{
				...
				if (ch && !ch->IsDead() && DISTANCE_APPROX(ch->GetX() - GetX(), ch->GetY() - GetY()) < 5000)
				{
					// No, you are not fucking changing any victim ffs
#ifdef __SHIP_DUNGEON_ENABLE__
					if (ch->GetDungeon() && ch->IsPC() && !IsPC() &&
							CShipDefendDungeonManager::instance().FindDungeonByPID(ch) && CShipDefendDungeonManager::instance().FindDungeonByVID(this) &&
							CShipDefendDungeonManager::instance().FindDungeonByPID(ch) == CShipDefendDungeonManager::instance().FindDungeonByVID(this)
							&& CShipDefendDungeonManager::instance().FindDungeonByPID(ch)->FindFieldMonster(this))
					{
						return;
					}
#endif
