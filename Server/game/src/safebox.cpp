...
void CSafebox::ItemPacket(DWORD dwPos)
{
	...
	pack.transmutate_id = pkItem->GetTransmutate();
#endif

#ifdef __ENABLE_REFINE_ELEMENT__
	pack.dwRefineElement = pkItem->GetRefineElement();
#endif
...
bool CSafebox::Add(DWORD dwPos, LPITEM pkItem)
{
	...
	ItemPacket(dwPos);
	sys_log(1, "SAFEBOX: ADD %s %s count %d", m_pkChrOwner->GetName(), pkItem->GetBaseName(), pkItem->GetCount());
	return true;
}
...
bool CSafebox::MoveItem(BYTE bCell, BYTE bDestCell, CountType count)
{
	LPITEM item;
...

	{
...
#ifdef __ITEM_TOGGLE_SYSTEM__
		if ((item2 = GetItem(bDestCell)) && item != item2 && CanModifyItem(item2) && CanStack(item, item2))
#else
		if ((item2 = GetItem(bDestCell)) && item != item2 && item2->IsStackable() && item2->GetSocket(2) == item->GetSocket(2) &&
...
#endif
		{
			...
			bool bRemoved = count >= item->GetCount();
			if (bRemoved)
			{
				Remove(bCell);
			}
		...
		bool removeOld = count == item->GetCount();
		...
		sys_log(1, "SAFEBOX: MOVE %s %d -> %d %s count %d", m_pkChrOwner->GetName(), bCell, bDestCell, item->GetBaseName(), count);
		...
		else
		{
			...
			if (!itemNew)
			{
				sys_err("cannot create safebox unstack item %u count %d", item->GetVnum(), count);
				return false;
			}

			itemNew->SetSockets(item->GetSockets());
			itemNew->SetAttributes(item->GetAttributes());

#ifdef __TRANSMUTATION_SYSTEM__
			itemNew->SetTransmutate(item->GetTransmutate());
#endif

#ifdef __ENABLE_REFINE_ELEMENT__
			itemNew->SetRefineElement(item->GetRefineElement());
#endif
...
DWORD CSafebox::GetEmptyPos(BYTE bItemSize)
{
	for (int i = 0; i < SAFEBOX_MAX_NUM; ++i)
	{
		if (IsEmpty(i, bItemSize))
		{
			return i;
		}
	}
