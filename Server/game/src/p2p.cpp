...
#ifdef __ADMIN_MANAGER__
	CAdminManager::instance().OnP2PLoginPlayer(pkCCI->dwPID);
#endif

#ifdef __ENABLE_OFFLINE_SHOP__
	COfflineShop::Login(pkCCI->dwPID, d);
#endif
...
#ifdef __ADMIN_MANAGER__
	CAdminManager::instance().OnP2PLogoutPlayer(pkCCI->dwPID);
#endif

#ifdef __ENABLE_OFFLINE_SHOP__
	COfflineShop::Logout(pkCCI->dwPID);
#endif
...
#ifdef __ADMIN_MANAGER__
LPDESC P2P_MANAGER::GetP2PDescByMapIndex(long lMapIndex)
{
	WORD wPort = CMapLocation::instance().GetPort(lMapIndex);
	if (wPort == 0 || wPort == mother_port)
	{
		return NULL;
	}

	for (TR1_NS::unordered_set<LPDESC>::iterator it = m_set_pkPeers.begin(); it != m_set_pkPeers.end(); ++it)
	{
		LPDESC pkDesc = *it;
	...
	sys_err("cannot find peer by port %hu (set size %u)", wPort, m_set_pkPeers.size());
