...
#endif
#ifndef __ENABLE_STOP_CONSUMING_STAMINA__
...
void CHARACTER::StateHorse()
{
	...
	float	START_TELEPORT_DISTANCE = 2500.0f;
	...
	if (fDist >= START_TELEPORT_DISTANCE)
	{
		Show(victim->GetMapIndex(), victim->GetX() + number(-200, 200), victim->GetY() + number(-200, 200));
