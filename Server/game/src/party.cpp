...
#ifndef __ENABLE_PARTY_UPDATE__
...
void CParty::Link(LPCHARACTER pkChr)
{
	...
	if (pkChr->IsPC())
	{
		...
		pkChr->SendPartyPositionInfo();
		for (auto elem : m_memberMap)
		{
			if (elem.second.pCharacter && elem.second.pCharacter->GetMapIndex() == pkChr->GetMapIndex())
			{
				elem.second.pCharacter->SendPartyPositionInfo(pkChr);
			}
		}
#endif
	}
...
void CParty::ComputeRolePoint(LPCHARACTER ch, BYTE bRole, bool bAdd)
{
	...
	auto k = m_iLeadership / 100.0f;

	auto bAdded = (m_iLeadership == SKILL_MAX_LEVEL);

	switch (bRole)
	{
	case PARTY_ROLE_ATTACKER:
	{
		int iBonus = (int) (20 + (200 * k));
		iBonus += (bAdded) ? ceil(iBonus * (1.0 / 10)) : 0;
...
{
	if (m_pkDungeon && pDungeon && m_pkDungeon != pDungeon)
	{
		m_pkDungeon->QuitParty(this);
	}
...
DWORD CParty::GetRandomPID(DWORD my_pid, bool bDungeon)
{
	LPCHARACTER ch;
	for (auto const& obj : m_memberMap)
	{
		if (obj.first != my_pid && (ch = CHARACTER_MANAGER::instance().FindByPID(obj.first)))
		{
			if (bDungeon && !ch->GetDungeon())
			{
				continue;
			}
...
BYTE CParty::PopulatePartyPids(DWORD tPids[])
{
	BYTE i = 0;
	LPCHARACTER ch;
	...
	for (auto const& obj : m_memberMap)
	{
		if ((ch = CHARACTER_MANAGER::instance().FindByPID(obj.first)))
		{
			tPids[i++] = obj.first;
			if (i >= PARTY_MAX_MEMBER)
			{
				break;
			}
		}
...
LPCHARACTER CParty::GetRandomPartyMember()
{
	std::vector<LPCHARACTER> v_online;
	for (auto const& obj : m_memberMap)
	{
		LPCHARACTER ch;
		if ((ch = CHARACTER_MANAGER::instance().FindByPID(obj.first)))
		{
			v_online.push_back(ch);
		}
	}

	return (v_online.size() ? v_online[number(0, v_online.size() - 1)] : nullptr);
}
#endif
