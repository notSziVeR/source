...
LPCHARACTER CHARACTER_MANAGER::SpawnMob(DWORD dwVnum, long lMapIndex, long x, long y, long z, bool bSpawnMotion, int iRot, bool bShow)
{
	...
	LPCHARACTER ch = CHARACTER_MANAGER::instance().CreateCharacter(pkMob->m_table.szLocaleName);
#else
	LPCHARACTER ch = CHARACTER_MANAGER::instance().CreateCharacter(LC_MOB_NAME(dwVnum, LC_DEFAULT));
#endif
...
void CHARACTER_MANAGER::EraseMonsterByValue(const DWORD& dwMonster, const long& lMapIndex)
{
	std::unordered_set<LPCHARACTER> s_monster_litter;
	for (const auto& pRecord : m_map_pkChrByVID)
	{
		LPCHARACTER ch = pRecord.second;
		if ((ch->IsMonster() || ch->IsStone()) && !ch->GetDungeon() && ch->GetRaceNum() == dwMonster && (!lMapIndex || ch->GetMapIndex() == lMapIndex))
		{
			if (ch->GetRegen())
			{
				(ch->GetRegen()->count)--;
			...
			s_monster_litter.insert(ch);
		}
	}

	for (const auto& ch : s_monster_litter)
	{
		M2_DESTROY_CHARACTER(ch);
	}
}

void CHARACTER_MANAGER::EraseMonsterByType(const MonsterRespawner::ESpotRefreshType& eType, const std::unordered_set<DWORD>& s_blacklist_monster)
{
	using namespace MonsterRespawner;
	std::unordered_set<LPCHARACTER> s_monster_litter;

	for (const auto& pRecord : m_map_pkChrByVID)
	{
		LPCHARACTER ch = pRecord.second;
		if (ch->GetDungeon() || s_blacklist_monster.find(ch->GetRaceNum()) != s_blacklist_monster.end())
		{
			continue;
		}
...
void CHARACTER_MANAGER::SendOnlineTeamlerList(LPDESC pkDesc)
{
	TPacketGCTeamlerStatus packet;
	packet.header = HEADER_GC_TEAMLER_STATUS;
	packet.is_online = true;

	auto& rkSet = m_set_OnlineTeamler;
	for (auto it = rkSet.begin(); it != rkSet.end(); ++it)
	{
		strlcpy(packet.szName, (*it).c_str(), sizeof(packet.szName));
		pkDesc->Packet(&packet, sizeof(packet));
	}
}

void CHARACTER_MANAGER::AddOnlineTeamler(const char* szName)
{
	m_set_OnlineTeamler.insert(szName);
}

void CHARACTER_MANAGER::RemoveOnlineTeamler(const char* szName)
{
	m_set_OnlineTeamler.erase(szName);
}
#endif

#ifdef __ENABLE_FIND_LOCATION__
TPacketGGPlayerLocation CHARACTER_MANAGER::GetPlayerLocation(const std::string& p_name)
{
	TPacketGGPlayerLocation p_ret;
	memset(&p_ret, 0, sizeof(p_ret));

	auto ch = FindPC(p_name.c_str());
	if (ch)
	{
		p_ret.lx = ch->GetX();
		p_ret.ly = ch->GetY();
		p_ret.dwMapIndex = ch->GetMapIndex();
	}
