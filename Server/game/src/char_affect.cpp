...
#define IS_NO_CLEAR_ON_DEATH_AFFECT(type) ((type) == AFFECT_BLOCK_CHAT || ((type) >= 500 && (type) < 600) || type == AFFECT_EXP_CURSE)

bool IS_NO_CLEAR_ON_DEATH_BUFF_AFFECT(uint32_t type)
{
	switch (type)
	{
	case SKILL_JEONGWI:
...
void CHARACTER::ClearAffect(bool bSave)
{
	...
	while (it != m_list_pkAffect.end())
	{
		...
		if (bSave)
		{
			if (IS_NO_CLEAR_ON_DEATH_BUFF_AFFECT(pkAff->dwType))
			{
				++it;
				continue;
			}
			...
			if (pkAff->dwType >= 857 && pkAff->dwType <= 859)
			{
				++it;
				continue;
			}
...
void CHARACTER::LoadAffect(DWORD dwCount, TPacketAffectElement * pElements)
{
	...
	for (DWORD i = 0; i < dwCount; ++i, ++pElements)
	{
...
#ifdef __ADMIN_MANAGER__
		if (pElements->dwType == AFFECT_BLOCK_CHAT)
		{
			CAdminManager::instance().OnPlayerChatban(this, pElements->lDuration);
		}
#endif
		...
		const auto SKILL_BOOK_AFFECTS = { AFFECT_SKILL_BOOK_BONUS, AFFECT_SKILL_NO_BOOK_DELAY };
		if (std::find_if(SKILL_BOOK_AFFECTS.begin(), SKILL_BOOK_AFFECTS.end(), [&](const BYTE& iAffect) { return iAffect == pElements->dwType; }) != SKILL_BOOK_AFFECTS.end())
...
bool CHARACTER::AddAffect(DWORD dwType, BYTE bApplyOn, long lApplyValue, DWORD dwFlag, long lDuration, long lSPCost, bool bOverride, bool IsCube )
{
	...
	if (dwType == AFFECT_EXP_CURSE && lDuration > 1)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Anti Exp has been enabled."));
	}
#endif
	...
	{
		CAdminManager::instance().OnPlayerChatban(this, lDuration);
	}
#endif
...
#if defined(__ENABLE_IGNORE_LOWER_BUFFS__)
	switch (dwType)
	{
	case SKILL_HOSIN:
	case SKILL_REFLECT:
...
#endif
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("The casted skill was not accepted. Current power (%ld%%) current power (%ld%%)."), lApplyValue, pkAffect->lApplyValue);
			return false;
		}
	}
	break;
...
#ifdef __ENABLE_SKILLS_INFORMATION__
		const auto SKILL_BOOK_AFFECTS = { AFFECT_SKILL_BOOK_BONUS, AFFECT_SKILL_NO_BOOK_DELAY };
		if (std::find_if(SKILL_BOOK_AFFECTS.begin(), SKILL_BOOK_AFFECTS.end(), [&](const BYTE& iAffect) { return iAffect == dwType; }) != SKILL_BOOK_AFFECTS.end())
			BroadcastSkillInformation();
#endif
...
#ifdef __ADMIN_MANAGER__
	if (pkAff->dwType == AFFECT_BLOCK_CHAT)
	{
		CAdminManager::instance().OnPlayerChatban(this, 0);
	}
#endif
...
#ifdef __ENABLE_ANTY_EXP__
	if (dwType == AFFECT_EXP_CURSE)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Anti Exp has been disabled."));
	}
#endif
