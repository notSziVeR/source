...
#define INVALID_LOGIN_RESET_TIME (3 * 60)
#define INVALID_LOGIN_MAX_COUNT 3

	typedef struct
	{
		DWORD	invalid_time;
		BYTE	invalid_counter;
	} TInvalidLoginInfo;
...
public:
	...
	int				GetInvalidLoginRemainTime(const std::string& c_rstIP);
	bool			IsInvalidLogin(const std::string& c_rstIP);
	void			AppendInvalidLogin(const std::string& c_rstIP);
...
private:
	...
	std::map<std::string, TInvalidLoginInfo>	m_map_InvalidLoginsByIP;
};
