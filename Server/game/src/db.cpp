...
bool DBManager::Connect(const char * host, const int port, const char * user, const char * pwd, const char * db)
{
	...
	if (!m_sql_direct.Setup(host, user, pwd, db, MYSQL_CHARSET, true, port))
	{
...
void DBManager::SendAuthLogin(LPDESC d)
{
...
#ifdef __ADMIN_MANAGER__
	ptod.accban_count = r.accban_count;
#endif
	...
	CLoginSim* ld = new CLoginSim;
	ld->CopyAuthPacket(ptod);
	ld->SetLoginKey(d->GetLoginKey());
	ld->SetHandle(d->GetHandle());

	CLoginQueue::instance().AddNewClient(ld);
#endif
...
void DBManager::AnalyzeReturnQuery(SQLMsg * pMsg)
{
	...
	case QID_AUTH_LOGIN:
	{
		...
		if (pMsg->Get()->uiNumRows == 0)
		{
			DESC_MANAGER::Instance().AppendInvalidLogin(d->GetHostName());
		...
		else
		{
			...
			#ifdef __HARDWARE_BAN__
				char HDId[HDID_MAX_LEN + 1];
			#endif
			DWORD dwID = 0;

#ifdef __ADMIN_MANAGER__
			DWORD dwAccbanCount = 0;
#endif
			...
			*/
			...
			str_to_number(dwAccbanCount, row[col++]);
#endif
			...
			#ifdef __HARDWARE_BAN__
				if (row[col] && *row[col])
					strlcpy(HDId, row[col++], sizeof(HDId));
				else
				{
			...
			char cBanReason[BAN_REASON_MAX_LEN] = "";
			if (row[col] && *row[col])
				strlcpy(cBanReason, row[col++], sizeof(cBanReason));
			...
			int invalidRemainTime = DESC_MANAGER::Instance().GetInvalidLoginRemainTime(d->GetHostName());
			if (invalidRemainTime > 0)
			{
				LoginFailure(d, "INVALID", invalidRemainTime);
				sys_log(0, "   INVALID");
				M2_DELETE(pinfo);
			}
			...
			{
				LoginFailure(d, "NOTAVAIL", 0, cBanReason);
				sys_log(0, "   NOTAVAIL");
				M2_DELETE(pinfo);
			}
...
#ifdef __HARDWARE_BAN__
			else if (!CheckHardware::instance().Check(HDId))
			{
				LoginFailure(d, "HARDBAN", 0, cBanReason);
				sys_log(0, "   STATUS: %s", "HARDWARBAN");
				M2_DELETE(pinfo);
			}
#endif
			...
			{
				LoginFailure(d, szStatus, 0, cBanReason);
...
#ifdef __ADMIN_MANAGER__
	case QID_ADMIN_MANAGER_BAN_LOG:
	{
		DWORD* pdwPlayerID = (DWORD*)qi->pvData;

		LPCHARACTER pkChr = CHARACTER_MANAGER::instance().FindByPID(qi->dwIdent);
		if (pkChr == NULL)
		{
			delete (pdwPlayerID);
			break;
		...
		TEMP_BUFFER buf;
		buf.write(pdwPlayerID, sizeof(DWORD));
		delete (pdwPlayerID);

		DWORD dwRowNum = pMsg->Get()->uiNumRows;
		buf.write(&dwRowNum, sizeof(DWORD));

		while (MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult))
		{
			int col = 0;

			char szReason[BLOCK_DESC_MAX_LEN + 1];
			char szProof[BLOCK_PROOF_MAX_LEN + 1];

			TAdminManagerBanClientLogInfo kInfo;
			str_to_number(kInfo.dwPlayerID, row[col++]);
			strlcpy(kInfo.szPlayerName, row[col++], sizeof(kInfo.szPlayerName));
			str_to_number(kInfo.dwGMPlayerID, row[col++]);
			strlcpy(kInfo.szGMName, row[col++], sizeof(kInfo.szGMName));
			str_to_number(kInfo.bType, row[col++]);
			str_to_number(kInfo.iNewDuration, row[col++]);
			strlcpy(szReason, row[col++], sizeof(szReason));
			kInfo.iReasonStrLen = strlen(szReason);
			strlcpy(szProof, row[col++], sizeof(szProof));
			kInfo.iProofStrLen = strlen(szProof);
			strlcpy(kInfo.szDate, row[col++], sizeof(kInfo.szDate));

			buf.write(&kInfo, sizeof(kInfo));
			buf.write(szReason, kInfo.iReasonStrLen);
			buf.write(szProof, kInfo.iProofStrLen);
		}

		CAdminManager::instance().SendClientPacket(pkChr->GetDesc(), CAdminManager::GC_SUBHEADER_BAN_PLAYER_LOG, buf.read_peek(), buf.size());
	}
	break;

	case QID_ADMIN_MANAGER_ITEM_SEARCH:
	{
		LPCHARACTER pkChr = CHARACTER_MANAGER::instance().FindByPID(qi->dwIdent);
		if (pkChr == NULL)
		{
		...
		while (MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult))
		{
			...
			szPlayerName[0] = '\0';
			DWORD dwAID = 0;
			char szAccountName[LOGIN_MAX_LEN + 1];
			szAccountName[0] = '\0';
			const char* szOwnerName;

			TAdminManagerItemInfo kInfo;
			if (row[col])
			{
			...
			if (row[col])
			{
				strlcpy(szPlayerName, row[col], sizeof(szPlayerName));
			}
			++col;
			if (row[col])
			{
			...
			if (row[col])
			{
				strlcpy(szAccountName, row[col], sizeof(szAccountName));
			}
			++col;
			str_to_number(kInfo.dwItemID, row[col++]);
			str_to_number(kInfo.kCell.window_type, row[col++]);
			str_to_number(kInfo.kCell.cell, row[col++]);
			str_to_number(kInfo.dwItemVnum, row[col++]);
			str_to_number(kInfo.bCount, row[col++]);
			str_to_number(kInfo.bIsGMItem, row[col++]);
			for (int i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
			{
				str_to_number(kInfo.alSockets[i], row[col++]);
			}
			for (int i = 0; i < ITEM_ATTRIBUTE_MAX_NUM; ++i)
			{
				str_to_number(kInfo.aAttr[i].bType, row[col++]);
				str_to_number(kInfo.aAttr[i].sValue, row[col++]);
			}

			if (kInfo.kCell.window_type == SAFEBOX || kInfo.kCell.window_type == MALL)
			{
				kInfo.bOwnerType = CAdminManager::ITEM_OWNER_ACCOUNT;
				kInfo.dwOwnerID = dwAID;
				szOwnerName = szAccountName;
			}
			else
			{
				kInfo.bOwnerType = CAdminManager::ITEM_OWNER_PLAYER;
				kInfo.dwOwnerID = dwPID;
				szOwnerName = szPlayerName;
			}

			kInfo.bOwnerNameLen = strlen(szOwnerName);
			buf.write(&kInfo, sizeof(kInfo));
			buf.write(szOwnerName, kInfo.bOwnerNameLen);
		}

		CAdminManager::instance().SendClientPacket(pkChr->GetDesc(), CAdminManager::GC_SUBHEADER_ITEM_SEARCH, buf.read_peek(), buf.size());
	}
	break;


	case QID_ADMIN_MANAGER_GET_LOG:
	{
		LPCHARACTER pkChr = CHARACTER_MANAGER::instance().FindByPID(qi->dwIdent);
		if (pkChr == NULL)
		{
		...
		while (MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult))
		{
			...
			TAdminManagerHackLog kLog;
			char dwHackerName[256];
			char dwReason[256];
			char dwTime[256];


			strlcpy(dwHackerName, row[col++], sizeof(dwHackerName));
			kLog.dwHackerNameLen = strlen(dwHackerName);
			strlcpy(dwReason, row[col++], sizeof(dwReason));
			kLog.dwHackerReasonLen = strlen(dwReason);
			str_to_number(kLog.dwHackCount, row[col++]);
			strlcpy(dwTime, row[col++], sizeof(dwTime));
			kLog.dwHackTimeLen = strlen(dwTime);

			buf.write(&kLog, sizeof(kLog));
			buf.write(dwHackerName, kLog.dwHackerNameLen);
			buf.write(dwReason, kLog.dwHackerReasonLen);
			buf.write(dwTime, kLog.dwHackTimeLen);
		}

		CAdminManager::instance().SendClientPacket(pkChr->GetDesc(), CAdminManager::GC_SUBHEADER_RECV_LOGS, buf.read_peek(), buf.size());
	}
	break;
#endif
...
void VCardUse(LPCHARACTER CardOwner, LPCHARACTER CardTaker, LPITEM item)
{
	...
	CardTaker->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d Playtime added to your account. (Payment number %d)"), item->GetSocket(1) / 60, item->GetSocket(0));
