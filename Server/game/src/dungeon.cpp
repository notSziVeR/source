...
CDungeon::~CDungeon()
{
	for (auto it = m_map_pkParty.begin(); it != m_map_pkParty.end(); ++it)
	{
		it->first->SetDungeon(NULL);
	}
	m_map_pkParty.clear();
	if (m_pParty != NULL)
	{
...
void CDungeon::DecMember(LPCHARACTER ch)
{
	...
	if (m_set_pkCharacter.empty())
	{
		...
		event_cancel(&deadEvent);
		deadEvent = event_create(dungeon_dead_event, info, PASSES_PER_SEC(DUNGEON_LIFETIME));
	}
}
...
struct FWarpToPosition
{
	...
	void operator()(LPENTITY ent)
	{
		...
		{
#ifdef __DUNGEON_RETURN_ENABLE__
			if (ch->GetParty() && ch->GetParty()->GetDungeon_for_Only_party())
			{
				ch->GetParty()->GetDungeon_for_Only_party()->AddAttenderByPID(ch->GetPlayerID());
			}
#endif
...
#ifdef __SHIP_DUNGEON_ENABLE__
	CShipDefendDungeonManager::instance().EraseDungeon(pDungeon);
#endif
...
struct FKillSectree
{
	void operator () (LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (!ch->IsPC() && !ch->IsPet() && !ch->GetRider() /* Is Mount/Horse */
...
void CDungeon::IncKillCount(LPCHARACTER pkKiller, LPCHARACTER pkVictim)
{
	...
	pkKiller->SetQuestFlag("dungeon_gui.monster_count", pkKiller->GetQuestFlag("dungeon_gui.monster_count") + 1);
	pkKiller->ChatPacket(CHAT_TYPE_COMMAND, "Dungeon_AddDatas monster_count %d", pkKiller->GetQuestFlag("dungeon_gui.monster_count"));
	if (pkVictim->IsStone())
	{
		pkKiller->SetQuestFlag("dungeon_gui.stone_count", pkKiller->GetQuestFlag("dungeon_gui.stone_count") + 1);
		pkKiller->ChatPacket(CHAT_TYPE_COMMAND, "Dungeon_AddDatas metin_count %d", pkKiller->GetQuestFlag("dungeon_gui.stone_count"));
	}
	else if (pkVictim->IsBoss())
	{
		pkKiller->SetQuestFlag("dungeon_gui.boss_count", pkKiller->GetQuestFlag("dungeon_gui.boss_count") + 1);
		pkKiller->ChatPacket(CHAT_TYPE_COMMAND, "Dungeon_AddDatas boss_count %d", pkKiller->GetQuestFlag("dungeon_gui.boss_count"));
	}
#endif
}
	...
	void operator() (LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (ch->IsPC())
			{
				...
				ch->RawChatPacket(CHAT_TYPE_NOTICE, "%s", m_psz);
#else
...
#ifdef __DUNGEON_RETURN_ENABLE__
LPDUNGEON CDungeonManager::FindDungeonByPID(const DWORD& dwPID, const DWORD & dwMapIndex)
{
	for (auto const& dng_p : m_map_pkMapDungeon)
	{
		...
		if (ch && CShipDefendDungeonManager::instance().FindDungeonByPID(ch)
				&& CShipDefendDungeonManager::instance().FindDungeonByPID(ch)->GetDungeon() == dng)
		{
			continue;
		}
#endif

		if (dwMapIndex && dng->GetMapIndex() / 10000 != dwMapIndex)
		{
			continue;
		}
...
{
	sys_err("AddAttenderByPID");
	s_attender_pid.insert(dwPID);
}

void CDungeon::EraseAttenderByPID(const DWORD& dwPID)
{
	sys_err("EraseAttenderByPID");
	s_attender_pid.erase(dwPID);
}
...
void CDungeon::SaveCoords(LPCHARACTER ch, bool bClear)
{
	sys_err("Saving coords on dungeon %d %d %d", ch->GetMapIndex(), ch->GetX(), ch->GetY());
	...
	if (bClear)
	{
		EraseAttenderByPID(ch->GetPlayerID());
	}
}

std::tuple<long, long, long> CDungeon::GetDungeonCoords(LPCHARACTER ch)
{
	if (!ch || !CheckAttenderByPID(ch->GetPlayerID()))
	{
		return std::make_tuple<long, long, long>(-1, -1, -1);
	}

	return std::make_tuple<long, long, long>(ch->GetQuestFlag("dungeon_return.x"), ch->GetQuestFlag("dungeon_return.y"), ch->GetQuestFlag("dungeon_return.index"));
}

BYTE CDungeon::GetPartyCount()
{
	return (m_pParty ? m_pParty->GetNearMemberCount() : 0);
}

long CDungeon::GetPartyX()
{
	if (m_pParty)
	{
		auto pMember = m_pParty->GetRandomPartyMember();
		if (pMember)
		{
			return pMember->GetX();
		}
