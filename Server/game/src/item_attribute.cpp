...
#ifdef __BELT_ATTR__
int CItem::GetBeltAttrCount()
{
	int i;

	for (i = 0; i < MAX_BELT_ATTR_NUM; ++i)
	{
		if (GetAttributeType(i) == 0)
		{
			break;
...
bool CItem::HasBeltAttr(BYTE bApply)
{
	for (int i = 0; i < MAX_BELT_ATTR_NUM; ++i)
		if (GetAttributeType(i) == bApply)
		{
			return true;
		}
...
bool CItem::ChangeBeltAttribute()
{
	if (GetBeltAttrCount() == 0)
	{
		return false;
	}

	int cnt = GetBeltAttrCount();
	...
	if (GetOwner() && GetOwner()->GetDesc())
	{
		LogManager::instance().ItemLog(GetOwner(), this, "SET_BELT_CHANGE", "");
	}
	else
	{
		LogManager::instance().ItemLog(0, 0, 0, GetID(), "SET_BELT_CHANGE", "", "", GetOriginalVnum());
	}
...
void CItem::ClearBeltAttr()
{
	for (size_t i = 0; i < MAX_BELT_ATTR_NUM; ++i)
	{
		m_aAttr[i].bType = 0;
		m_aAttr[i].sValue = 0;
	}
...
bool CItem::AddBeltAttribute()
{
	int count = GetBeltAttrCount();

	if (count >= MAX_BELT_ATTR_NUM)
	{
		return false;
	}
	...
	for (int i = 0; i < MAX_APPLY_NUM; ++i)
	{
		const TItemBeltAttrTable& r = g_map_itemBelt[i];

		if (r.dwApplyIndex != 0 && r.bMaxLevelBySet[nAttrSet] > 0 && HasBeltAttr(i) != true)
		{
			avail.push_back(i);
		}
	}

	const TItemBeltAttrTable& r = g_map_itemBelt[avail[number(0, avail.size() - 1)]];

	int rv = number(0, r.bMaxLevelBySet[nAttrSet] - 1);

	attr.bType = r.dwApplyIndex;
	attr.sValue = r.lValues[rv];
	...
	LogManager::instance().ItemLog(pos, attr.bType, attr.sValue, GetID(), "SET_Belt", "", pszIP ? pszIP : "", GetOriginalVnum());
	return true;
}
