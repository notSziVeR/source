...
LPITEM ITEM_MANAGER::CreateItem(DWORD vnum, CountType count, DWORD id, bool bTryMagic, int iRarePct, bool bSkipSave, bool bGameMasterCreated, bool bSkipCreateUtils)
{
	if (0 == vnum)
	{
		return NULL;
...
#ifdef __ITEM_TOGGLE_SYSTEM__
		if (!bSkipCreateUtils)
			OnCreateItem(item, bGameMasterCreated);
#endif
	...
	else if (item->IsStackable())
	{
		...
		count = boost::algorithm::clamp<CountType>(count, 1, GetItemMaxCount(item));
	...
	if (item->GetType() == ITEM_TOGGLE && (item->GetSubType() == TOGGLE_RED_SOUL || item->GetSubType() == TOGGLE_BLUE_SOUL))
	{
...
void ITEM_MANAGER::SaveSingleItem(LPITEM item, DWORD alternativeOwner)
{
	if (!item->GetOwner() && !alternativeOwner)
	{
...
void ITEM_MANAGER::GetPlayerItem(LPITEM item, TPlayerItem* result)
{
	...
	result->window = item->GetWindow();
	switch (result->window)
	{
	case EQUIPMENT:
		result->pos = item->GetCell() - INVENTORY_MAX_NUM;
		break;
#ifdef __ENABLE_BELT_INVENTORY_EX__
	case INVENTORY:
		if (BELT_INVENTORY_SLOT_START <= item->GetCell() && BELT_INVENTORY_SLOT_END > item->GetCell())
		{
			result->window = BELT_INVENTORY;
			result->pos = item->GetCell() - BELT_INVENTORY_SLOT_START;
			break;
		}
#endif
	default:
		result->pos = item->GetCell();
		break;
	}
	result->count = item->GetCount();
	result->vnum = item->GetOriginalVnum();
	if (item->GetOwner())
	{
		switch (result->window)
		{
		case SAFEBOX:
		case MALL:
			result->owner = item->GetOwner()->GetDesc()->GetAccountTable().id;
			break;
		default:
			result->owner = item->GetOwner()->GetPlayerID();
			break;
		}
	}
	else
	...
	result->is_gm_owner = item->IsGMOwner();
#endif
	...
	result->transmutate_id = item->GetTransmutate();
#endif

#ifdef __ENABLE_REFINE_ELEMENT__
	result->dwRefineElement = item->GetRefineElement();
#endif
}
...
void ITEM_MANAGER::RemoveItem(LPITEM item, const char * c_pszReason)
{
	...
	if ((o = item->GetOwner()))
	{
		...
		{
#ifdef __ITEM_TOGGLE_SYSTEM__
			OnRemoveItem(item->GetOwner(), item);
#endif
...
int ITEM_MANAGER::RealNumber(DWORD vnum)
{
	...
	{
		mid = (bot + top) >> 1;
		if (mid >= m_vec_prototype.size())
		{
			mid = m_vec_prototype.size() - 1;
		}
...
bool ITEM_MANAGER::GetDropPct(LPCHARACTER pkChr, LPCHARACTER pkKiller, OUT int& iDeltaPercent, OUT int& iRandRange)
{
	...
	iRandRange = iRandRange * 100 /
				 (100 + CPrivManager::instance().GetPriv(pkKiller, PRIV_ITEM_DROP));
...
bool ITEM_MANAGER::CreateDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM>& vec_item)
{
	...
	//If level between booth is > 15 then return;
	if ((pkKiller->GetLevel() > pkChr->GetLevel()) && (pkKiller->GetLevel() - pkChr->GetLevel()) > MAX_LEVEL_DIFFERENCE)
	{
		return false;
	}
	...
	for (const auto & info : g_vec_pkCommonDropItem[bRank])
	{
		...
		sys_log(3, "CreateDropItem %d ~ %d %d(%d)", info.m_iLevelStart, info.m_iLevelEnd, info.m_dwVnum, iPercent, info.m_iPercent);
		...
		if (iPercent >= Random::get(1, iRandRange))
		{
			...
			if (table->bType == ITEM_POLYMORPH)
			{
				...
				{
					item = CreateItem(info.m_dwVnum, 1, 0, true);
...

	{
		...
		if (it != m_map_pkDropItemGroup.end())
		{
			...
			{
				auto iPercent = v[i].dwPct;
				DropBoost(pkChr, pkKiller, iPercent);

#ifdef __DEBUG_PRINT
				cerr << "ITEM_MANAGER::CreateDropItem:: Try drop item! " << v[i].dwVnum << " Percent " << iPercent << endl;
#endif

				if (iPercent >= number(1, 10000))
				{
					...
					{
#ifdef __DEBUG_PRINT
						cerr << "ITEM_MANAGER::CreateDropItem:: PushBackItem " << item->GetVnum() << endl;
#endif
...
#ifdef __ENABLE_FIND_LETTERS_EVENT__
	if (quest::CQuestManager::instance().GetEventFlag("find_letters_drop"))
	{
		int dropChance = quest::CQuestManager::instance().GetEventFlag("find_letters_drop_chance");
		if (dropChance > 0 && dropChance < 10000)
		{
			for (int i = 0; i < FIND_LETTERS_SLOTS_NUM; i++)
			{
				BYTE bLetterAscii = CFindLetters::instance().GetLetterByPos(i);

				if (bLetterAscii == 0 || bLetterAscii == 95)
				{
					continue;
				}

				if (dropChance >= number(1, 10000))
				{
					if ((item = CreateItem(90500 + bLetterAscii, 1, 0, true)))
					{
						vec_item.push_back(item);
					}
				}
...
#ifdef __TRANSMUTATION_SYSTEM__
	pkNewItem->SetTransmutate(pkOldItem->GetTransmutate());
#endif
}
...
void ITEM_MANAGER::InitializeDropInfo()
{
	...
	{
		const TMobTable& c_rMobTable = it->second->m_table;

		TVecDropInfo* pvecDropInfo = new TVecDropInfo();

		WORD wRace = c_rMobTable.dwVnum;
...

		{
			...
			if (it != m_map_pkDropItemGroup.end())
			{
				auto vecDrops = it->second->GetVector();
				for (auto it2 = vecDrops.begin(); it2 != vecDrops.end(); ++it2)
				{
					SDropInfo kInfo;
					kInfo.dwVNum = it2->dwVnum;
					kInfo.dwCount = it2->iCount;
					kInfo.iMaxLevel = 0;
					kInfo.iMaxLevel = 0;
					kInfo.iRarity = it2->dwPct;
...

		{
			...
			if (it != m_map_pkMobItemGroup.end())
			{
				...
				if (pGroup && !pGroup->IsEmpty())
				{
					...
					auto& infoVec = pGroup->GetVec();
					for (auto& info : infoVec)
					{
						SDropInfo kInfo;
						kInfo.dwVNum = info.dwItemVnum;
						kInfo.dwCount = info.iCount;
						kInfo.iMinLevel = 0;
						kInfo.iMaxLevel = 0;
						kInfo.iRarity = info.iRarePct;
...

		{
			...
			if (it != m_map_pkLevelItemGroup.end())
			{
				...
				for (DWORD i = 0; i < v.size(); i++)
				{
					...
					kInfo.dwCount = v[i].iCount;
					kInfo.iMinLevel = 0;
					kInfo.iMaxLevel = it->second->GetLevelLimit();
					kInfo.iRarity = v[i].dwPct;
		...
		this->m_map_pkDropInfoByVNum.insert({ c_rMobTable.dwVnum, pvecDropInfo });
	}
}

ITEM_MANAGER::TVecDropInfo* ITEM_MANAGER::FindDropInfo(DWORD dwVNum)
{
...
void ITEM_MANAGER::GetClosestItems(LPCHARACTER pkChr, std::unordered_set<DWORD>& us_items)
{
	for (const auto& rPair : m_VIDMap)
	{
		if (rPair.second && rPair.second->IsOwnership(pkChr) && rPair.second->DistanceValid(pkChr))
		{
			us_items.insert(rPair.first);
		}
	}
}
...
void ITEM_MANAGER::DropBoost(LPCHARACTER pkChar, LPCHARACTER pkKiller, DWORD& iPercent)
{
	auto iDoubleDropPercent = 0.0;
	if (pkKiller->GetPremiumRemainSeconds(PREMIUM_ITEM) > 0)
	{
		//irremovable globes + mall
		iDoubleDropPercent += 2.0;
	}

	if (pkKiller->IsEquipUniqueItem(THIEF_GLOVES_30))
	{
		iDoubleDropPercent += 0.3;
	}

	if (pkKiller->IsEquipUniqueItem(THIEF_GLOVES_60))
	{
		iDoubleDropPercent += 0.5;
	}


	iPercent += iPercent * iDoubleDropPercent;

	iPercent += (CPrivManager::instance().GetPriv(pkKiller, PRIV_ITEM_DROP) / 100) * iPercent;

#ifdef __DEBUG_PRINT
	cerr << "ITEM_MANAGER::DropBoost:: Change percent! " << iPercent << endl;
#endif
}
