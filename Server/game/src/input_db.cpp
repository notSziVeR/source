...
void CInputDB::Boot(const char* data)
{
...
#ifdef __SHIP_DUNGEON_ENABLE__
	CShipDefendDungeonManager::instance().Initialize();
#endif
	...
	* ITEM COSTUME
	*/
	if (decode_2bytes(data) != sizeof(TItemBeltAttrTable))
	{
		sys_err("item belt table size error");
		thecore_shutdown();
		return;
	}
	data += 2;
	...
	size = decode_2bytes(data);
	data += 2;
	sys_log(0, "BOOT: ITEM_BELT: %d", size);

	if (size)
	{
		TItemBeltAttrTable* p = (TItemBeltAttrTable*)data;
		...
		for (int i = 0; i < size; ++i, ++p)
		{
			...
			g_map_itemBelt[p->dwApplyIndex] = *p;
			sys_log(0, "ITEM_BELT[%d]: %s %u", p->dwApplyIndex, p->szApply, p->dwProb);
		}
	}

	data += size * sizeof(TItemBeltAttrTable);
#endif
	...
	GM::init((DWORD*)data);
	data += sizeof(DWORD) * GM_MAX_NUM;

	CAdminManager::instance().Initialize((DWORD*)data);
	data += sizeof(DWORD) * GM_MAX_NUM;
	...
	if (!SECTREE_MANAGER::instance().Build(szMapIndexFileName, GetMapPath().c_str()))
	{
	...
	ITEM_MANAGER::instance().InitializeDropInfo();
	sys_log(0, "Initialized drop info map.");
#endif
...
// Legendary bosses respawn load
#ifdef __LEGENDARY_MONSTER_HELPER_ENABLE__
	CEventFunctionHandler::instance().AddEvent([] ( const SArgumentSupportImpl * empty_arg) { MonsterSpecialHelper::InitBossRespawn(); }, "LEGENDARY_BOSS_INIT_EVENT", 3);
#endif
}
...
void CInputDB::ItemLoad(LPDESC d, const char * c_pData)
{
	...
	for (DWORD i = 0; i < dwCount; ++i, ++p)
	{
		...
		item->SetTransmutate(p->transmutate_id);
#endif
...
#ifdef __ENABLE_BELT_INVENTORY_EX__
		...
		auto tmpStruct = TItemPos(p->window, p->pos);
		if (tmpStruct.IsSpecialStorage() && item->GetVirtualWindow() != tmpStruct.GetSpecialStorageType())
		{
			sys_err("SPECIAL_STORAGE: Cannot create item because of types incompatibility. Booted type: %d, item's type: %d", item->GetVirtualWindow(), tmpStruct.GetSpecialStorageType());
		...
		else
		{
			...
			case SWITCHBOT:
#endif
				if (item->AddToCharacter(ch, TItemPos(p->window, p->pos)))
					OnLoadItem(ch, item);
				break;
...
void CInputDB::BillingExpire(const char * c_pData)
{
	...
	else
	{
		if ((p->dwRemainSeconds - d->GetBillingExpireSecond()) > 60)
		{
			...
			if (ch)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Your playing time is going to run out in %d minutes."), (p->dwRemainSeconds / 60));
			}
		}
	}
}
...
#ifdef __ADMIN_MANAGER__
	GM::init((DWORD*)c_pData);
	c_pData += sizeof(DWORD) * GM_MAX_NUM;

	CAdminManager::Instance().Initialize((DWORD*)c_pData, true);
	c_pData += sizeof(DWORD) * GM_MAX_NUM;
#endif
}
//END_RELOAD_ADMIN
...
#ifdef __ADMIN_MANAGER__
void CInputDB::AdminManagerSearchPlayer(const char* c_pData)
{
	DWORD dwGMPid = *(DWORD*)c_pData;
	c_pData += sizeof(DWORD);
	bool bSuccess = *(BYTE*)c_pData;
	c_pData += sizeof(BYTE);
	TAdminManagerBanClientPlayerInfo* pPlayerInfo = NULL;
	if (bSuccess)
	{
		pPlayerInfo = (TAdminManagerBanClientPlayerInfo*)c_pData;
	}

	CAdminManager::Instance().P2P_SearchBanChatPlayer(dwGMPid, bSuccess, pPlayerInfo);
}
#endif
...
int CInputDB::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	case HEADER_DG_CHANNEL_SWITCH:
		ChannelSwitch(DESC_MANAGER::instance().FindByHandle(m_dwHandle), ((TChannelSwitchInfo*)c_pData));
		break;
#endif

#ifdef __OFFLINE_MESSAGE_SYSTEM__
	case HEADER_DG_OFFLINE_MESSAGE:
	{
		const TPacketGDOfflineMessage * p_MsgStrc = (TPacketGDOfflineMessage *) c_pData;
		switch (p_MsgStrc->subheader)
		{
		case SUBHEADER_OFFLINE_MESSAGE_CHANGE_NAME:
			COfflineMessage::instance().P2PUpdateName(p_MsgStrc->sender, p_MsgStrc->target);
			break;
		case SUBHEADER_OFFLINE_MESSAGE_DELETE_NAME:
			COfflineMessage::instance().P2PDeleteMessage(p_MsgStrc->sender);
			break;
		case SUBHEADER_OFFLINE_MESSAGE_ADD_RECORD:
			COfflineMessage::instance().P2PAddMessage(p_MsgStrc);
			break;
		case SUBHEADER_OFFLINE_MESSAGE_REMOVE_RECORD:
			COfflineMessage::instance().P2PDeleteMessage(p_MsgStrc->target);
			break;
		}
	}
	break;
...
#ifdef __ADMIN_MANAGER__
	case HEADER_DG_ADMIN_MANAGER_CHAT_SEARCH_PLAYER:
		AdminManagerSearchPlayer(c_pData);
		break;
#endif

#ifdef __BATTLE_PASS_ENABLE__
	case HEADER_DG_BATTLEPASS_RANKING:
		CBattlePassManager::instance().RecvHighScoreUpdate((const TBattlePassHighScore *) c_pData);
		break;
#endif
	...
	case HEADER_DG_ITEMSHOP:
		CItemShopManager::instance().RecvItemShopPacket((const TItemShopUniversalPacket *) c_pData);
		break;
#endif

	#ifdef __DUNGEON_INFO_ENABLE__
		case HEADER_DG_DUNGEONINFO_RANKING:
			DIHighscoreRecord(c_pData);
		break;
	#endif

	#ifdef __HARDWARE_BAN__
		case HEADER_DG_UPDATE_BAN:
			UpdateHardwareBan(c_pData);
			break;
	#endif
...
#ifdef __ENABLE_CHANGE_CHANNEL__
void CInputDB::ChannelSwitch(LPDESC pkDesc, TChannelSwitchInfo* pkData)
{
	if (!pkDesc)
	{
		return;
	...
	LPCHARACTER pkCharacter = pkDesc->GetCharacter();
	if (!pkCharacter)
	{
		return;
	}

	long lAddr = inet_addr(pkData->szIP);
	if (lAddr == 0 && pkData->wPort)
	{
		pkCharacter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("The selected channel does not contain this map."));
		return;
	}

	pkCharacter->StartSwitchChannelEvent(lAddr, pkData->wPort);
}
#endif
...
#ifdef __DUNGEON_INFO_ENABLE__
void CInputDB::DIHighscoreRecord(const char* c_pData)
{
	CDungeonInfoManager::instance().RecvHighscoreRecord((TPacketGDDIRanking *) c_pData);
}
#endif

#ifdef __HARDWARE_BAN__
void CInputDB::UpdateHardwareBan(const char * pcData)
{
	TPacketGDDisconnectPlayer * data = (TPacketGDDisconnectPlayer *) pcData;

	if (data->type)
		CheckHardware::instance().AddBan(data->ban_hardware);
	else
		CheckHardware::instance().RemoveBan(data->ban_hardware);
}
#endif
