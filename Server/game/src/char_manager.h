...
class CharacterVectorInteractor;
	...
	typedef struct SPacketGGPlayerLocation TPacketGGPlayerLocation;
#endif
...
#ifdef __CYCLIC_MONSTER_RESPAWNER__
	void			EraseMonsterByValue(const DWORD & dwMonster, const long & lMapIndex);
	void			EraseMonsterByType(const MonsterRespawner::ESpotRefreshType & eType, const std::unordered_set<DWORD> & s_blacklist_monster);
#endif

#ifdef __ENABLE_FIND_LOCATION__
	TPacketGGPlayerLocation	GetPlayerLocation(const std::string& p_name);
#endif
...
public:
	void				SendOnlineTeamlerList(LPDESC pkDesc);
	void				AddOnlineTeamler(const char* szName);
	void				RemoveOnlineTeamler(const char* szName);

private:
	std::set<std::string>			m_set_OnlineTeamler;
#endif
};
