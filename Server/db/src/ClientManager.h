...
public:
	...
	typedef std::unordered_map<std::string, CPlayerTableCache*> TPlayerTableNameCacheMap;
#endif
...
#ifdef __ADMIN_MANAGER__
	CPlayerTableCache* GetPlayerCacheByName(const char* c_pszName);
#endif
...
#ifdef __ENABLE_MULTI_LANGUAGE_DB_LOAD_ITEM__
	const TEntityLocalizationMap* GetItemNames(BYTE langID) const;
	const char* GetItemName(BYTE langID, DWORD vnum) const;
#endif
...
private:
	bool		InitializeTables(bool isReload = false);
...
#ifdef __ENABLE_MULTI_LANGUAGE_DB_LOAD_ITEM__
	bool		InitializeItemLocalization();
	bool		InitializeItemLocalization(BYTE langID, const std::string& fileName);
#endif
...
private:
	...
	void		ChannelSwitch(CPeer* pkPeer, DWORD dwHandle, TChannelSwitch* pkPacket);
#endif
...
#ifdef __BELT_ATTR__
	std::vector<TItemBeltAttrTable>		m_vec_itemBeltTable;
#endif
...
#ifdef __ADMIN_MANAGER__
	TPlayerTableNameCacheMap		m_map_playerNameCache;
#endif
...
#ifdef __ENABLE_MULTI_LANGUAGE_DB_LOAD_ITEM__
	std::array<TEntityLocalizationMap, LC_COUNT> m_map_ItemLocalization;
#endif
...
private:
	...
	bool __GetAdminConfig(DWORD adwAdminConfig[GM_MAX_NUM], DWORD adwAdminManagerConfig[GM_MAX_NUM]);
...
#ifdef __ADMIN_MANAGER__
public:
	void AdminManagerSearchPlayer(CPeer* pkPeer, const char* c_pData);
#endif
