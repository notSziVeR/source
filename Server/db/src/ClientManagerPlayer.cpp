...
bool CreateItemTableFromRes(MYSQL_RES * res, std::vector<TPlayerItem> * pVec, DWORD dwPID)
{
	...
	for (int i = 0; i < rows; ++i)
	{
		...
		{
			str_to_number(item.alSockets[k], row[cur++]);
		}
		...
		str_to_number(item.transmutate_id, row[cur++]);
#endif
...
size_t CreatePlayerSaveQuery(char * pszQuery, size_t querySize, TPlayerTable * pkTab)
{
...
#endif
#ifdef __ENABLE_MARBLE_CREATOR_SYSTEM__
						, pkTab->m_MarbleActiveMission
						, pkTab->m_MarbleActiveExtandedCount
						, pkTab->m_MarbleKilledMonsters
#endif
					   );
	...
	CDBManager::instance().EscapeString(text, pkTab->letterSlots, sizeof(pkTab->letterSlots));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, "letter_slots = '%s' ", text);
#endif
...
#ifdef __ADMIN_MANAGER__
CPlayerTableCache* CClientManager::GetPlayerCacheByName(const char* c_pszName)
{
	TPlayerTableNameCacheMap::iterator it = m_map_playerNameCache.find(str_to_lower(c_pszName));

	if (it == m_map_playerNameCache.end())
	{
		return NULL;
	}
	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		else
		{
			CDBManager::instance().ReturnQuery(
				fmt::format("SELECT id, `window`+0, pos, count, vnum, is_gm_owner,"
							...
							"FROM item WHERE owner_id = {} AND (`window` in ({}, {}, {}, {}, {}))",
							pTab->id, INVENTORY, EQUIPMENT, DRAGON_SOUL_INVENTORY, BELT_INVENTORY, SWITCHBOT
							).c_str(), QID_ITEM, peer->GetHandle(), new ClientHandleInfo(dwHandle, pTab->id));

			CDBManager::instance().ReturnQuery(
				fmt::format("SELECT dwPID, szName, szState, lValue FROM quest WHERE dwPID={}", pTab->id).c_str(),
				QID_QUEST, peer->GetHandle(), new ClientHandleInfo(dwHandle, pTab->id));											  

			CDBManager::instance().ReturnQuery(
				fmt::format("SELECT dwPID, bType, bApplyOn, lApplyValue, dwFlag, lDuration, lSPCost FROM affect WHERE dwPID={}", pTab->id).c_str(),
				QID_AFFECT, peer->GetHandle(), new ClientHandleInfo(dwHandle, pTab->id));
		}
	}
	else
	{
		...
		CDBManager::instance().ReturnQuery(
			fmt::format("SELECT id, `window`+0, pos, count, vnum, is_gm_owner,"
				...
				"trans_id, refine_element "
				"FROM item WHERE owner_id = {} AND (`window` in ({}, {}, {}, {}, {}))",
				packet->player_id, INVENTORY, EQUIPMENT, DRAGON_SOUL_INVENTORY, BELT_INVENTORY, SWITCHBOT
			).c_str(), QID_ITEM, peer->GetHandle(), new ClientHandleInfo(dwHandle, packet->player_id));

		CDBManager::instance().ReturnQuery(
			fmt::format("SELECT dwPID, szName, szState, lValue FROM quest WHERE dwPID={}", packet->player_id).c_str(),
			QID_QUEST, peer->GetHandle(), new ClientHandleInfo(dwHandle, packet->player_id, packet->account_id));

		CDBManager::instance().ReturnQuery(
			fmt::format("SELECT dwPID, bType, bApplyOn, lApplyValue, dwFlag, lDuration, lSPCost FROM affect WHERE dwPID={}", packet->player_id).c_str(),
			QID_AFFECT, peer->GetHandle(), new ClientHandleInfo(dwHandle, packet->player_id));
	}
}
...
#ifdef __GAYA_SHOP_SYSTEM__
	str_to_number(pkTab->gaya_coins, row[col++]);
#endif
#ifdef __ENABLE_HIDE_COSTUMES__
	str_to_number(pkTab->dwCostumeFlag, row[col++]);
#endif
#ifdef __ADMIN_MANAGER__
	str_to_number(pkTab->chat_ban_count, row[col++]);
#endif
#ifdef __ENABLE_BIOLOG_SYSTEM__
...
#ifdef __ENABLE_MARBLE_CREATOR_SYSTEM__
	str_to_number(pkTab->m_MarbleActiveMission, row[col++]);
	str_to_number(pkTab->m_MarbleActiveExtandedCount, row[col++]);
	str_to_number(pkTab->m_MarbleKilledMonsters, row[col++]);
#endif

#ifdef __ENABLE_FIND_LETTERS_EVENT__
	if (row[col])
	{
		thecore_memcpy(pkTab->letterSlots, row[col], sizeof(pkTab->letterSlots));
	}
	else
	{
		memset(pkTab->letterSlots, 0, sizeof(pkTab->letterSlots));
	}
#endif

#ifdef ENABLE_OFFICIAL_SELECT_GUI
	pkTab->last_play = pkTab->logoff_interval;
#endif
...
void CClientManager::__QUERY_PLAYER_CREATE(CPeer *peer, DWORD dwHandle, TPlayerCreatePacket* packet)
{
	...
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen,
						 ...
						 packet->player_table.part_base, packet->player_table.part_base, packet->player_table.parts[PART_HAIR], packet->player_table.gold, packet->player_table.parts[PART_SASH]);

#ifdef __EXTANDED_GOLD_AMOUNT__
	sys_log(0, "PlayerCreate accountid %d, name %s, level %d, gold %lld, st %d, ht %d, job %d, hair %d sash %d",
#else
	sys_log(0, "PlayerCreate accountid %d name %s level %d gold %d, st %d ht %d job %d, hair %d sash %d",
#endif
			packet->account_id,
			...
			packet->player_table.parts[PART_SASH]);
...
#ifdef __ENABLE_FIND_LETTERS_EVENT__
	CDBManager::instance().EscapeString(text, packet->player_table.letterSlots, sizeof(packet->player_table.letterSlots));
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, "'%s', ", text);
#endif
...
#ifdef __OFFLINE_MESSAGE_SYSTEM__
		snprintf(queryStr, sizeof(queryStr), "DELETE FROM whisper_offline%s WHERE target = '%s' OR sender = '%s'", GetTablePostfix(), szName, szName);
		CDBManager::instance().AsyncQuery(queryStr);

		// Updating player name container
		CMessageOfflineManager::instance().DeletePlayer(szName);
#endif
...
#ifdef __ENABLE_MULTI_LANGUAGE_DB_LOAD_ITEM__
const CClientManager::TEntityLocalizationMap* CClientManager::GetItemNames(BYTE langID) const
{
	return &m_map_ItemLocalization[langID];
}

const char* CClientManager::GetItemName(BYTE langID, DWORD vnum) const
{
	auto it = m_map_ItemLocalization[langID].find(vnum);
	if (it == m_map_ItemLocalization[langID].end())
	{
		return NULL;
	}
