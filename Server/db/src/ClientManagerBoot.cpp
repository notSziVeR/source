...
bool CClientManager::InitializeTables(bool isReload)
{
	...
	if (!InitializeItemLocalization())
	{
		sys_err("InitializeItemLocalization FAILED");
		return false;
	}
#endif
...
#ifdef __BELT_ATTR__
	if (!InitializeItemBeltTable())
	{
		sys_err("InitializeItemBeltTable FAILED");
		return false;
	}
#endif
...
bool CClientManager::InitializeShopTable()
{
	...
	static const char * s_szQuery =
		...
		"ON shop.vnum = shop_item.shop_vnum ORDER BY shop.vnum, shop_item.id";
...
bool CClientManager::InitializeBanwordTable()
{
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		if (data[0])
		{
			...
			if (std::find(m_vec_banwordTable.begin(), m_vec_banwordTable.end(), data[0]) == m_vec_banwordTable.end())
			{
...
#if defined(ENABLE_ITEM_ATTR_COSTUME) && defined(__ENABLE_WEAPON_COSTUME_SYSTEM__)
			 ", costume_weapon"
#endif
#endif
...
#ifdef __BELT_ATTR__
bool CClientManager::InitializeItemBeltTable()
{
	char query[4096];
	snprintf(query, sizeof(query),
			 "SELECT apply, apply+0, prob, lv1, lv2, lv3, belt FROM item_attr_belt ORDER BY apply");
	...
	if (!pRes->uiNumRows)
	{
		sys_err("no result from belt_attr");
		return false;
	}

	if (!m_vec_itemBeltTable.empty())
	{
		sys_log(0, "RELOAD: costume_attr");
		m_vec_itemBeltTable.clear();
	}

	m_vec_itemBeltTable.reserve(pRes->uiNumRows);

	MYSQL_ROW	data;

	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		TItemBeltAttrTable t;

		memset(&t, 0, sizeof(TItemBeltAttrTable));
		...
		sys_log(0, "ITEM_BELT: %-20s %4lu { %3d %3d %3d } { %d }",
				t.szApply,
				t.dwProb,
				t.lValues[0],
				t.lValues[1],
		...
		m_vec_itemBeltTable.push_back(t);
	}
...
#ifdef __ENABLE_CLEAR_OLD_GUILD_LANDS_BY_INACTIVITY__
...
bool CClientManager::InitializeObjectProto()
{
	...
	if (pRes->uiNumRows > 0)
		while ((data = mysql_fetch_row(pRes->pSQLResult)))
		{
			...
			sys_log(0, "OBJ_PROTO: vnum %lu price %lld mat %lu %lu",
#else
...
bool CClientManager::InitializeItemTableFromDB()
{
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
...
#ifdef __SPECIAL_STORAGE_ENABLE__
		VERIFY_IFIELD(IProto::dwVirtualWindow, item_table->dwVirtualWindow);
#endif

		VERIFY_IFIELD(IProto::equipmentSetId, item_table->equipmentSetId);

#if !defined(ENABLE_AUTODETECT_VNUMRANGE)
		VERIFY_IFIELD(IProto::vnum_range,
#else
		...
		if (item_table->dwVnum == 90500)
		{
			item_table->dwVnumRange = 100;
		}
#endif
...
#undef NUMBER_FROM_RESULT
		...
		itemQuery << "transmutation, ";
#endif
#ifdef __ENABLE_REFINE_ELEMENT__
		itemQuery << "refineElement, ";
#endif
...
#define NUMBER_FROM_RESULT(field) str_to_number(item.field, itemData[col++]);
...
#ifdef __TRANSMUTATION_SYSTEM__
			NUMBER_FROM_RESULT(transmutation);
#endif
#ifdef __ENABLE_REFINE_ELEMENT__
			NUMBER_FROM_RESULT(refineElement);
#endif
			for (size_t i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
			{
			...
			for (size_t i = 0; i < ITEM_ATTRIBUTE_MAX_NUM; ++i)
			{
				NUMBER_FROM_RESULT(attributes[i].bType);
				NUMBER_FROM_RESULT(attributes[i].sValue);
			}
...
#ifdef __ENABLE_MULTI_LANGUAGE_DB_LOAD_ITEM__
bool CClientManager::InitializeItemLocalization()
{
	if (!InitializeItemLocalization(LC_ENGLISH, "locale/lang/en/entities/item_names.txt"))
	{
		return false;
	}

	if (!InitializeItemLocalization(LC_GERMANY, "locale/lang/de/entities/item_names.txt"))
	{
		return false;
	}

	if (!InitializeItemLocalization(LC_POLAND, "locale/lang/pl/entities/item_names.txt"))
	{
		return false;
	}
...
bool CClientManager::InitializeItemLocalization(BYTE langID, const std::string& fileName)
{
	auto& localeMap = m_map_ItemLocalization[langID];
	localeMap.clear();

	io::CSVReader<2, io::trim_chars<' ', '\t'>, io::no_quote_escape<'\t'>> csv(fileName);
