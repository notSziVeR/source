...
// END_OF_MYSHOP_PRICE_LIST
	...
	uint32_t g_offlineShopCacheFlushSeconds = 5;// 60 * 10;
	uint32_t g_offlineShopItemCacheFlushSeconds = 5;// 60 * 10;
#endif
...
int main()
{
	...
	CNetPoller::get_instance();
	CDBManager::get_instance();
	CClientManager::get_instance();
	CGuildManager::get_instance();
	CPrivManager::get_instance();
	CMoneyLog::get_instance();
	ItemAwardManager::get_instance();
	marriage::CManager::get_instance();
	CBlockCountry::get_instance();
	CItemIDRangeManager::get_instance();
#ifdef __SHOP_SEARCH__
	CShopSearchManager::get_instance();
#endif
	if (!Start())
	{
		return 1;
	...
	CMapManager::get_instance();

#ifdef __OFFLINE_MESSAGE_SYSTEM__
	CMessageOfflineManager::get_instance();
#endif

#ifdef __BATTLE_PASS_ENABLE__
	CBattlePassRankingManager::instance().get_instance();
#endif
...
#ifdef __DUNGEON_INFO_ENABLE__
	CDungeonInfoRankingManager::instance().get_instance();
#endif

	CGuildManager::instance().Initialize();
	marriage::CManager::instance().Initialize();
	CBlockCountry::instance().Load();
	CItemIDRangeManager::instance().Build();
#ifdef __ENABLE_OFFLINE_SHOP__
...
#ifdef __OFFLINE_MESSAGE_SYSTEM__
	CMessageOfflineManager::instance().Destroy();
#endif

#ifdef __BATTLE_PASS_ENABLE__
	CBattlePassRankingManager::instance().Save();
#endif

#ifdef __DUNGEON_INFO_ENABLE__
	CDungeonInfoRankingManager::instance().Flush(true);
#endif
