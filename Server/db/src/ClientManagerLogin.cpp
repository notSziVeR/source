...
void CClientManager::QUERY_LOGIN_BY_KEY(CPeer * pkPeer, DWORD dwHandle, TPacketGDLoginByKey * p)
{
	...
	pkTab->accban_count = r.accban_count;
#endif
...
#ifdef __ENABLE_SASH_SYSTEM__
					pkTab->players[j].dwSashPart		= pt->parts[PART_SASH];
#endif
...
#ifdef __ENABLE_SASH_SYSTEM__
					str_to_number(pkTab->players[j].dwSashPart, row[col++]);
#endif
