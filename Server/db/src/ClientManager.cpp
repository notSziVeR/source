...
void CClientManager::QUERY_BOOT(CPeer* peer, TPacketGDBoot * p)
{
...
#ifdef __ADMIN_MANAGER__
	DWORD adwAdminConfig[GM_MAX_NUM], adwAdminManagerConfig[GM_MAX_NUM];
	__GetAdminConfig(adwAdminConfig, adwAdminManagerConfig);
#endif
	...
	DWORD dwPacketSize =
...
#ifdef __BELT_ATTR__
		sizeof(WORD) + sizeof(WORD) + sizeof(TItemBeltAttrTable) * m_vec_itemBeltTable.size() +
#endif
		...
		//ADMIN_MANAGER
#ifdef __ADMIN_MANAGER__
		sizeof(WORD) + sizeof(WORD) + sizeof(tAdminInfo) * vAdmin.size() + sizeof(adwAdminConfig) +
		sizeof(adwAdminManagerConfig) +
#else
		...
		//END_ADMIN_MANAGER
#ifdef __ENABLE_OFFLINE_SHOP__
		sizeof(DWORD) + sizeof(DWORD) + sizeof(TOfflineShop) * offlineShopCache_.size() +
		sizeof(DWORD) + sizeof(DWORD) + sizeof(TOfflineShopItem) * offlineShopItemCache_.size() +
#endif
		sizeof(WORD);
...
#ifdef __BELT_ATTR__
	sys_log(0, "sizeof(TItemBeltTable) = %d", sizeof(TItemBeltAttrTable));
#endif
...
#ifdef __BELT_ATTR__
	peer->EncodeWORD(sizeof(TItemBeltAttrTable));
	peer->EncodeWORD(m_vec_itemBeltTable.size());
	peer->Encode(&m_vec_itemBeltTable[0], sizeof(TItemBeltAttrTable)* m_vec_itemBeltTable.size());
#endif
	...
	peer->Encode(&adwAdminManagerConfig[0], sizeof(adwAdminManagerConfig));
#endif
...
void CClientManager::RESULT_SAFEBOX_LOAD(CPeer * pkPeer, SQLMsg * msg)
{
	...
	if (pi->account_index == 0)
	{
		...
		snprintf(szQuery, sizeof(szQuery),
				 ...
				 "is_gm_owner,"
#endif
				 "socket0, socket1, socket2, socket3, socket4, socket5, socket6, socket7, socket8, socket9, "
...
void CClientManager::QUERY_SETUP(CPeer * peer, DWORD dwHandle, const char * c_pData)
{
	...
	for (size_t i = 0; i < (sizeof(p->alMaps) / sizeof(p->alMaps[0])); ++i)
		if (p->alMaps[i] != 0)
		{
			CMapManager::instance().AddMap(p->bChannel, p->alMaps[i], p->szPublicIP, p->wListenPort);
		}
...
void CClientManager::QUERY_ITEM_SAVE(CPeer * pkPeer, const char * c_pData)
{
	...
	if (p->window == SAFEBOX || p->window == MALL)
	{
		...
		CDBManager::instance().ReturnQuery(fmt::format("REPLACE INTO item (id, owner_id, window, pos, count, vnum, is_gm_owner,"
			"socket0, socket1, socket2, socket3, socket4, socket5, socket6, socket7, socket8, socket9,"
			"attrtype0, attrvalue0,"
			"attrtype1, attrvalue1,"
			"attrtype2, attrvalue2,"
			...
			"attrtype5, attrvalue5,"
			"attrtype6, attrvalue6,"
			"trans_id, refine_element"
			") VALUES({id}, {owner_id}, {window}, {pos}, {count}, {vnum}, {is_gm_owner}, {socket0}, {socket1}, {socket2}, {socket3}, {socket4}, {socket5}, {socket6}, {socket7}, {socket8}, {socket9}, {attrtype0}, {attrvalue0}, {attrtype1}, {attrvalue1}, {attrtype2}, {attrvalue2}, {attrtype3}, {attrvalue3}, {attrtype4}, {attrvalue4}, {attrtype5}, {attrvalue5}, {attrtype6}, {attrvalue6}, {trans_id}, {refine_element})",
			"id"_a = p->id,
			"owner_id"_a = p->owner,
			"window"_a = p->window,
			"pos"_a = p->pos,
			"count"_a = p->count,
			"vnum"_a = p->vnum,
			"is_gm_owner"_a = p->is_gm_owner,
			"socket0"_a = p->alSockets[0],
			"socket1"_a = p->alSockets[1],
			"socket2"_a = p->alSockets[2],
			"socket3"_a = p->alSockets[3],
			"socket4"_a = p->alSockets[4],
			"socket5"_a = p->alSockets[5],
			"socket6"_a = p->alSockets[6],
			"socket7"_a = p->alSockets[7],
			"socket8"_a = p->alSockets[8],
			"socket9"_a = p->alSockets[9],
			"attrtype0"_a = p->aAttr[0].bType,
			"attrvalue0"_a = p->aAttr[0].sValue,
			"attrtype1"_a = p->aAttr[1].bType,
			"attrvalue1"_a = p->aAttr[1].sValue,
			"attrtype2"_a = p->aAttr[2].bType,
			"attrvalue2"_a = p->aAttr[2].sValue,
			"attrtype3"_a = p->aAttr[3].bType,
			"attrvalue3"_a = p->aAttr[3].sValue,
			"attrtype4"_a = p->aAttr[4].bType,
			"attrvalue4"_a = p->aAttr[4].sValue,
			"attrtype5"_a = p->aAttr[5].bType,
			"attrvalue5"_a = p->aAttr[5].sValue,
			"attrtype6"_a = p->aAttr[6].bType,
			"attrvalue6"_a = p->aAttr[6].sValue,
			"trans_id"_a = p->transmutate_id,
			"refine_element"_a = p->dwRefineElement).c_str(), QID_ITEM_SAVE, pkPeer->GetHandle(), NULL);
	}
	else
	{
		if (g_test_server)
...
void CClientManager::BlockChat(TPacketBlockChat* p)
{
	...
	{
		if (CPlayerTableCache* pCache = GetPlayerCache(p->dwPID))
		{
			pCache->Get()->chat_ban_count++;
			pCache->OnFlush();
		}
		else
		{
			char szQuery[1024 + 1];
			snprintf(szQuery, sizeof(szQuery), "UPDATE player SET chatban_count = chatban_count + 1 WHERE id = %u", p->dwPID);
			CDBManager::instance().AsyncQuery(szQuery);
		}
	}
}
...
void CClientManager::ChannelSwitch(CPeer* pkPeer, DWORD dwHandle, TChannelSwitch* pkPacket)
{
	CMapManager::TMapHostInfo* pkHostInfo = CMapManager::instance().GetMapHostInfo(pkPacket->byChannel, pkPacket->lMapIndex);

	TChannelSwitchInfo packet;
	if (pkHostInfo)
	{
		memcpy(packet.szIP, pkHostInfo->szIP, sizeof(packet.szIP));
		packet.wPort = pkHostInfo->wPort;
	}
	else
	{
		memset(packet.szIP, '\0', sizeof(packet.szIP));
		packet.wPort = 0;
	}

	pkPeer->EncodeHeader(HEADER_DG_CHANNEL_SWITCH, dwHandle, sizeof(TChannelSwitchInfo));
	pkPeer->Encode(&packet, sizeof(TChannelSwitchInfo));
}
#endif
...
#ifdef __ENABLE_CHANGE_CHANNEL__
		case HEADER_GD_CHANNEL_SWITCH:
			ChannelSwitch(peer, dwHandle, ((TChannelSwitch*)data));
			break;
#endif
...
#ifdef __OFFLINE_MESSAGE_SYSTEM__
		case HEADER_GD_OFFLINE_MESSAGE:
		{
			const TPacketGDOfflineMessage * message_packet = (const TPacketGDOfflineMessage *) data;
			switch (message_packet->subheader)
			{
			case SUBHEADER_OFFLINE_MESSAGE_BOOT:
				CMessageOfflineManager::instance().BroadcastResults(peer);
				break;
			case SUBHEADER_OFFLINE_MESSAGE_CHANGE_NAME:
				CMessageOfflineManager::instance().UpdatePlayerName(message_packet);
				break;
			case SUBHEADER_OFFLINE_MESSAGE_ADD_RECORD:
				CMessageOfflineManager::instance().RecvNewMessage(message_packet);
				break;
			case SUBHEADER_OFFLINE_MESSAGE_REMOVE_RECORD:
				CMessageOfflineManager::instance().DeleteMessage(message_packet);
				break;
			}
		}
		break;
...
#ifdef __ADMIN_MANAGER__
		case HEADER_GD_ADMIN_MANAGER_CHAT_SEARCH_PLAYER:
			AdminManagerSearchPlayer(peer, data);
			break;
#endif

#ifdef __BATTLE_PASS_ENABLE__
		case HEADER_GD_BATTLEPASS_RANKING:
			CBattlePassRankingManager::instance().RecvGamePacket(peer, (const TBattlePassHighScore*)data);
			break;
#endif

#ifdef __ITEM_SHOP_ENABLE__
		case HEADER_GD_ITEMSHOP:
			CItemShopManager::instance().RecvItemShopPacket(peer, (const TItemShopUniversalPacket *) data);
			break;
#endif

		#ifdef __DUNGEON_INFO_ENABLE__
			case HEADER_GD_DUNGEONINFO_RANKING:
			case HEADER_GD_DUNGEONINFO_RANKING_BOOT:
				DungeonInfoRankingUpdate(peer, header, data);
				break;
		#endif

		#ifdef __HARDWARE_BAN__
			case HEADER_GD_UPDATE_BAN:
				if (m_pkAuthPeer)
				{
					m_pkAuthPeer->EncodeHeader(HEADER_DG_UPDATE_BAN, 0, sizeof(TPacketGDDisconnectPlayer)); // Thats for length but THIS FUCKG SHIT DOES NOT WORK
					m_pkAuthPeer->Encode(data, sizeof(TPacketGDDisconnectPlayer));
				}
				break;
		#endif
...
#ifdef __SHOP_SEARCH__
	CShopSearchManager::Instance().CleanupPeer(pPeer);
#endif
...
#ifdef __OFFLINE_MESSAGE_SYSTEM__
			CMessageOfflineManager::instance().CleanupOutdatedMessage();
#endif
#ifdef __BATTLE_PASS_ENABLE__
			CBattlePassRankingManager::instance().Flush();
#endif
...
#ifdef __ITEM_SHOP_ENABLE__
			CItemShopManager::instance().Process();
#endif

#ifdef __DUNGEON_INFO_ENABLE__
			CDungeonInfoRankingManager::instance().Flush();
#endif
		}
...
#ifdef __ADMIN_MANAGER__
	bool CClientManager::__GetAdminConfig(DWORD pAdminConfig[GM_MAX_NUM], DWORD pAdminManagerConfig[GM_MAX_NUM])
#else
	bool CClientManager::__GetAdminConfig(DWORD pAdminConfig[GM_MAX_NUM])
#endif
{
	memset(pAdminConfig, 1, sizeof(DWORD) * GM_MAX_NUM);
	memset(pAdminManagerConfig, 0, sizeof(DWORD) * GM_MAX_NUM);

	char szQuery[512];
#ifdef __ADMIN_MANAGER__
	snprintf(szQuery, sizeof(szQuery), "SELECT authority, general_allow+0, manager_allow+0 FROM gmconfig");
#else
	...
	while ((row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
	{
		...
		else if (!stAuthority.compare("VIP"))
		{
			bAuthority = GM_VIP;
		}
#endif
...
#ifdef __ADMIN_MANAGER__
		DWORD dwManagerAllow = 0;
		if (row[col])
		{
			str_to_number(dwManagerAllow, row[col]);
		}
		col++;
#endif
...
#ifdef __ADMIN_MANAGER__
		pAdminManagerConfig[bAuthority] = dwManagerAllow;
#endif
	}
...
void CClientManager::ReloadAdmin()
{
	...
	__GetAdminConfig(adwAdminConfig, adwAdminManagerConfig);
#else
	DWORD adwAdminConfig[GM_MAX_NUM];
	__GetAdminConfig(adwAdminConfig);
#endif

	DWORD dwPacketSize = sizeof(WORD) + sizeof(tAdminInfo) * vAdmin.size() + sizeof(DWORD) * GM_MAX_NUM;
#ifdef __ADMIN_MANAGER__
	dwPacketSize += sizeof(DWORD) * GM_MAX_NUM;
#endif
...
#ifdef __ADMIN_MANAGER__
void CClientManager::AdminManagerSearchPlayer(CPeer* pkPeer, const char* c_pData)
{
	...
	char* szEscapeName = new char[iNameLen * 2 + 1];
	CDBManager::instance().EscapeString(szEscapeName, szName, iNameLen);
	delete[] szName;

	TAdminManagerBanClientPlayerInfo kPlayerInfo;
	memset(&kPlayerInfo, 0, sizeof(kPlayerInfo));

	if (CPlayerTableCache* pCache = GetPlayerCacheByName(szName))
	{
		TPlayerTable* pTab = pCache->Get(false);

		kPlayerInfo.dwPID = pTab->id;
		strlcpy(kPlayerInfo.szName, pTab->name, sizeof(kPlayerInfo.szName));
		kPlayerInfo.bRace = pTab->job;
		kPlayerInfo.bLevel = pTab->level;
		kPlayerInfo.wChatbanCount = pTab->chat_ban_count;

		{
			char szQuery[1024 + 1];
			snprintf(szQuery, sizeof(szQuery), "SELECT account.ban_count FROM account.account "
					 "INNER JOIN player.player ON player.account_id = account.id "
					 "WHERE player.id = %u", pTab->id);
			std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szQuery));
			if (pMsg->Get()->uiNumRows)
			{
				str_to_number(kPlayerInfo.wAccountbanCount, mysql_fetch_row(pMsg->Get()->pSQLResult)[0]);
			}
		}

		{
			char szQuery[1024 + 1];
			snprintf(szQuery, sizeof(szQuery), "SELECT lDuration FROM affect WHERE dwPID = %u AND bType = %u", pTab->id, 223);
			std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szQuery));
			if (pMsg->Get()->uiNumRows)
			{
				str_to_number(kPlayerInfo.dwDuration, mysql_fetch_row(pMsg->Get()->pSQLResult)[0]);
			}
		}
	}
	else
	{
		char szQuery[1024 + 1];
		snprintf(szQuery, sizeof(szQuery), "SELECT player.id, player.name, player.job, player.level, "
				 "player.chatban_count, account.ban_count, affect.lDuration FROM player "
				 "INNER JOIN account.account ON account.id = player.account_id "
				 "LEFT JOIN affect ON affect.dwPID = player.id AND affect.bType = %u "
				 "WHERE player.name = '%s'", 223, szEscapeName);
		sys_log(0, "Name[%s, %s] Query[%s]", szName, szEscapeName, szQuery);
		std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szQuery));
		if (!pMsg->Get()->uiNumRows)
		{
			pkPeer->EncodeHeader(HEADER_DG_ADMIN_MANAGER_CHAT_SEARCH_PLAYER, 0, sizeof(DWORD) + sizeof(BYTE));
			pkPeer->EncodeDWORD(dwGMPid);
			pkPeer->EncodeBYTE(false);
			return;
		}
		...
		str_to_number(kPlayerInfo.dwPID, row[col++]);
		strlcpy(kPlayerInfo.szName, row[col++], sizeof(kPlayerInfo.szName));
		str_to_number(kPlayerInfo.bRace, row[col++]);
		str_to_number(kPlayerInfo.bLevel, row[col++]);
		str_to_number(kPlayerInfo.wChatbanCount, row[col++]);
		str_to_number(kPlayerInfo.wAccountbanCount, row[col++]);
		str_to_number(kPlayerInfo.dwDuration, row[col++]);
	}

	pkPeer->EncodeHeader(HEADER_DG_ADMIN_MANAGER_CHAT_SEARCH_PLAYER, 0, sizeof(DWORD) + sizeof(BYTE) + sizeof(TAdminManagerBanClientPlayerInfo));
	pkPeer->EncodeDWORD(dwGMPid);
	pkPeer->EncodeBYTE(true);
	pkPeer->Encode(&kPlayerInfo, sizeof(kPlayerInfo));
}
#endif
...
void CClientManager::DestroyOfflineShop(uint32_t id)
{
	...
	std::pair<uint32_t, uint8_t> key = { id, 0 };
	for (uint8_t i = 0; i < OFFLINE_SHOP_ITEM_COUNT; ++i)
	{
		key.second = i;
...
#ifdef __DUNGEON_INFO_ENABLE__
void CClientManager::DungeonInfoRankingUpdate(CPeer * peer, DWORD dwHeader, const char * data)
{
	switch (dwHeader)
	{
		case HEADER_GD_DUNGEONINFO_RANKING:
			CDungeonInfoRankingManager::instance().RecvRankingUpdate((TPacketGDDIRanking *) data);
			break;
		case HEADER_GD_DUNGEONINFO_RANKING_BOOT:
		{
			TPacketGDDIRankingBoot * pack = (TPacketGDDIRankingBoot *) data;
			CDungeonInfoRankingManager::instance().RecvRankingConfig(pack->sKey, peer);
			break;
		}
		default:
			sys_err("Wtf are u doing?");
	}
}
#endif
